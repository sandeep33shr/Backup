...
#End Region
                ...
                If CType(Session(CNMode), Mode) = Mode.EditClaim Or CType(Session(CNMode), Mode) = Mode.PayClaim Or CType(Session(CNMode), Mode) = Mode.PayClaim _
                    ...
                    If (Session(CNDataSet) IsNot Nothing) Then
                        ...
                        If ProgressStatusValue = "CLOSED" Then
                            errorMessages.AddRange(ValidateStatus("Claim Decision", ClaimDecision, 5, ProgressStatusValue))
                            errorMessages.AddRange(ValidateStatus("Salvage Status", SalvageStatus, 1, ProgressStatusValue))
                            errorMessages.AddRange(ValidateStatus("Legal Status", LegalStatus, 15, ProgressStatusValue))
        ...
        Private Function ValidateStatus(statusName As String, actualStatus As Integer, expectedStatus As Integer, progressValue As String) As List(Of String)
            Dim errorMessages As New List(Of String)()
            If actualStatus = expectedStatus Then
                If progressValue = "CLOSED" Then
                    errorMessages.Add(String.Format("The {0} cannot be 'To be decided' when Progress Status is 'Closed'.", statusName))
                Else
                    errorMessages.Add(String.Format("The {0} to be updated for progress status to be set to 'Finalised'.", statusName))
                End If
            End If
