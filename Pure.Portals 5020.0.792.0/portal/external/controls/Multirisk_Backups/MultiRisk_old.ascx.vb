    ...
    Partial Class Controls_MultiRisk : Inherits System.Web.UI.UserControl
        ...
        Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
            ...
            If Not Page.IsPostBack Then
                ...
                End If
		'------------------------------------------------------------'
                '------------------START: AutoCancellation ------------------'
                '------------------------------------------------------------'
				If Session(CNMTAType) IsNot Nothing Then
                    If Session(CNMTAType) = MTAType.CANCELLATION Or Session(CNMTAType) = MTAType.REINSTATEMENT Then
                        Dim nRiskCount As Integer = 0
                        Dim conditionFlagCount As Integer = 0
                        'Only execute autoquote method when all risks are unquoted
                        For nRiskCount = 0 To oQuote.Risks.Count - 1
                            If oQuote.Risks(nRiskCount).StatusCode.ToUpper().Trim() = "UNQUOTED" Then
                                conditionFlagCount = conditionFlagCount + 1
                            End If
                        Next

                        If conditionFlagCount = nRiskCount Then
                            AutoQuoteAll()
                        End If
                    End If
				End If				
				'------------------------------------------------------------'
                '-------------------END: AutoCancellation -------------------'
                '------------------------------------------------------------'
				
				
				'------------------------------------------------------------'
                '-------------START: Autoquote Mandatory risk----------------'
                '------------------------------------------------------------'
                Dim nCount As Integer = 0
                Dim conditionFlag As Integer = 0
                Dim mandatoryRiskCount As Integer = 0
                'Only do the autoquoting when there's an unquoted mandatory risk, and all other optional risks are already quoted
                For nCount = 0 To oQuote.Risks.Count - 1
                    If Not oQuote.Risks(nCount).StatusCode Is Nothing Then
                        If oQuote.Risks(nCount).IsMandatoryRisk And oQuote.Risks(nCount).StatusCode.ToUpper().Trim() = "UNQUOTED" Then
                            mandatoryRiskCount = mandatoryRiskCount + 1
                            conditionFlag = conditionFlag + 1
                        End If

                        If (Not oQuote.Risks(nCount).IsMandatoryRisk) And oQuote.Risks(nCount).StatusCode.ToUpper().Trim() = "QUOTED" Then
                            conditionFlag = conditionFlag + 1
                        End If
                    End If
                ...
                If conditionFlag = nCount And mandatoryRiskCount > 0 Then
                    AutoQuoteAll()
                End If
                
                '------------------------------------------------------------'
                '--------------END: Autoquote Mandatory risk-----------------'
                '------------------------------------------------------------'
            End If
        End Sub
