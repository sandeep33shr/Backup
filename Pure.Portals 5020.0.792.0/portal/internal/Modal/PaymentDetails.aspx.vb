...
Namespace Nexus
    Partial Class Modal_PaymentDetails
                ...
                If Session.Item(CNClaim) IsNot Nothing Then
                    ...
                    If Session(CNMode) = Mode.PayClaim Then
                        ...
                        Else
                            ...
                            txtPaymentAmountDisabled.Text = String.Format(oFormatString, Math.Round(CDbl(txtCurrencyRate.Text.Trim) * Math.Round(CDbl(txtPaymentAmount.Text.Trim), 2), 2))
        ...
        Private Sub CheckClaimDecision()
            If Session(CNMode) = Mode.PayClaim Then
                Dim iPeril As Integer = CInt(Session(CNClaimPerilIndex))
                Dim iPaymentIndex As Integer = CInt(Request("PaymentIndex"))

                Dim oClaimOpen As NexusProvider.ClaimOpen = CType(Session.Item(CNClaim), NexusProvider.ClaimOpen)
                Dim ClaimDecision As Integer = 0
                        ...
                        For Each Node As XmlNode In oNodelist
                            If Node IsNot Nothing Then
                                If Node.Attributes.GetNamedItem("CLMDECISION") IsNot Nothing Then
                                    ...
                                    ' Value below indicates the claim decision based on the value you get 
                                    ' => 1 Approved
                                    ' => 2 Rejected
                                    ' => 3 Within Excess
                                    ' => 4 Withdrawn
                ...
                End If
			With oClaimOpen.ClaimPeril(iPeril)
                If .ClaimReserve(iPaymentIndex).IsIndemnity = True And ClaimDecision = 2 And .Payment.IsExGratia = False Then
                        Dim oCustomValidator As New CustomValidator
                        oCustomValidator.IsValid = False
                        oCustomValidator.ErrorMessage = "Payment for indemnity not allowed when Claim's decision status is set to Rejected."
                        oCustomValidator.Display = ValidatorDisplay.None 'we only want the error messages in the validation summary
                        oCustomValidator.ValidationGroup = "PaymentDetails"
                        'Add the validator to the page, this will have the effect of making the page invalid
                        Page.Validators.Add(oCustomValidator)
                    End If

                    'If payment is indemnity and claim descision is allowed.
                    If .ClaimReserve(iPaymentIndex).IsIndemnity = True And ClaimDecision <> 1 Then
                        Dim oCustomValidator As New CustomValidator
                        oCustomValidator.IsValid = False
                        oCustomValidator.ErrorMessage = "Payment is not allowed when Claim's decision status is not set to Approved."
                        oCustomValidator.Display = ValidatorDisplay.None 'we only want the error messages in the validation summary
                        oCustomValidator.ValidationGroup = "PaymentDetails"
                        'Add the validator to the page, this will have the effect of making the page invalid
                        Page.Validators.Add(oCustomValidator)
					...
					' ' If payment is indemnity and claim descision is allowed.
                    ' If .ClaimReserve(iPaymentIndex).IsIndemnity = false And ClaimDecision <> 1 Then
                        ' Dim oCustomValidator As New CustomValidator
                        ' oCustomValidator.IsValid = False
                        ' oCustomValidator.ErrorMessage = "Payment is not allowed when Claim's decision status is not set to Approved."
                        ' oCustomValidator.Display = ValidatorDisplay.None 'we only want the error messages in the validation summary
                    ...
                    'Expense payment is allowed except when decision is set to Input Error
                    If (.ClaimReserve(iPaymentIndex).TypeCode = "Ex Gratia" Or .ClaimReserve(iPaymentIndex).IsExpense) = True And ClaimDecision = 6 Then
                        Dim oCustomValidator As New CustomValidator
                        oCustomValidator.IsValid = False
                        oCustomValidator.ErrorMessage = "No payment is allowed when Claim Decision is 'Input Error'."
                        oCustomValidator.Display = ValidatorDisplay.None 'we only want the error messages in the validation summary
                        oCustomValidator.ValidationGroup = "PaymentDetails"
                        'Add the validator to the page, this will have the effect of making the page invalid
                        Page.Validators.Add(oCustomValidator)
                    ...
                    ''If payment is indemnity and claim descision is allowed.
                    'If .ClaimReserve(iPaymentIndex).IsIndemnity = True And ClaimDecision = 5 Then
                    '    Dim oCustomValidator As New CustomValidator
                    '    oCustomValidator.IsValid = False
                    '    oCustomValidator.ErrorMessage = "No payment is allowed when Claim Decision is 'To Be Decided'"
                    '    oCustomValidator.Display = ValidatorDisplay.None 'we only want the error messages in the validation summary
                    '    oCustomValidator.ValidationGroup = "PaymentDetails"
                    '    'Add the validator to the page, this will have the effect of making the page invalid
                    '    Page.Validators.Add(oCustomValidator)
