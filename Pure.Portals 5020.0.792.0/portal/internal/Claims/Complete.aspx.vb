...
#Region " Page Events "
        ...
        Protected Shadows Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
            ...
            If prodCode = "CMA" Or prodCode = "CMM" Or prodCode = "CFA" Or prodCode = "CFM" Or prodCode = "AHA" Or prodCode = "AHM" Or prodCode = "CTA" Or prodCode = "CTM" Or prodCode = "CTS" Or prodCode = "CAA" Or prodCode = "RTA" Or prodCode = "CCA" Or prodCode = "TSM" Or prodCode = "CCM" Or prodCode = "CBA" Or prodCode = "AAR" Or prodCode = "CHA" Or prodCode = "CHM" Or prodCode = "BMA" Or prodCode = "BMM" Or prodCode = "BMO" Or prodCode = "TAM" Or prodCode = "CAA" Or prodCode = "CAM" Then
                isSPM = False
            Else
            ...
            If isSPM = True Then
                ...
                Else
                    CreateNonMotorClaim()
                End If
                token = RequestToken(Trim(oOpenClaim.HandlerCode))
			Else
                CodeplexSession.Visible = False
                hypCodeplex.Visible = False
            ...
            If Session.Item(CNClaimNumber) IsNot Nothing Then
                ...
                If isSPM = True Then
                    ...
                    hypCodeplex.PostBackUrl = "https://codeplex.driveable.com.na/SPMUAT/Login.aspx?sMode=edit&sClaimNo=" & Trim(Session.Item(CNClaimNumber)) & "_" & BasePerilKey & "&sSessionToken=" & token & "&sUser =" & Trim(oOpenClaim.HandlerCode)
                End If
        ...
        Private Sub DocumentsDisplay()
            ...
            If Session(CNMode) = Mode.NewClaim Or Session.Item(CNMode) = Mode.NewClaim Then
                docMgr.Documents = "Treaty RI Notification,Large Loss Notification Treaty,Acknowledgement of Debt,TP Letter of Demand"
            ElseIf Session(CNMode) = Mode.EditClaim Or Session.Item(CNMode) = Mode.EditClaim Then
                docMgr.Documents = "Treaty RI Notification,Large Loss Notification Treaty,Acknowledgement of Debt,TP Letter of Demand"
            ElseIf Session(CNMode) = Mode.ViewClaim Or Session.Item(CNMode) = Mode.ViewClaim Then
                docMgr.Documents = "Treaty RI Notification,Large Loss Notification Treaty,Acknowledgement of Debt,TP Letter of Demand"
            ElseIf Session(CNMode) = Mode.PayClaim Or Session.Item(CNMode) = Mode.PayClaim Then
                docMgr.Documents = "Payment Confirmation Letter,Treaty RI Notification,Large Loss Notification Treaty"
            ElseIf Session(CNMode) = Mode.TPRecovery Or Session.Item(CNMode) = Mode.TPRecovery Then
                docMgr.Documents = "TP Letter of Demand"
            End If
        End Sub
        ...
        Private Function CreateMotorClaim(ByVal tagName As String) As String
            ...
            Dim SumInsured As Decimal = 0.00
            Dim ReserveAmount As Decimal = 0.00
            ...
            For Each item As NexusProvider.PerilSummary In oOpenClaim.ClaimPeril
				If (item.Reserve IsNot Nothing) Then
                    BasePerilKey = item.BaseClaimPerilKey
            ...
            If oNodelist.Count > 0 Then
                ...
                If (Trim(oClaimQuote.AgentCode) IsNot Nothing) And Not (Trim(oClaimQuote.AgentCode) = "") Then
                ...
                For Each Node As XmlNode In oNodelist
                    If Node IsNot Nothing Then
                        ...
                        If Node.Attributes("ALCOHOL") IsNot Nothing Then
                            Dim AlcoholValue As String
                            AlcoholValue = GetListItemDescfromCode("UserDefined", "YESNONA", Node.Attributes("ALCOHOL").Value)

                            If AlcoholValue = "Yes" Then
                                isAlcoholTest = "Y"
                            ElseIf AlcoholValue = "No" Then
                                isAlcoholTest = "N"
                            Else
                                isAlcoholTest = "U"
                            End If
                        ...
                        If Node.Attributes("DRIVERNAME") IsNot Nothing Then
                            DriverName = Node.Attributes("DRIVERNAME").Value
                ...
                SoapStr = SoapStr & "<ClaimSpecialist>" & Trim(oOpenClaim.HandlerCode) & "</ClaimSpecialist>"
                ...
                SoapStr = SoapStr & "<PerilID>" & Trim(oOpenClaim.PrimaryCauseCode) & "</PerilID>"
                ...
                SoapStr = SoapStr & "<IncidentDT>" & Trim(oOpenClaim.LossToDate.ToString("dd MMM yyyy")) & "</IncidentDT>"
                SoapStr = SoapStr & "<DateCaptured>" & Trim(oOpenClaim.ReportedDate.ToString("dd MMM yyyy")) & "</DateCaptured>"
        ...
        Private Function PostWebservice(ByVal soapAction As String, ByVal xmlBody As String, ByVal tagElement As String) As String
            ...
            Catch ex As WebException
                Page.Validators.Add(ex.ToString())
            End Try
