        ...
        Protected Shadows Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
            ...
            If Not Page.IsPostBack Then
                ...
                If oQuote IsNot Nothing Then
					...
					'Enable or Disable the Business Description Field
					EnableDescriptionField()
					
					'Check if there is already a Business Description Saved
...
"<script language=""javascript"" type=""text/javascript"">" & sScriptBuilder.ToString() & "</script>")
		...
		Private Sub GetBusinessDescription()
			Dim oQuote As NexusProvider.Quote = Session(CNQuote)
			Dim sBusinessDescription As String
			Dim Reader As SqlDataReader
			Dim constr As String = ConfigurationManager.ConnectionStrings("Pure").ConnectionString
			...
			Using objConn As New SqlConnection(constr)
                objConn.Open()
                Dim strSQL As New StringBuilder()
                strSQL.Append("hnam.GetDebitOrderPolicyHeaderDetails")

                Using cmd As New SqlCommand(strSQL.ToString, objConn)
                    cmd.CommandType = CommandType.StoredProcedure
                    cmd.Parameters.Add(New SqlParameter("@InsuranceFolderCnt", SqlDbType.Int, 0)).Value = oQuote.InsuranceFolderKey
					cmd.Parameters.Add(New SqlParameter("@InsuranceFileCnt", SqlDbType.Int, 0)).Value = oQuote.InsuranceFileKey
					
					
                    Reader = cmd.ExecuteReader()
                    While Reader.Read()
                        sBusinessDescription = Reader(0)
                    End While
                End Using
                objConn.Close()
            End Using

            If (sBusinessDescription <> "Null") Then
                POLICYHEADER__BUSINESSDESC.Text = sBusinessDescription
            End If
        End Sub
		
		Private Sub UpdateBusinessDescription()
            Dim oQuote As NexusProvider.Quote = Session(CNQuote)
            Dim constr As String = ConfigurationManager.ConnectionStrings("Pure").ConnectionString
            ...
            Using objConn As New SqlConnection(constr)
                objConn.Open()
                Dim strSQL As New StringBuilder()
                strSQL.Append("hnam.UpdateCommercialBusinessDescription")

                Using cmd As New SqlCommand(strSQL.ToString, objConn)
                    cmd.CommandType = CommandType.StoredProcedure
					cmd.Parameters.Add(New SqlParameter("@InsuranceFileCnt", SqlDbType.Int, 0)).Value = oQuote.InsuranceFileKey
                    cmd.Parameters.Add(New SqlParameter("@InsuranceFolderCnt", SqlDbType.Int, 0)).Value = oQuote.InsuranceFolderKey
					cmd.Parameters.Add(New SqlParameter("@BusinessDescription", SqlDbType.VarChar, 0)).Value = POLICYHEADER__BUSINESSDESC.Text
                    Dim rows As Integer
                    rows = cmd.ExecuteNonQuery()
                End Using
                objConn.Close()
		...
		Private Sub EnableDescriptionField()
			If oQuote.ProductCode = "CMA" Or oQuote.ProductCode = "CMM" or oQuote.ProductCode = "AHA" Or oQuote.ProductCode = "AHM" or oQuote.ProductCode = "CFA" Or oQuote.ProductCode = "CFM" or oQuote.ProductCode = "AAR" Or oQuote.ProductCode = "CHA" or oQuote.ProductCode = "TLA" Or oQuote.ProductCode = "TLM" or oQuote.ProductCode = "CCA" Or oQuote.ProductCode = "CCM" or oQuote.ProductCode = "CBA" Or oQuote.ProductCode = "BMA" or oQuote.ProductCode = "BMM" Or oQuote.ProductCode = "BMO" or oQuote.ProductCode = "BMSP" or oQuote.ProductCode = "CAA" Or oQuote.ProductCode = "CAM" or oQuote.ProductCode = "CTA" Or oQuote.ProductCode = "CTM" or oQuote.ProductCode = "CTS" Then
				vldrqdBusinessDesc.Enabled = True
				POLICYHEADER__BUSINESSDESC.CssClass = "form-control field-mandatory"
				POLICYHEADER__BUSINESSDESC.Enabled = True
			Else
				vldrqdBusinessDesc.Enabled = False
				POLICYHEADER__BUSINESSDESC.CssClass = "form-control"
				POLICYHEADER__BUSINESSDESC.Enabled = False
			End If
		End Sub
    End Class
End Namespace
