...
Namespace Nexus
    Partial Class PB2_TSMSNOTES_Note_Details : Inherits BaseRisk
		...
		Protected Sub onValidate_TAMSNOTES__NOTE_DETAILS()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        	If TAMSNOTES__Date_Created.Text.ToString = "" Then
        		TAMSNOTES__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        		TAMSNOTES__Created_By.Text = Session(CNLoginName).ToString
        	End If
        End If
End Sub
Protected Sub onValidate_TAMSNOTES__NoteCheck()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_TAMSNOTES__NOTE_DETAILS()
    onValidate_TAMSNOTES__NoteCheck()
End Sub
