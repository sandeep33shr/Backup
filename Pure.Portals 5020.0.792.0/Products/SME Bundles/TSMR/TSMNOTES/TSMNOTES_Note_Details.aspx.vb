...
Namespace Nexus
    Partial Class PB2_TSMNOTES_Note_Details : Inherits BaseRisk
		...
		Protected Sub onValidate_TAMNOTES__NOTE_DETAILS()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        	If TAMNOTES__Date_Created.Text.ToString = "" Then
        		TAMNOTES__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        		TAMNOTES__Created_By.Text = Session(CNLoginName).ToString
        	End If
        End If
End Sub
Protected Sub onValidate_TAMNOTES__NoteCheck()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_TAMNOTES__NOTE_DETAILS()
    onValidate_TAMNOTES__NoteCheck()
End Sub
