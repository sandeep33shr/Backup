...
Namespace Nexus
    Partial Class PB2_PCOMPEQRSK_Referral : Inherits BaseRisk
		...
		Protected Sub onValidate_REFERRALS__ShowHide()
...
Protected Sub CallRuleScripts()
    onValidate_REFERRALS__ShowHide()
...
End Sub
        ...
        Private Sub REFERRALS__REFERRAL_DETAILS_Load(sender As Object, e As EventArgs) Handles REFERRALS__REFERRAL_DETAILS.Load
        	HandleReferrals()
        End Sub
        
        Private Sub HandleReferrals()
        	Dim oQuote As NexusProvider.Quote = Session(CNQuote)
        	Dim oDoc As New XmlDocument
        	Dim xmlReferralNodes As XmlNodeList
        	Dim sXML As String = String.Empty
        	Dim iUserLevel As Integer
        	Dim iMinimumApprovalLevel As Integer
        	Dim iUnApprovedReferralsCount As Integer = 0
        	Dim iAwaitingApproval As Integer = 0
        	Dim sRefStatusCode As String = String.Empty
        
        	'Count referrals based on status
        	sXML = oQuote.Risks(Session(CNCurrentRiskKey)).XMLDataset
        	oDoc.LoadXml(sXML)
        	xmlReferralNodes = oDoc.SelectNodes("// " & Session(CNDataModelCode) & "_POLICY_BINDER/REFERRALS/REFERRAL_DETAILS")
        	If xmlReferralNodes IsNot Nothing Then
        		For Each xNode As XmlNode In xmlReferralNodes
        			If xNode.Attributes("REFSTATUS") IsNot Nothing Then
        				sRefStatusCode = xNode.Attributes("REFSTATUS").Value
        				If GetPMLookupItemCodefromID("UDL_REF_STATUS", xNode.Attributes("REFSTATUS").Value) = "APPREQ" Then
        					iUnApprovedReferralsCount = iUnApprovedReferralsCount + 1
        				End If
        				If GetPMLookupItemCodefromID("UDL_REF_STATUS", xNode.Attributes("REFSTATUS").Value) = "AWAITAPPRO" Then
        					iAwaitingApproval = iAwaitingApproval + 1
        				End If
        			End If
        		Next
        	End If
        
        	REFERRALS__PendingApprovalCount.Text = iAwaitingApproval
        	REFERRALS__RequestNotSentCount.Text = iUnApprovedReferralsCount
        			
        	Integer.TryParse(REFERRALS__UserLevel.Text, iUserLevel)
        	Integer.TryParse(REFERRALS__MinimumApprovalLevel.Text, iMinimumApprovalLevel)
        
        	'Enable Requestor comment and button box only when required
        	If (iUserLevel < iMinimumApprovalLevel) And (iUnApprovedReferralsCount > 0) Then
        
        		'Enable Requestor button and comment
        		btnRequestApproval.Enabled = True
        
        		'Disble Approver button and comment
        		btnSendApproval.Visible = False
        	Else
        		If iUnApprovedReferralsCount = 0 Then
        			''REFERRALS__RequestorComment.Enabled = False
        			btnRequestApproval.Visible = False
        		End If
        		If iUserLevel < iMinimumApprovalLevel Then
        			btnSendApproval.Visible = False
        		End If
        	End If
        
        	'Enable Approver's comment and button box only when required
        	If (iUserLevel >= iMinimumApprovalLevel) And (iAwaitingApproval > 0) Then
        
        		'Enable Approver button and comment
        		btnSendApproval.Visible = True
        
        		'Disable Requestor button and comment
        		btnRequestApproval.Visible = False
        	ElseIf (iAwaitingApproval = 0) Then
        		'Enable Approver button and comment
        		btnSendApproval.Visible = False
        	End If
        ...
        #Region "Referral Management"	
        	''' <summary>
        	''' Handles Request Approval Button Click Event
        	''' </summary>
        	...
        	'pdateXML(Current.Session(CNCurrentRiskKey), "REQUESTORCOMMENT", REFERRALS__REQUESTORCOMMENT.Text, "REFERRALS")
        
        	Call RequestApproval(sGroupCode)
        
        	ScriptManager.RegisterStartupScript(Me.Page, Me.Page.[GetType](), "showalert", "alert('Request Sent');", True)
        
        	End Sub
        
        	''' <summary>
        	''' 'Loops through all triggered referrals to authorise them in one goal
        	''' </summary>
        	...
        	Dim bRefStatusChanged As Boolean = False
        	...
        	Try
        		...
        		If objDataSet.Risk.Item("REFERRALS").Count("REFERRAL_DETAILS") > 0 Then
        			For iCount = 1 To objDataSet.Risk.Item("REFERRALS").Count("REFERRAL_DETAILS")
        				...
        				If sRefStatusCode = "APPREQ" Then
        					...
        					objDataSet.Risk.Item("REFERRALS").Item("REFERRAL_DETAILS", iCount).Item("REFSTATUSCODE").Value = "AWAITAPPRO"
        					bRefStatusChanged = True
        				End If
        			Next iCount
        		...
        		If sGroupCode <> "" And bRefStatusChanged Then
        			oUsersList = oWebService.GetUserGroupUsers(sGroupCode, DateTime.Now(), False, False, Nothing)
        			If oUsersList IsNot Nothing Then
        				For iUserCount As Integer = 0 To oUsersList.Count - 1
        					sEmailAddress = oUsersList(iUserCount).EmailAddress
        					sFullName = oUsersList(iUserCount).FullName
        					If sFullName = "" Then
        						sFullName = oUsersList(iUserCount).UserName
        					End If
        					If sEmailAddress <> "" Then
        						'Comment when Pure Send mail using Aspose is fixed
        						'SendApprovalRequest(oUsersList(iUserCount).EmailAddress, sFullName)
        
        						'Uncomment when Pure Send mail using Aspose is fixed
        						SendApprovalRequest_Pure(oUsersList(iUserCount).EmailAddress, sFullName)
        					End If
        				Next
        			End If
        	...
        	Protected Sub SendApprovalRequest_Pure(ByVal sRecipientEmail As String, ByVal sRecipientName As String)
        	...
        	sSubject = "Policy Referral - " & Trim(oQuote.InsuranceFileRef) & " for " & sInsuredName
        	sMessage = "Hi " & sRecipientName & vbCrLf & vbCrLf
        	sMessage = sMessage & "Please have a look at Quotation reference " & Trim(oQuote.InsuranceFileRef) & " for " & sInsuredName & " which has been referred To the authorisation group."
        	...
        	Dim sInsuredName As String = String.Empty
        	Dim sSenderName As String = GetValue("GENERAL", "LOGGEDINUSERFULLNAME")
        	Dim oSmtp_Server As New System.Net.Mail.SmtpClient
        	Dim oMail As New System.Net.Mail.MailMessage()
        	Dim sSMTPServer = AppSettings("SMTPServer")
        	Dim sRelayEmail = AppSettings("RelayEmail")
        	Dim sRelayEmailPassword = AppSettings("RelayEmailPassword")
        	Dim bEmailSent As Boolean = False
        	Dim oQuote As NexusProvider.Quote = Session(CNQuote)
        	...
        	sMessage = "Hi " & sRecipientName & vbCrLf & vbCrLf
        	sMessage = sMessage & "Please have a look at Quotation reference " & Trim(oQuote.InsuranceFileRef) & " for " & sInsuredName & " which has been referred To the authorisation group."
        	If REFERRALS__REQUESTORCOMMENT.Value <> "" Then
        		sMessage = sMessage & vbCrLf & vbCrLf & "Extra Information: " & REFERRALS__REQUESTORCOMMENT.Value
        	End If
        	sMessage = sMessage & vbCrLf & vbCrLf & "Regards"
        	sMessage = sMessage & vbCrLf & vbCrLf & sSenderName
        	...
        	Try
        		...
        		oSmtp_Server.Credentials = New Net.NetworkCredential(sRelayEmail, sRelayEmailPassword)
        		oSmtp_Server.Port = 587
        		oSmtp_Server.EnableSsl = True
        		oSmtp_Server.Host = sSMTPServer
        
        		oMail = New System.Net.Mail.MailMessage()
        		oMail.From = New System.Net.Mail.MailAddress(sRelayEmail)
        
        		sRecipientEmail = sRecipientEmail
        
        		oMail.To.Add(sRecipientEmail)
        		oMail.Subject = sSubject
        		oMail.IsBodyHtml = False
        		oMail.Body = sMessage
        		oSmtp_Server.Send(oMail)
        
        	Catch errorError As Exception
        		'MsgBox(errorError.ToString)
        	End Try
        	...
        	If sTransactionType = "NB" Or sTransactionType = "MTA" Then
        		oWorkManager.Description = sSenderName & " has referred quote no " & Trim(oQuote.InsuranceFileRef) & " for " & sInsuredName & " and it's awaiting action."
        	...
        	ElseIf sTransactionType = "REN" Then
        		oWorkManager.Description = sSenderName & " has referred quote no " & Trim(oQuote.InsuranceFileRef) & " for " & sInsuredName & " And it 's awaiting review."
        		oWorkManager.Task = "RENAMEND"
        		oWorkManager.TaskGroup = "UWRENEWAL"
        	End If
        	oWorkManager.AllocationUserGroup = sGroupCode
        ...
        #Region "Generic methods and functions"
                ''' <summary>
                ...
                Protected Sub UpdateXML(ByVal iRisk As Integer, ByVal sFieldName As String, ByVal sFieldValue As String, ByVal sObjectName As String)
                    Dim oWebService As NexusProvider.ProviderBase = Nothing
                    ...
                    oDataSet.LoadFromXML(sDataSetDefinition, oQuote.Risks(iRisk).XMLDataset)
        
                    Dim srXMLDataset As String = oQuote.Risks(iRisk).XMLDataset
                    Dim oDoc As New XmlDocument
                    ...
                    oDataSet.ReturnAsXML(oQuote.Risks(iRisk).XMLDataset)
        ...
        #Region "Referral Approval Request"
                ...
                Private Sub ApproveReferral()
                    ...
                    Try
                        ...
                        If objDataSet.Risk.Item("REFERRALS").Count("REFERRAL_DETAILS") > 0 Then
                            For iCount = 1 To objDataSet.Risk.Item("REFERRALS").Count("REFERRAL_DETAILS")
                                ...
                                If sRefStatusCode = "AWAITAPPRO" Then
                                    ...
                                    objDataSet.Risk.Item("REFERRALS").Item("REFERRAL_DETAILS", iCount).Item("RefStatus").Value = iRefStatusId
                                    objDataSet.Risk.Item("REFERRALS").Item("REFERRAL_DETAILS", iCount).Item("REFSTATUSCODE").Value = "APPROVED"
                        ...
                        If sUser <> "" Then
                            ...
                            For iCount = 0 To oUserDetails.AvailableUsergroups.Count - 1
                                If oUserDetails.AvailableUsergroups(iCount).Code.Trim() <> "SAMADMIN" Then
                                    sGroupCode = oUserDetails.AvailableUsergroups(iCount).Code.Trim()
                                    Exit For
                                End If
                            Next
                            ...
                            If Not strEmailAddresses Is Nothing Then
                                ...
                                'Comment after sending main through Pure/Aspose is fixed
                                'Call SendEmailToGenerator(strEmailAddresses.ToString, sUserFullName)
        
                                'Uncomment after sending main through Pure/Aspose is fixed
                                Call SendEmailToGenerator_Pure(strEmailAddresses.ToString, sUserFullName)
                            End If
                ...
                '''' <summary>
                '''' Send an approval/decline email to the underwriter who sent the request
                '''' </summary>
                '''' <param name="sRecipientEmail"></param>
                '''' <param name="sRecipientName"></param>
                Protected Sub SendEmailToGenerator_Pure(ByVal sRecipientEmail As String, ByVal sRecipientName As String)
                    ...
                    'Build e-mail subject          
                    sMessage = "Hi " & sRecipientName & vbCrLf & vbCrLf
                    sMessage = sMessage & "Quotation reference " & Trim(oQuote.InsuranceFileRef) & " for " & sInsuredName & " which you referred for approval has been actioned. Please proceed to finalise the quote."
                    sMessage = sMessage & vbCrLf & vbCrLf & "Regards"
                    sMessage = sMessage & vbCrLf & vbCrLf & sSenderName
                ...
                Protected Sub SendEmailToGenerator(ByVal sRecipientEmail As String, ByVal sRecipientName As String)
                    ...
                    If REFERRALS__APPROVERCOMMENT.Value <> "" Then
                        sMessage = sMessage & vbCrLf & vbCrLf & "Extra Information: " & REFERRALS__APPROVERCOMMENT.Value
                    End If
                ...
                ''' <summary>
                '''  create an approval/decline email to the underwriter who sent the request
                ''' </summary>
                ''' <param name="sGroupCode"></param>
                ...
                Private Sub CreateWorkManagerForGenerator(ByVal sGroupCode As String, ByRef sUser As String)
                    ...
                    If sTransactionType = "NB" Or sTransactionType = "MTA" Then
                        oWorkManager.Description = "Quote number " + Trim(oQuote.InsuranceFileRef) + " which you referred for approval has been actioned. Please proceed to finalise the quote"
                        If sTransactionType = "NB" Then
                            oWorkManager.Task = "UNDERNB"
                        End If
                        If sTransactionType = "MTA" Then
                        ...
                        oWorkManager.TaskGroup = "UNDER"
                    ElseIf sTransactionType = "REN" Then
                        oWorkManager.Description = "Renewal Quote number " + Trim(oQuote.InsuranceFileRef) + " which you referred for approval has been actioned. Please proceed to finalise the quote"
                        oWorkManager.Task = "RENAMEND"
                        oWorkManager.TaskGroup = "UWRENEWAL"
                    End If
                    oWorkManager.AllocationUserGroup = sGroupCode
                    oWorkManager.AllocationUser = sUser
                    'oWorkManager.AllocationUser = "(Any Group Member)"
