...
Namespace Nexus
    Partial Class PB2_MTNOTES_Note_Details : Inherits BaseRisk
		...
		Protected Sub onValidate_MTNOTE_DETAILS__NOTE_DETAILS()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        	If MTNOTE_DETAILS__Date_Created.Text.ToString = "" Then
        		MTNOTE_DETAILS__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        		MTNOTE_DETAILS__Created_By.Text = Session(CNLoginName).ToString
        	End If
        End If
End Sub
Protected Sub onValidate_MTNOTE_DETAILS__NoteCheck()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_MTNOTE_DETAILS__NOTE_DETAILS()
    onValidate_MTNOTE_DETAILS__NoteCheck()
End Sub

		    
        
        Protected Sub btnNextAlarm_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnNext.Click, btnNextTop.Click, btnFinish.Click, btnFinishTop.Click
        	Dim NoteAddedCheck = GetValue("{Object}", "{Property}")
        	
        	If NoteAddedCheck = 0 Then
        		MTNOTE_DETAILS__NoteCheck.Text = 1
        	End If
        End Sub
