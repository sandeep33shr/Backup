...
Namespace Nexus
    Partial Class PB2_DTNOVALUE_Non_Band_Deductibles_Details : Inherits BaseRisk
		...
		Protected Sub onValidate_DED_NON_BAND__COUNTER_ID()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_DED_NON_BAND__COUNTER_ID()
End Sub

		    
        'DED_NON_BAND - Parent object
        'DED_BAND - Child object
        		 ...
        		 Dim oQuote As NexusProvider.Quote = Session(CNQuote)
                    ...
                    If (transactionValue <> "VIEW") Then
                        ...
                        For index = 0 To oQuote.Risks.Count - 1
                            ...
                            Dim sChildId As String = GetAutoNumber("//" & "DED_NON_BAND" & "/" & "DED_BAND", sXMLDataset)
                            If sChildId <> "0" Then
                                DED_BAND__COUNTER_ID.Text = "I" & sChildId
                            End If
                        Next
