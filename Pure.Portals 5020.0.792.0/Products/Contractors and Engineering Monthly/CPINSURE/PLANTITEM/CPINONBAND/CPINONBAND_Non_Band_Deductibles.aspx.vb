...
Namespace Nexus
    Partial Class PB2_CPINONBAND_Non_Band_Deductibles : Inherits BaseRisk
		...
		Protected Sub onValidate_NONBAND__COUNTER_ID()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_NONBAND__COUNTER_ID()
End Sub

		    
        'PLANTDET - Parent object
        'NONBAND - Child object
        		 ...
        		 Dim oQuote As NexusProvider.Quote = Session(CNQuote)
                    ...
                    If (transactionValue <> "VIEW") Then
                        ...
                        For index = 0 To oQuote.Risks.Count - 1
                            ...
                            Dim sChildId As String = GetAutoNumber("//" & "PLANTDET" & "/" & "NONBAND", sXMLDataset)
                            If sChildId <> "0" Then
                                NONBAND__COUNTER_ID.Text = "I" & sChildId
                            End If
                        Next
