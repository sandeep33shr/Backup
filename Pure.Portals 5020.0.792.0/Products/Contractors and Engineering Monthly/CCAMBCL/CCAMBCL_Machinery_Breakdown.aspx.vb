...
Namespace Nexus
    Partial Class PB2_CCAMBCL_Machinery_Breakdown : Inherits BaseRisk
		...
		Protected Sub onValidate_MBREAK__ATTACHMENTDATE()
        
        
End Sub
Protected Sub onValidate_MBREAK__EFFECTIVEDATE()
        
End Sub
Protected Sub onValidate_SEQCNT_MB__COUNT()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_MBREAK__ATTACHMENTDATE()
    onValidate_MBREAK__EFFECTIVEDATE()
    onValidate_SEQCNT_MB__COUNT()
End Sub
        ...
        Protected Shadows Sub Page_Load_AttachmentDate(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	...
        	If MBREAK__ATTACHMENTDATE.Text = "" or MBREAK__ATTACHMENTDATE.Text Is Nothing Then
        		If transactionValue = "NB" Then
        			MBREAK__ATTACHMENTDATE.Text = RTrim(LTrim(oQuote.InceptionDate))
        		Else
        			MBREAK__ATTACHMENTDATE.Text = RTrim(LTrim(oQuote.CoverStartDate))
        		End If
        	End If
        	
        	If oQuote.Risks(Session(CNCurrentRiskKey)).RiskInceptionDate <> oQuote.InceptionDate And (MBREAK__ATTACHMENTDATE.Text <> "" Or MBREAK__ATTACHMENTDATE.Text IsNot Nothing) Then
        		If transactionValue <> "NB" Then
        ...
        Protected Shadows Sub Page_Load_EffectiveDate(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	...
        	If transactionValue = "NB" Then
        		MBREAK__EFFECTIVEDATE.Text = RTrim(LTrim(oQuote.InceptionDate))
        	End If
        	
        	If transactionValue = "MTA" Then
        		MBREAK__EFFECTIVEDATE.Text = RTrim(LTrim(oQuote.CoverStartDate))
        	End If
        	
        	If transactionValue = "REN" Then
        		MBREAK__EFFECTIVEDATE.Text = RTrim(LTrim(oQuote.InceptionTPI))
        	End If
        ...
        End Sub
        		...
        		Private Sub NumberLocacationItems()
                    ...
                    Try
                        'Number all MBREAKs  sequentially for New Business transaction type 
                        If sTransactionType = "NB"  Then
                            If objDataSet.Risk.Item("MBREAK").Count("MB_ITEM") > 0 Then
                                For icount As Integer = 1 To objDataSet.Risk.Item("MBREAK").Count("MB_ITEM")
                                    objDataSet.Risk.Item("MBREAK").Item("MB_ITEM", icount).Item("INCR").Value = icount
                                    objDataSet.Risk.Item("MBREAK").Item("MB_ITEM", icount).Item("LASTITEMNUM").Value = objDataSet.Risk.Item("MBREAK").Count("MB_ITEM")
        							objDataSet.Risk.Item("MBREAK").Item("MB_ITEM", icount).Item("ISLASTLASTITEMSET").Value = 1
                                Next
                        ...
                        ElseIf sTransactionType = "MTA" Or sTransactionType = "REN" Or sTransactionType = "MTR" Then
                            ...
                            'Set a session variable to keep the last MBREAK number
                            If objDataSet.Risk.Item("MBREAK").Count("MB_ITEM") = 0 Then
                                Session("LastItemNo") = 0
                            ElseIf objDataSet.Risk.Item("MBREAK").Count("MB_ITEM") > 0 Then
                                Session("LastItemNo") = objDataSet.Risk.Item("MBREAK").Item("MB_ITEM", 1).Item("LASTITEMNUM").Value
                            End If
        
                            Integer.TryParse(Session("LastItemNo"), iNextItemNo)
        
                            'Number only MBREAKs being added in this session
                            If objDataSet.Risk.Item("MBREAK").Count("MB_ITEM") > 0 Then
                                For icount As Integer = 1 To objDataSet.Risk.Item("MBREAK").Count("MB_ITEM")
                                    If objDataSet.Risk.Item("MBREAK").Item("MB_ITEM", icount).Item("ISLASTLASTITEMSET").Value <> 1 Then
                                        iCountAfterNB = iCountAfterNB + 1
                                        objDataSet.Risk.Item("MBREAK").Item("MB_ITEM", icount).Item("INCR").Value = iNextItemNo + iCountAfterNB
                                    End If
                                Next
                                objDataSet.ReturnAsXML(sDataSet)
                                oQuote.Risks(Session(CNCurrentRiskKey)).XMLDataset = sDataSet
        			...
        			If Session("Refresh") = "1" Then
        				Dim sUrl As String = "/Products/Construction%20and%20Engineering%20Annual/CCAMBCL/CCAMBCL_Machinery_Breakdown.aspx"
