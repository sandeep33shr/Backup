...
Namespace Nexus
    Partial Class PB2_DSENDPRE_Endorsement_Premium : Inherits BaseRisk
		...
		Protected Sub onValidate_DS_CLAUSEPREM__COUNTER_ID()
        
End Sub
Protected Sub onValidate_DS_CLAUSEPREM__ENDORSE_ID()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_DS_CLAUSEPREM__COUNTER_ID()
    onValidate_DS_CLAUSEPREM__ENDORSE_ID()
End Sub

		    
        'DSTOCK - Parent object
        'DS_CLAUSEPREM - Child object
        		 ...
        		 Dim oQuote As NexusProvider.Quote = Session(CNQuote)
                    ...
                    If (transactionValue <> "VIEW") Then
                        ...
                        For index = 0 To oQuote.Risks.Count - 1
                            ...
                            Dim sChildId As String = GetAutoNumber("//" & "DSTOCK" & "/" & "DS_CLAUSEPREM", sXMLDataset)
                            If sChildId <> "0" Then
                                DS_CLAUSEPREM__COUNTER_ID.Text = "I" & sChildId
                            End If
                        Next
        ...
        'Parameters:
        'DSTOCK - Standard Wording Parent Object
        'REFERRAL_CLAUSES - Standard Wording Parent Property
        'DS_CLAUSEPREM - Child Screen Object
        ...
        Protected Shadows Sub Page_Load_SelectedEndorsements(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	...
        	Dim Doc As New XmlDocument
        	Doc.Load(xmlTR)
        	Dim xmlLocations As XmlNodeList = Doc.SelectNodes("//" & Session.Item(CNDataModelCode) & "_POLICY_BINDER/DSTOCK/SW.REFERRAL_CLAUSES")
        	Dim EndorsementScheme As Object = oMaster.FindControl("DS_CLAUSEPREM__ENDORSE_DESC")
        ...
        Protected Shadows Sub btnNextFinish_SelectedEndorsements(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnNext.Click, btnFinish.Click, btnNextTop.Click, btnFinishTop.Click
        	...
        	Dim Doc As New XmlDocument
        	Doc.Load(xmlTR)
        	Dim xmlLocations As XmlNodeList = Doc.SelectNodes("//" & Session.Item(CNDataModelCode) & "_POLICY_BINDER/DSTOCK/DS_CLAUSEPREM")
        
        	Dim CurrentSelected As String = DS_CLAUSEPREM__ENDORSE_CAP.Text
