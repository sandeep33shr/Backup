...
Namespace Nexus
    Partial Class PB2_MBENDPRE_Endorsement_Premium : Inherits BaseRisk
		...
		Protected Sub onValidate_MB_CLAUSEPREM__COUNTER_ID()
        
End Sub
Protected Sub onValidate_MB_CLAUSEPREM__ENDORSE_ID()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_MB_CLAUSEPREM__COUNTER_ID()
    onValidate_MB_CLAUSEPREM__ENDORSE_ID()
End Sub

		    
        'MBREAK - Parent object
        'MB_CLAUSEPREM - Child object
        		 ...
        		 Dim oQuote As NexusProvider.Quote = Session(CNQuote)
                    ...
                    If (transactionValue <> "VIEW") Then
                        ...
                        For index = 0 To oQuote.Risks.Count - 1
                            ...
                            Dim sChildId As String = GetAutoNumber("//" & "MBREAK" & "/" & "MB_CLAUSEPREM", sXMLDataset)
                            If sChildId <> "0" Then
                                MB_CLAUSEPREM__COUNTER_ID.Text = "I" & sChildId
                            End If
                        Next
        ...
        'Parameters:
        'MBREAK - Standard Wording Parent Object
        'REFFERRAL_CLAUSES - Standard Wording Parent Property
        'MB_CLAUSEPREM - Child Screen Object
        ...
        Protected Shadows Sub Page_Load_SelectedEndorsements(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	...
        	Dim Doc As New XmlDocument
        	Doc.Load(xmlTR)
        	Dim xmlLocations As XmlNodeList = Doc.SelectNodes("//" & Session.Item(CNDataModelCode) & "_POLICY_BINDER/MBREAK/SW.REFFERRAL_CLAUSES")
        	Dim EndorsementScheme As Object = oMaster.FindControl("MB_CLAUSEPREM__ENDORSE_DESC")
        ...
        Protected Shadows Sub btnNextFinish_SelectedEndorsements(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnNext.Click, btnFinish.Click, btnNextTop.Click, btnFinishTop.Click
        	...
        	Dim Doc As New XmlDocument
        	Doc.Load(xmlTR)
        	Dim xmlLocations As XmlNodeList = Doc.SelectNodes("//" & Session.Item(CNDataModelCode) & "_POLICY_BINDER/MBREAK/MB_CLAUSEPREM")
        
        	Dim CurrentSelected As String = MB_CLAUSEPREM__ENDORSE_CAP.Text
