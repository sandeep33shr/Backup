...
Namespace Nexus
    Partial Class PB2_MBSNOTE_Note_Details : Inherits BaseRisk
		...
		Protected Sub onValidate_MB_SNOTES__NOTE_DETAILS()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        	If MB_SNOTES__Date_Created.Text.ToString = "" Then
        		MB_SNOTES__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        		MB_SNOTES__Created_By.Text = Session(CNLoginName).ToString
        	End If
        End If
End Sub
Protected Sub onValidate_MB_SNOTES__NoteCheck()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_MB_SNOTES__NOTE_DETAILS()
    onValidate_MB_SNOTES__NoteCheck()
End Sub
