...
Namespace Nexus
    Partial Class PB2_CCAEEQ_Electronic_Equipment : Inherits BaseRisk
...
Protected Sub onValidate_GENERAL__TRANSTYPE()
        ...
        End If
End Sub
Protected Sub onValidate_GENERAL__INS_FILE_CNT()
...
End Sub
Protected Sub onValidate_GENERAL__PRIMARY_INDUSTRY()
        If eLifecycleEvent = LifecycleEvent.Page_Load
...
End Sub
Protected Sub onValidate_ELECTEQUIP__ATTACHMENTDATE()
        
        
End Sub
Protected Sub onValidate_ELECTEQUIP__EFFECTIVEDATE()
        
End Sub
Protected Sub onValidate_SEQCNT_EE__COUNT()
...
Protected Sub onValidate_GENERAL__TRANSACTION_TYPE()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        		...
        		Else
        			transactionValue = "NB"
        		End  If	
        		GENERAL__TRANSACTION_TYPE.Text= transactionValue
        End If
End Sub
Protected Sub CallRuleScripts()
    onValidate_GENERAL__PRODCODE()
    onValidate_GENERAL__TRANSTYPE()
    onValidate_GENERAL__INS_FILE_CNT()
    onValidate_ADDRESS__TOWN()
    onValidate_ADDRESS__COUNTRY()
    onValidate_GENERAL__PRIMARY_INDUSTRY()
    onValidate_ELECTEQUIP__ATTACHMENTDATE()
    onValidate_ELECTEQUIP__EFFECTIVEDATE()
    onValidate_SEQCNT_EE__COUNT()
...
End Sub
        ...
        Protected Shadows Sub SetFileCount(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	Dim oQuote As NexusProvider.Quote = Session(CNQuote)
        	Dim FileCnt As Integer = oQuote.InsuranceFileKey
        	Dim value As Integer
        	Integer.TryParse(GENERAL__INS_FILE_CNT.Text, value)
        	
        	'Compare if the previous value set on GENERAL__INS_FILE_CNT as File count is the same as the new File count
        	If (value <> FileCnt) Then
        		
        		GENERAL__CHECK_ELECEQ.Value = ""
        		GENERAL__INS_FILE_CNT.Text = oQuote.InsuranceFileKey
        	End If
        ...
        Protected Sub SetIndustry()
        	oWebService = New NexusProvider.ProviderManager().Provider
        	Dim oQuote As NexusProvider.Quote = Session(CNQuote)
            If oWebService.GetParty(oQuote.PartyKey).GetType.Name = "CorporateParty" Then
        		Dim oParty As NexusProvider.CorporateParty = oWebService.GetParty(oQuote.PartyKey)
        		Dim PrimaryKey As String = GetDatafromXML("//CCLIENT", "PRIMARY_INDUSTRY", oParty.XMLDataset)
        		Dim SecondaryKey As String = GetDatafromXML("//CCLIENT", "SECOND_INDUSTRY", oParty.XMLDataset)
        		Dim TertiaryKey As String = GetDatafromXML("//CCLIENT", "TERTIARY_INDUSTRY", oParty.XMLDataset)
        		Dim IndustryKey As String = GetDatafromXML("//CCLIENT", "INDUSTRY", oParty.XMLDataset)
        		Dim ListIndOne = oWebService.GetList(NexusProvider.ListType.UserDefined, "CMA_INDONE", True, True, Nothing, -1, Nothing)
        		Dim ListIndTwo = oWebService.GetList(NexusProvider.ListType.UserDefined, "CMA_INDTWO", True, True, Nothing, -1, Nothing)
        		Dim ListIndThree = oWebService.GetList(NexusProvider.ListType.UserDefined, "CMA_INDTHR", True, True, Nothing, -1, Nothing)
        		Dim ListInd = oWebService.GetList(NexusProvider.ListType.UserDefined, "CMA_INDUST", True, True, Nothing, -1, Nothing)
        		
        		If PrimaryKey <> "" Then
        			If ListIndOne.FindItemByKey(PrimaryKey).ToString() IsNot Nothing Then
        				If GENERAL__PRIMARY_INDUSTRY.Value Is Nothing Then
        					GENERAL__PRIMARY_INDUSTRY.Value = PrimaryKey
        				End If
        			End If
        
        			If ListIndTwo.FindItemByKey(SecondaryKey).ToString() IsNot Nothing Then
        				If GENERAL__SECOND_INDUSTRY.Value Is Nothing Then
        					GENERAL__SECOND_INDUSTRY.Value = SecondaryKey
        				End If
        			End If
        
        			If ListIndThree.FindItemByKey(TertiaryKey).ToString() IsNot Nothing Then
        				If GENERAL__TERTIARY_INDUSTRY.Value Is Nothing Then
        					GENERAL__TERTIARY_INDUSTRY.Value = TertiaryKey
        				End If
        			End If
        
        			If ListInd.FindItemByKey(IndustryKey).ToString() IsNot Nothing Then
        				If GENERAL__INDUSTRY.Value Is Nothing Then
        					GENERAL__INDUSTRY.Value = IndustryKey
        				End If
        			End If
        		End If
        	End If
        ...
        Protected Shadows Sub Page_Load_AttachmentDate(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	...
        	If ELECTEQUIP__ATTACHMENTDATE.Text = "" or ELECTEQUIP__ATTACHMENTDATE.Text Is Nothing Then
        		If transactionValue = "NB" Then
        			ELECTEQUIP__ATTACHMENTDATE.Text = RTrim(LTrim(oQuote.InceptionDate))
        		Else
        			ELECTEQUIP__ATTACHMENTDATE.Text = RTrim(LTrim(oQuote.CoverStartDate))
        		End If
        	End If
        	
        	If oQuote.Risks(Session(CNCurrentRiskKey)).RiskInceptionDate <> oQuote.InceptionDate And (ELECTEQUIP__ATTACHMENTDATE.Text <> "" Or ELECTEQUIP__ATTACHMENTDATE.Text IsNot Nothing) Then
        		If transactionValue <> "NB" Then
        ...
        Protected Shadows Sub Page_Load_EffectiveDate(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	...
        	If transactionValue = "NB" Then
        		ELECTEQUIP__EFFECTIVEDATE.Text = RTrim(LTrim(oQuote.InceptionDate))
        	End If
        	
        	If transactionValue = "MTA" Then
        		ELECTEQUIP__EFFECTIVEDATE.Text = RTrim(LTrim(oQuote.CoverStartDate))
        	End If
        	
        	If transactionValue = "REN" Then
        		ELECTEQUIP__EFFECTIVEDATE.Text = RTrim(LTrim(oQuote.InceptionTPI))
        	End If
        ...
        End Sub
        		...
        		Private Sub NumberLocacationItems()
                    ...
                    Try
                        'Number all ELECTEQUIPs  sequentially for New Business transaction type 
                        If sTransactionType = "NB"  Then
                            If objDataSet.Risk.Item("ELECTEQUIP").Count("ELECTEQ") > 0 Then
                                For icount As Integer = 1 To objDataSet.Risk.Item("ELECTEQUIP").Count("ELECTEQ")
                                    objDataSet.Risk.Item("ELECTEQUIP").Item("ELECTEQ", icount).Item("INCR").Value = icount
                                    objDataSet.Risk.Item("ELECTEQUIP").Item("ELECTEQ", icount).Item("LASTITEMNUM").Value = objDataSet.Risk.Item("ELECTEQUIP").Count("ELECTEQ")
        							objDataSet.Risk.Item("ELECTEQUIP").Item("ELECTEQ", icount).Item("ISLASTLASTITEMSET").Value = 1
                                Next
                        ...
                        ElseIf sTransactionType = "MTA" Or sTransactionType = "REN" Or sTransactionType = "MTR" Then
                            ...
                            'Set a session variable to keep the last ELECTEQUIP number
                            If objDataSet.Risk.Item("ELECTEQUIP").Count("ELECTEQ") = 0 Then
                                Session("LastItemNo") = 0
                            ElseIf objDataSet.Risk.Item("ELECTEQUIP").Count("ELECTEQ") > 0 Then
                                Session("LastItemNo") = objDataSet.Risk.Item("ELECTEQUIP").Item("ELECTEQ", 1).Item("LASTITEMNUM").Value
                            End If
        
                            Integer.TryParse(Session("LastItemNo"), iNextItemNo)
        
                            'Number only ELECTEQUIPs being added in this session
                            If objDataSet.Risk.Item("ELECTEQUIP").Count("ELECTEQ") > 0 Then
                                For icount As Integer = 1 To objDataSet.Risk.Item("ELECTEQUIP").Count("ELECTEQ")
                                    If objDataSet.Risk.Item("ELECTEQUIP").Item("ELECTEQ", icount).Item("ISLASTLASTITEMSET").Value <> 1 Then
                                        iCountAfterNB = iCountAfterNB + 1
                                        objDataSet.Risk.Item("ELECTEQUIP").Item("ELECTEQ", icount).Item("INCR").Value = iNextItemNo + iCountAfterNB
                                    End If
                                Next
                                objDataSet.ReturnAsXML(sDataSet)
                                oQuote.Risks(Session(CNCurrentRiskKey)).XMLDataset = sDataSet
        			...
        			If Session("Refresh") = "1" Then
        				Dim sUrl As String = "/Products/Construction%20and%20Engineering%20Annual/CCAEEQ/CCAEEQ_Electronic_Equipment.aspx"
