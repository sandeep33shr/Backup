...
Namespace Nexus
    Partial Class PB2_CWONOTES_Note_Details : Inherits BaseRisk
		...
		Protected Sub onValidate_CWNOTES__NOTE_DETAILS()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        	If CWNOTES__Date_Created.Text.ToString = "" Then
        		CWNOTES__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        		CWNOTES__Created_By.Text = Session(CNLoginName).ToString
        	End If
        End If
End Sub
Protected Sub onValidate_CWNOTES__NoteCheck()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_CWNOTES__NOTE_DETAILS()
    onValidate_CWNOTES__NoteCheck()
End Sub
