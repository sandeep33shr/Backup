...
Namespace Nexus
    Partial Class PB2_CCWNOTES_Notes_Details : Inherits BaseRisk
		...
		Protected Sub onValidate_CWSNOTES__NOTE_DETAILS()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        	If CWSNOTES__Date_Created.Text.ToString = "" Then
        		CWSNOTES__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        		CWSNOTES__Created_By.Text = Session(CNLoginName).ToString
        	End If
        End If
End Sub
Protected Sub onValidate_CWSNOTES__NoteCheck()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_CWSNOTES__NOTE_DETAILS()
    onValidate_CWSNOTES__NoteCheck()
End Sub
