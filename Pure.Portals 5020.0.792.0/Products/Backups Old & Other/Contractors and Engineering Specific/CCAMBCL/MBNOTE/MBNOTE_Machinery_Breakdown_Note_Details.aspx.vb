...
Namespace Nexus
    Partial Class PB2_MBNOTE_Machinery_Breakdown_Note_Details : Inherits BaseRisk
		...
		Protected Sub onValidate_MB_NOTES__NOTE_DETAILS()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        	If MB_NOTES__Date_Created.Text.ToString = "" Then
        		MB_NOTES__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        		MB_NOTES__Created_By.Text = Session(CNLoginName).ToString
        	End If
        End If
End Sub
Protected Sub onValidate_MB_NOTES__NoteCheck()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_MB_NOTES__NOTE_DETAILS()
    onValidate_MB_NOTES__NoteCheck()
End Sub
