...
Namespace Nexus
    Partial Class PB2_MSSNOTES_Note_Details : Inherits BaseRisk
		...
		Protected Sub onValidate_SCNOTE_DETAILS__NOTE_DETAILS()
                If eLifecycleEvent = LifecycleEvent.Page_Load Then
                        If SCNOTE_DETAILS__Date_Created.Text.ToString = "" Then
                            SCNOTE_DETAILS__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        					SCNOTE_DETAILS__Created_By.Text = Session(CNLoginName).ToString
                        End If
                    End If
End Sub
Protected Sub onValidate_SCNOTE_DETAILS__NoteCheck()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_SCNOTE_DETAILS__NOTE_DETAILS()
    onValidate_SCNOTE_DETAILS__NoteCheck()
End Sub
        ...
        Protected Sub NotesAdded(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnNext.Click, btnNextTop.Click, btnFinish.Click, btnFinishTop.Click
        	...
        	If NoteAddedCheck = 0 Then
        		SCNOTE_DETAILS__NoteCheck.Text = 1
        	End If
        End Sub
