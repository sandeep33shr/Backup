...
Namespace Nexus
    Partial Class PB2_PAFPPLNP_Note_Details : Inherits BaseRisk
		...
		Protected Sub onValidate_EL_ELSNOTES_DETAILS__NOTE_DETAILS()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        	If EL_ELSNOTES_DETAILS__Date_Created.Text.ToString = "" Then
        		EL_ELSNOTES_DETAILS__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        		EL_ELSNOTES_DETAILS__Created_By.Text = Session(CNLoginName).ToString
        	End If
        End If
End Sub
Protected Sub CallRuleScripts()
    onValidate_EL_ELSNOTES_DETAILS__NOTE_DETAILS()
End Sub
