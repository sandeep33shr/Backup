...
Namespace Nexus
    Partial Class PB2_MMCNOTES_Note_Details : Inherits BaseRisk
		...
		Protected Sub onValidate_MMC_DETAILS__Note_Details()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        	If MMC_DETAILS__Date_Created.Text.ToString = "" Then
        		MMC_DETAILS__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        		MMC_DETAILS__Created_By.Text = Session(CNLoginName).ToString
        	End If
        End If
End Sub
Protected Sub onValidate_MMC_DETAILS__NoteCheck()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_MMC_DETAILS__Note_Details()
    onValidate_MMC_DETAILS__NoteCheck()
End Sub
