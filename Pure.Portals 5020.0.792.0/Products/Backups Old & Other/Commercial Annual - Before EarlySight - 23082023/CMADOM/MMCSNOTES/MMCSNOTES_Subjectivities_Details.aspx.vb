...
Namespace Nexus
    Partial Class PB2_MMCSNOTES_Subjectivities_Details : Inherits BaseRisk
		...
		Protected Sub onValidate_MMC_SDETAILS__Note_Details()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        	If MMC_SDETAILS__Date_Created.Text.ToString = "" Then
        		MMC_SDETAILS__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        		MMC_SDETAILS__Created_By.Text = Session(CNLoginName).ToString
        	End If
        End If
End Sub
Protected Sub CallRuleScripts()
    onValidate_MMC_SDETAILS__Note_Details()
End Sub
