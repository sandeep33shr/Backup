...
Namespace Nexus
    Partial Class PB2_CMAMISC_Business_All_Risk : Inherits BaseRisk
		...
		Protected Sub onValidate_BAR__ATTACHMENTDATE()
        
End Sub
Protected Sub onValidate_BAR__EFFECTIVEDATE()
        
        
End Sub
Protected Sub onValidate_SEQCNT_BAR__COUNT()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_BAR__ATTACHMENTDATE()
    onValidate_BAR__EFFECTIVEDATE()
    onValidate_SEQCNT_BAR__COUNT()
End Sub
        ...
        Protected Shadows Sub Page_Load_AttachmentDate(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	...
        	If BAR__ATTACHMENTDATE.Text = "" or BAR__ATTACHMENTDATE.Text Is Nothing Then
        		If transactionValue = "NB" Then
        			BAR__ATTACHMENTDATE.Text = RTrim(LTrim(oQuote.InceptionDate))
        		Else
        			BAR__ATTACHMENTDATE.Text = RTrim(LTrim(oQuote.CoverStartDate))
        		End If
        	End If
        	
        	If oQuote.Risks(Session(CNCurrentRiskKey)).RiskInceptionDate <> oQuote.InceptionDate And (BAR__ATTACHMENTDATE.Text <> "" Or BAR__ATTACHMENTDATE.Text IsNot Nothing) Then
        		If transactionValue <> "NB" Then
        ...
        Protected Shadows Sub Page_Load_EffectiveDate(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	...
        	If transactionValue = "NB" Then
        		BAR__EFFECTIVEDATE.Text = RTrim(LTrim(oQuote.InceptionDate))
        	End If
        	
        	If transactionValue = "MTA" Then
        		BAR__EFFECTIVEDATE.Text = RTrim(LTrim(oQuote.CoverStartDate))
        	End If
        	
        	If transactionValue = "REN" Then
        		BAR__EFFECTIVEDATE.Text = RTrim(LTrim(oQuote.InceptionTPI))
        	End If
        ...
        End Sub
        		...
        		Private Sub NumberLocacationItems()
                    ...
                    Try
                        'Number all BARs  sequentially for New Business transaction type 
                        If sTransactionType = "NB"  Then
                            If objDataSet.Risk.Item("BAR").Count("BALLRISK") > 0 Then
                                For icount As Integer = 1 To objDataSet.Risk.Item("BAR").Count("BALLRISK")
                                    objDataSet.Risk.Item("BAR").Item("BALLRISK", icount).Item("INCR").Value = icount
                                    objDataSet.Risk.Item("BAR").Item("BALLRISK", icount).Item("LASTITEMNUM").Value = objDataSet.Risk.Item("BAR").Count("BALLRISK")
        							objDataSet.Risk.Item("BAR").Item("BALLRISK", icount).Item("ISLASTLASTITEMSET").Value = 1
                                Next
                        ...
                        ElseIf sTransactionType = "MTA" Or sTransactionType = "REN" Or sTransactionType = "MTR" Then
                            ...
                            'Set a session variable to keep the last BAR number
                            If objDataSet.Risk.Item("BAR").Count("BALLRISK") = 0 Then
                                Session("LastItemNo") = 0
                            ElseIf objDataSet.Risk.Item("BAR").Count("BALLRISK") > 0 Then
                                Session("LastItemNo") = objDataSet.Risk.Item("BAR").Item("BALLRISK", 1).Item("LASTITEMNUM").Value
                            End If
        
                            Integer.TryParse(Session("LastItemNo"), iNextItemNo)
        
                            'Number only BARs being added in this session
                            If objDataSet.Risk.Item("BAR").Count("BALLRISK") > 0 Then
                                For icount As Integer = 1 To objDataSet.Risk.Item("BAR").Count("BALLRISK")
                                    If objDataSet.Risk.Item("BAR").Item("BALLRISK", icount).Item("ISLASTLASTITEMSET").Value <> 1 Then
                                        iCountAfterNB = iCountAfterNB + 1
                                        objDataSet.Risk.Item("BAR").Item("BALLRISK", icount).Item("INCR").Value = iNextItemNo + iCountAfterNB
                                    End If
                                Next
                                objDataSet.ReturnAsXML(sDataSet)
                                oQuote.Risks(Session(CNCurrentRiskKey)).XMLDataset = sDataSet
        			...
        			If Session("Refresh") = "1" Then
        				Dim sUrl As String = "/Products/Commercial%20Annual/CMAMISC/CMAMISC_Business_All_Risk.aspx"
