...
Namespace Nexus
    Partial Class PB2_MMMSNOTES_Subjectivities_Details : Inherits BaseRisk
		...
		Protected Sub onValidate_MMM_EXT_DETAILS__Note_Details()
                If eLifecycleEvent = LifecycleEvent.Page_Load Then
                        If EXT_DETAILS__Date_Created.Text.ToString = "" Then
                            EXT_DETAILS__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        					EXT_DETAILS__Created_By.Text = Session(CNLoginName).ToString
                        End If
                    End If
End Sub
Protected Sub CallRuleScripts()
    onValidate_MMM_EXT_DETAILS__Note_Details()
End Sub
