...
Namespace Nexus
    Partial Class PB2_FIRENDPREM_Endorsement_Premium : Inherits BaseRisk
		...
		Protected Sub onValidate_FI_CLAUSE__ENDORSE_ID()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_FI_CLAUSE__ENDORSE_ID()
End Sub

		    
        'Script to add premium per selected Endorsement
        'Parameters:
        'FIRE - Standard Wording Parent Object
        'REFERRAL_CLAUSES - Standard Wording Parent Property
        'FI_CLAUSE - Child Screen Object
        ...
        Protected Shadows Sub Page_Load_SelectedEndorsements(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	...
        	Doc.Load(xmlTR)
        	Dim xmlLocations As XmlNodeList = Doc.SelectNodes("//" & Session.Item(CNDataModelCode) & "_POLICY_BINDER/FIRE/SW.REFERRAL_CLAUSES")
        	Dim EndorsementScheme As Object = oMaster.FindControl("FI_CLAUSE__ENDORSE_DESC")
        ...
        Protected Shadows Sub btnNextFinish_SelectedEndorsements(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnNext.Click, btnFinish.Click, btnNextTop.Click, btnFinishTop.Click
        	...
        	Dim Doc As New XmlDocument
        	Doc.Load(xmlTR)
        	Dim xmlLocations As XmlNodeList = Doc.SelectNodes("//" & Session.Item(CNDataModelCode) & "_POLICY_BINDER/FIRE/FI_CLAUSE")
        
        	'Dim CurrentSelected As String = CLAUSEPREM__ENDORSE_CAP.Text
        	
        	Dim CurrentSelected As String = FI_CLAUSE__ENDORSE_CAP.Text 'Changed so it can just use property passed as Parameters to make it more dynamic @Badimu
