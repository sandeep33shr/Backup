...
Namespace Nexus
    Partial Class PB2_ARENDPREM_Endorsement_Premium : Inherits BaseRisk
		...
		Protected Sub onValidate_AR_CLAUSE__ENDORSE_ID()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_AR_CLAUSE__ENDORSE_ID()
End Sub

		    
        'Script to add premium per selected Endorsement
        'Parameters:
        'AR - Standard Wording Parent Object
        'REFERRAL_CLAUSES - Standard Wording Parent Property
        'AR_CLAUSE - Child Screen Object
        ...
        Protected Shadows Sub Page_Load_SelectedEndorsements(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	...
        	Doc.Load(xmlTR)
        	Dim xmlLocations As XmlNodeList = Doc.SelectNodes("//" & Session.Item(CNDataModelCode) & "_POLICY_BINDER/AR/SW.REFERRAL_CLAUSES")
        	Dim EndorsementScheme As Object = oMaster.FindControl("AR_CLAUSE__ENDORSE_DESC")
        ...
        Protected Shadows Sub btnNextFinish_SelectedEndorsements(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnNext.Click, btnFinish.Click, btnNextTop.Click, btnFinishTop.Click
        	...
        	Dim Doc As New XmlDocument
        	Doc.Load(xmlTR)
        	Dim xmlLocations As XmlNodeList = Doc.SelectNodes("//" & Session.Item(CNDataModelCode) & "_POLICY_BINDER/AR/AR_CLAUSE")
        
        	'Dim CurrentSelected As String = CLAUSEPREM__ENDORSE_CAP.Text
        	
        	Dim CurrentSelected As String = AR_CLAUSE__ENDORSE_CAP.Text 'Changed so it can just use property passed as Parameters to make it more dynamic @Badimu
