...
Namespace Nexus
    Partial Class PB2_BCGEYSRCHD_Geyser_Extension : Inherits BaseRisk
		...
		Protected Sub onValidate_BC_GEYSER_ITEMS__COUNTER_ID()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_BC_GEYSER_ITEMS__COUNTER_ID()
End Sub

		    
        'Script to auto count Item in a child screen 
        'Parameters:
        'BUILDCOM - Parent Object
        'BC_GEYSER_ITEMS - Child Object
        ...
        Protected Shadows Sub Page_Load1(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	...
        	If (transactionValue <> "VIEW") Then
        		...
        		For index = 0 To oQuote.Risks.Count - 1
        			...
        			Dim sChildId As String = GetAutoNumber("//BUILDCOM/BC_GEYSER_ITEMS", sXMLDataset)
        			If sChildId <> "0" Then
        				BC_GEYSER_ITEMS__COUNTER_ID.Text = "I" & sChildId
        			End If
        		Next
        					...
        					If oLastNode.Attributes("COUNTER_ID") Is Nothing Then
        						...
        						If oNode IsNot Nothing Then
        							If oNode.Attributes("COUNTER_ID") IsNot Nothing Then
        								Dim nIdValue = CInt(oNode.Attributes("COUNTER_ID").Value.Replace("I", "")) + 1
        								dStrValue = nIdValue
        							End If
        						End If
        					End If
