...
Namespace Nexus
    Partial Class PB2_BPD_SNOTE_Note_Details : Inherits BaseRisk
		...
		Protected Sub onValidate_PL_SCNOTE_DETAILS__Note_Details()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        	If PL_SCNOTE_DETAILS__Date_Created.Text.ToString = "" Then
        		PL_SCNOTE_DETAILS__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        		PL_SCNOTE_DETAILS__Created_By.Text = Session(CNLoginName).ToString
        	End If
        End If
End Sub
Protected Sub CallRuleScripts()
    onValidate_PL_SCNOTE_DETAILS__Note_Details()
End Sub
