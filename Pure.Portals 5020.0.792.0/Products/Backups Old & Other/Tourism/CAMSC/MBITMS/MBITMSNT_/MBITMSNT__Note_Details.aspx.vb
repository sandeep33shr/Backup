...
Namespace Nexus
    Partial Class PB2_MBITMSNT__Note_Details : Inherits BaseRisk
		...
		Protected Sub onValidate_MBITMS_SCNOTE__NOTE_DETAILS()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        	If MBITMS_SCNOTE__Date_Created.Text.ToString = "" Then
        		MBITMS_SCNOTE__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        		MBITMS_SCNOTE__Created_By.Text = Session(CNLoginName).ToString
        	End If
        End If
End Sub
Protected Sub CallRuleScripts()
    onValidate_MBITMS_SCNOTE__NOTE_DETAILS()
End Sub
