...
Namespace Nexus
    Partial Class PB2_MBITMNT__Note_Details : Inherits BaseRisk
		...
		Protected Sub onValidate_MBITMS_CNOTE__NOTE_DETAILS()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        	If MBITMS_CNOTE__Date_Created.Text.ToString = "" Then
        		MBITMS_CNOTE__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        		MBITMS_CNOTE__Created_By.Text = Session(CNLoginName).ToString
        	End If
        End If
End Sub
Protected Sub onValidate_MBITMS_CNOTE__NoteCheck()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_MBITMS_CNOTE__NOTE_DETAILS()
    onValidate_MBITMS_CNOTE__NoteCheck()
End Sub
        ...
        Protected Sub NotesAdded(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnNext.Click, btnNextTop.Click, btnFinish.Click, btnFinishTop.Click
        	...
        	If NoteAddedCheck = 0 Then
        		MBITMS_CNOTE__NoteCheck.Text = 1
        	End If
        End Sub
