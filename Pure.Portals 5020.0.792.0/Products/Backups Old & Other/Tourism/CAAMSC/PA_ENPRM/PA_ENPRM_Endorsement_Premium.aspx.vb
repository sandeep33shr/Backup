...
Namespace Nexus
    Partial Class PB2_PA_ENPRM_Endorsement_Premium : Inherits BaseRisk
		...
		Protected Sub onValidate_GPA_ENDOSE__COUNTER_ID()
        
End Sub
Protected Sub onValidate_GPA_ENDOSE__ENDORSE_ID()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_GPA_ENDOSE__COUNTER_ID()
    onValidate_GPA_ENDOSE__ENDORSE_ID()
End Sub

		    
        'EMPDE - Parent object
        'GPA_ENDOSE - Child object
        		 ...
        		 Dim oQuote As NexusProvider.Quote = Session(CNQuote)
                    ...
                    If (transactionValue <> "VIEW") Then
                        ...
                        For index = 0 To oQuote.Risks.Count - 1
                            ...
                            Dim sChildId As String = GetAutoNumber("//" & "EMPDE" & "/" & "GPA_ENDOSE", sXMLDataset)
                            If sChildId <> "0" Then
                                GPA_ENDOSE__COUNTER_ID.Text = "I" & sChildId
                            End If
                        Next
        ...
        'Parameters:
        'EMPDE - Standard Wording Parent Object
        'EMPDE_CLAUSES - Standard Wording Parent Property
        'GPA_ENDOSE - Child Screen Object
        ...
        Protected Shadows Sub Page_Load_SelectedEndorsements(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	...
        	Dim Doc As New XmlDocument
        	Doc.Load(xmlTR)
        	Dim xmlLocations As XmlNodeList = Doc.SelectNodes("//" & Session.Item(CNDataModelCode) & "_POLICY_BINDER/EMPDE/SW.EMPDE_CLAUSES")
        	Dim EndorsementScheme As Object = oMaster.FindControl("GPA_ENDOSE__ENDORSE_DESC")
        ...
        Protected Shadows Sub btnNextFinish_SelectedEndorsements(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnNext.Click, btnFinish.Click, btnNextTop.Click, btnFinishTop.Click
        	...
        	Dim Doc As New XmlDocument
        	Doc.Load(xmlTR)
        	Dim xmlLocations As XmlNodeList = Doc.SelectNodes("//" & Session.Item(CNDataModelCode) & "_POLICY_BINDER/EMPDE/GPA_ENDOSE")
        
        	Dim CurrentSelected As String = GPA_ENDOSE__ENDORSE_CAP.Text
