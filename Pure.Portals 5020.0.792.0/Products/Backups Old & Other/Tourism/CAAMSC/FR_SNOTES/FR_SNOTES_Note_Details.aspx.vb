...
Namespace Nexus
    Partial Class PB2_FR_SNOTES_Note_Details : Inherits BaseRisk
		...
		Protected Sub onValidate_FIDCNOTE__NOTE_DETAILS()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        	If FIDCNOTE__Date_Created.Text.ToString = "" Then
        		FIDCNOTE__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        		FIDCNOTE__Created_By.Text = Session(CNLoginName).ToString
        	End If
        End If
End Sub
Protected Sub onValidate_FIDCNOTE__NoteCheck()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_FIDCNOTE__NOTE_DETAILS()
    onValidate_FIDCNOTE__NoteCheck()
End Sub
