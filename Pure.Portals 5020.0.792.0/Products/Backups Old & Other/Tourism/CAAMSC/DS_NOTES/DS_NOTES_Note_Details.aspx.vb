...
Namespace Nexus
    Partial Class PB2_DS_NOTES_Note_Details : Inherits BaseRisk
		...
		Protected Sub onValidate_DS_NOTES__NOTE_DETAILS()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        	If DS_NOTES__Date_Created.Text.ToString = "" Then
        		DS_NOTES__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        		DS_NOTES__Created_By.Text = Session(CNLoginName).ToString
        	End If
        End If
End Sub
Protected Sub onValidate_DS_NOTES__NoteCheck()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_DS_NOTES__NOTE_DETAILS()
    onValidate_DS_NOTES__NoteCheck()
End Sub
