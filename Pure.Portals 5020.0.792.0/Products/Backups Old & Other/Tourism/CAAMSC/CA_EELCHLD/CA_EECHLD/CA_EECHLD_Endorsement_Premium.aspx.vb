...
Namespace Nexus
    Partial Class PB2_CA_EECHLD_Endorsement_Premium : Inherits BaseRisk
		...
		Protected Sub onValidate_EE_CLAUSEPREM__COUNTER_ID()
        
End Sub
Protected Sub onValidate_EE_CLAUSEPREM__ENDORSE_ID()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_EE_CLAUSEPREM__COUNTER_ID()
    onValidate_EE_CLAUSEPREM__ENDORSE_ID()
End Sub

		    
        'ELECTEQUIP - Parent object
        'EE_CLAUSEPREM - Child object
        		 ...
        		 Dim oQuote As NexusProvider.Quote = Session(CNQuote)
                    ...
                    If (transactionValue <> "VIEW") Then
                        ...
                        For index = 0 To oQuote.Risks.Count - 1
                            ...
                            Dim sChildId As String = GetAutoNumber("//" & "ELECTEQUIP" & "/" & "EE_CLAUSEPREM", sXMLDataset)
                            If sChildId <> "0" Then
                                EE_CLAUSEPREM__COUNTER_ID.Text = "I" & sChildId
                            End If
                        Next
        ...
        'Parameters:
        'ELECTEQ - Standard Wording Parent Object
        'ELECTEQ_CLAUSES - Standard Wording Parent Property
        'EE_CLAUSEPREM - Child Screen Object
        ...
        Protected Shadows Sub Page_Load_SelectedEndorsements(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	...
        	Dim Doc As New XmlDocument
        	Doc.Load(xmlTR)
        	Dim xmlLocations As XmlNodeList = Doc.SelectNodes("//" & Session.Item(CNDataModelCode) & "_POLICY_BINDER/ELECTEQUIP/ELECTEQ/SW.ELECTEQ_CLAUSES")
        	Dim EndorsementScheme As Object = oMaster.FindControl("EE_CLAUSEPREM__ENDORSE_DESC")
        ...
        Protected Shadows Sub btnNextFinish_SelectedEndorsements(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnNext.Click, btnFinish.Click, btnNextTop.Click, btnFinishTop.Click
        	...
        	Dim Doc As New XmlDocument
        	Doc.Load(xmlTR)
        	Dim xmlLocations As XmlNodeList = Doc.SelectNodes("//" & Session.Item(CNDataModelCode) & "_POLICY_BINDER/ELECTEQUIP/ELECTEQ/EE_CLAUSEPREM")
        
        	'Dim CurrentSelected As String = CLAUSEPREM__ENDORSE_CAP.Text
        	
        	Dim CurrentSelected As String = EE_CLAUSEPREM__ENDORSE_CAP.Text 'Changed so it can just use property passed as Parameters to make it more dynamic @Badimu
