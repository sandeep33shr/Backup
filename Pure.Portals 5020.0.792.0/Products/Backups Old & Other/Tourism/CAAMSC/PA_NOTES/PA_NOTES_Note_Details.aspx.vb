...
Namespace Nexus
    Partial Class PB2_PA_NOTES_Note_Details : Inherits BaseRisk
		...
		Protected Sub onValidate_PASNOTES__NOTE_DETAILS()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        	If PASNOTES__Date_Created.Text.ToString = "" Then
        		PASNOTES__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        		PASNOTES__Created_By.Text = Session(CNLoginName).ToString
        	End If
        End If
End Sub
Protected Sub onValidate_PASNOTES__NoteCheck()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_PASNOTES__NOTE_DETAILS()
    onValidate_PASNOTES__NoteCheck()
End Sub
