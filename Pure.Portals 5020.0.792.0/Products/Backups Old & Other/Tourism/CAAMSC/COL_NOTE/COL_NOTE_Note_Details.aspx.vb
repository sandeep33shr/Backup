...
Namespace Nexus
    Partial Class PB2_COL_NOTE_Note_Details : Inherits BaseRisk
		...
		Protected Sub onValidate_CL_NOTES__NOTE_DETAILS()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        	If CL_NOTES__Date_Created.Text.ToString = "" Then
        		CL_NOTES__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        		CL_NOTES__Created_By.Text = Session(CNLoginName).ToString
        	End If
        End If
End Sub
Protected Sub onValidate_CL_NOTES__NoteCheck()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_CL_NOTES__NOTE_DETAILS()
    onValidate_CL_NOTES__NoteCheck()
End Sub
