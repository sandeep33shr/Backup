...
Namespace Nexus
    Partial Class PB2_PA_SNOTES_Notes_Details : Inherits BaseRisk
		...
		Protected Sub onValidate_PANOTES__NOTE_DETAILS()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        	If PANOTES__Date_Created.Text.ToString = "" Then
        		PANOTES__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        		PANOTES__Created_By.Text = Session(CNLoginName).ToString
        	End If
        End If
End Sub
Protected Sub CallRuleScripts()
    onValidate_PANOTES__NOTE_DETAILS()
End Sub
