...
Namespace Nexus
    Partial Class PB2_CA_EENDPRM_Endorsement_Premium : Inherits BaseRisk
		...
		Protected Sub onValidate_EE_CLAUSE__COUNTER_ID()
        
End Sub
Protected Sub onValidate_EE_CLAUSE__ENDORSE_ID()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_EE_CLAUSE__COUNTER_ID()
    onValidate_EE_CLAUSE__ENDORSE_ID()
End Sub

		    
        'ELECTEQUIP - Parent object
        'EE_CLAUSE - Child object
        		 ...
        		 Dim oQuote As NexusProvider.Quote = Session(CNQuote)
                    ...
                    If (transactionValue <> "VIEW") Then
                        ...
                        For index = 0 To oQuote.Risks.Count - 1
                            ...
                            Dim sChildId As String = GetAutoNumber("//" & "ELECTEQUIP" & "/" & "EE_CLAUSE", sXMLDataset)
                            If sChildId <> "0" Then
                                EE_CLAUSE__COUNTER_ID.Text = "I" & sChildId
                            End If
                        Next
        ...
        'Parameters:
        'ELECTEQUIP - Standard Wording Parent Object
        'ELECTEQUIP_CLAUSES - Standard Wording Parent Property
        'EE_CLAUSE - Child Screen Object
        ...
        Protected Shadows Sub Page_Load_SelectedEndorsements(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	...
        	Dim Doc As New XmlDocument
        	Doc.Load(xmlTR)
        	Dim xmlLocations As XmlNodeList = Doc.SelectNodes("//" & Session.Item(CNDataModelCode) & "_POLICY_BINDER/ELECTEQUIP/SW.ELECTEQUIP_CLAUSES")
        	Dim EndorsementScheme As Object = oMaster.FindControl("EE_CLAUSE__ENDORSE_DESC")
        ...
        Protected Shadows Sub btnNextFinish_SelectedEndorsements(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnNext.Click, btnFinish.Click, btnNextTop.Click, btnFinishTop.Click
        	...
        	Dim Doc As New XmlDocument
        	Doc.Load(xmlTR)
        	Dim xmlLocations As XmlNodeList = Doc.SelectNodes("//" & Session.Item(CNDataModelCode) & "_POLICY_BINDER/ELECTEQUIP/EE_CLAUSE")
        
        	Dim CurrentSelected As String = EE_CLAUSE__ENDORSE_CAP.Text
