...
Namespace Nexus
    Partial Class PB2_GT_ENPREM_Endorsement_Premium : Inherits BaseRisk
		...
		Protected Sub onValidate_GIT_CLAUSEPREM__COUNTER_ID()
        
End Sub
Protected Sub onValidate_GIT_CLAUSEPREM__ENDORSE_ID()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_GIT_CLAUSEPREM__COUNTER_ID()
    onValidate_GIT_CLAUSEPREM__ENDORSE_ID()
End Sub

		    
        'GIT - Parent object
        'GIT_CLAUSEPREM - Child object
        		 ...
        		 Dim oQuote As NexusProvider.Quote = Session(CNQuote)
                    ...
                    If (transactionValue <> "VIEW") Then
                        ...
                        For index = 0 To oQuote.Risks.Count - 1
                            ...
                            Dim sChildId As String = GetAutoNumber("//" & "GIT" & "/" & "GIT_CLAUSEPREM", sXMLDataset)
                            If sChildId <> "0" Then
                                GIT_CLAUSEPREM__COUNTER_ID.Text = "I" & sChildId
                            End If
                        Next
        ...
        'Parameters:
        'GIT - Standard Wording Parent Object
        'GIT_CLAUSES - Standard Wording Parent Property
        'GIT_CLAUSEPREM - Child Screen Object
        ...
        Protected Shadows Sub Page_Load_SelectedEndorsements(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	...
        	Dim Doc As New XmlDocument
        	Doc.Load(xmlTR)
        	Dim xmlLocations As XmlNodeList = Doc.SelectNodes("//" & Session.Item(CNDataModelCode) & "_POLICY_BINDER/GIT/SW.GIT_CLAUSES")
        	Dim EndorsementScheme As Object = oMaster.FindControl("GIT_CLAUSEPREM__ENDORSE_DESC")
        ...
        Protected Shadows Sub btnNextFinish_SelectedEndorsements(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnNext.Click, btnFinish.Click, btnNextTop.Click, btnFinishTop.Click
        	...
        	Dim Doc As New XmlDocument
        	Doc.Load(xmlTR)
        	Dim xmlLocations As XmlNodeList = Doc.SelectNodes("//" & Session.Item(CNDataModelCode) & "_POLICY_BINDER/GIT/GIT_CLAUSEPREM")
        
        	Dim CurrentSelected As String = GIT_CLAUSEPREM__ENDORSE_CAP.Text
