...
Namespace Nexus
    Partial Class PB2_CAAMG_Tourism_Motor_Genera : Inherits BaseRisk
		...
		Protected Sub onValidate_MOTGEN__ATTACHDATE()
        
End Sub
Protected Sub onValidate_MOTGEN__EFFECTIVEDATE()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_MOTGEN__ATTACHDATE()
    onValidate_MOTGEN__EFFECTIVEDATE()
End Sub
        ...
        Protected Shadows Sub Page_Load_AttachmentDate(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	...
        	If MOTGEN__ATTACHDATE.Text = "" or MOTGEN__ATTACHDATE.Text Is Nothing Then
        		If transactionValue = "NB" Then
        			MOTGEN__ATTACHDATE.Text = RTrim(LTrim(oQuote.InceptionDate))
        		Else
        			MOTGEN__ATTACHDATE.Text = RTrim(LTrim(oQuote.CoverStartDate))
        		End If
        	End If
        	
        	If oQuote.Risks(Session(CNCurrentRiskKey)).RiskInceptionDate <> oQuote.InceptionDate And (MOTGEN__ATTACHDATE.Text <> "" And MOTGEN__ATTACHDATE.Text IsNot Nothing) Then
        		If transactionValue <> "NB" Then
        			MOTGEN__ATTACHDATE.Text = RTrim(LTrim(oQuote.CoverStartDate))
        		Else
        			MOTGEN__ATTACHDATE.Text = RTrim(LTrim(oQuote.InceptionDate))
        ...
        Protected Shadows Sub Page_Load_EffectiveDate(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	...
        	If transactionValue = "NB" Then
        		MOTGEN__EFFECTIVEDATE.Text = RTrim(LTrim(oQuote.InceptionDate))
        	End If
        	
        	If transactionValue = "MTA" Then
        		MOTGEN__EFFECTIVEDATE.Text = RTrim(LTrim(oQuote.CoverStartDate))
        	End If
        	
        	If transactionValue = "REN" Then
        		MOTGEN__EFFECTIVEDATE.Text = RTrim(LTrim(oQuote.InceptionTPI))
        	End If
