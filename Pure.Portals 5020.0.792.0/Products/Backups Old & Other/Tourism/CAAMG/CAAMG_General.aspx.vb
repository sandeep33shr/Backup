...
Namespace Nexus
    Partial Class PB2_CAAMG_General : Inherits BaseRisk
		...
		Protected Sub onValidate_GENERAL__ATTACHDATE()
...
Protected Sub CallRuleScripts()
    onValidate_GENERAL__ATTACHDATE()
...
End Sub
        ...
        Protected Shadows Sub Page_Load_AttachmentDate(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	...
        	If GENERAL__ATTACHDATE.Text = "" or GENERAL__ATTACHDATE.Text Is Nothing Then
        		If transactionValue = "NB" Then
        			GENERAL__ATTACHDATE.Text = RTrim(LTrim(oQuote.InceptionDate))
        		Else
        			GENERAL__ATTACHDATE.Text = RTrim(LTrim(oQuote.CoverStartDate))
        		End If
        	End If
        	
        	If oQuote.Risks(Session(CNCurrentRiskKey)).RiskInceptionDate <> oQuote.InceptionDate And (GENERAL__ATTACHDATE.Text <> "" And GENERAL__ATTACHDATE.Text IsNot Nothing) Then
        		If transactionValue <> "NB" Then
        			GENERAL__ATTACHDATE.Text = RTrim(LTrim(oQuote.CoverStartDate))
        		Else
        			GENERAL__ATTACHDATE.Text = RTrim(LTrim(oQuote.InceptionDate))
