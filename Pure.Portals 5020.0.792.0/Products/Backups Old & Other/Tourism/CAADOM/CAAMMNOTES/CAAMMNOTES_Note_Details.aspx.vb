...
Namespace Nexus
    Partial Class PB2_CAAMMNOTES_Note_Details : Inherits BaseRisk
		...
		Protected Sub onValidate_MMM_DETAILS__Note_Details()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        	If MMM_DETAILS__Date_Created.Text.ToString = "" Then
        		MMM_DETAILS__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        		MMM_DETAILS__Created_By.Text = Session(CNLoginName).ToString
        	End If
        End If
End Sub
Protected Sub onValidate_MMM_DETAILS__NoteCheck()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_MMM_DETAILS__Note_Details()
    onValidate_MMM_DETAILS__NoteCheck()
End Sub
