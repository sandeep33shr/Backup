...
Namespace Nexus
    Partial Class PB2_NCONT_NOTE_Note_Details : Inherits BaseRisk
		...
		Protected Sub onValidate_OC_SNOTE_DETAILS__Note_Details()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        	If OC_SNOTE_DETAILS__Date_Created.Text.ToString = "" Then
        		OC_SNOTE_DETAILS__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        		OC_SNOTE_DETAILS__Created_By.Text = Session(CNLoginName).ToString
        	End If
        End If
End Sub
Protected Sub CallRuleScripts()
    onValidate_OC_SNOTE_DETAILS__Note_Details()
End Sub
