...
Namespace Nexus
    Partial Class PB2_CA_ADSNOTE_Note_Details : Inherits BaseRisk
		...
		Protected Sub onValidate_AD_SNOTE_DETAILS__Note_Details()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        	If AD_SNOTE_DETAILS__Date_Created.Text.ToString = "" Then
        		AD_SNOTE_DETAILS__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        		AD_SNOTE_DETAILS__Created_By.Text = Session(CNLoginName).ToString
        	End If
        End If
End Sub
Protected Sub CallRuleScripts()
    onValidate_AD_SNOTE_DETAILS__Note_Details()
End Sub
