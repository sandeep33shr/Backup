...
Namespace Nexus
    Partial Class PB2_CAAGRPFIRE_Business_Interruption : Inherits BaseRisk
		...
		Protected Sub onValidate_BI__RISK_ATTACH_DATE()
        
End Sub
Protected Sub onValidate_BI__EFFECTIVEDATE()
        
End Sub
Protected Sub onValidate_REINSEXP_BI__TOTAL_RI_EXP()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_BI__RISK_ATTACH_DATE()
    onValidate_BI__EFFECTIVEDATE()
    onValidate_REINSEXP_BI__TOTAL_RI_EXP()
End Sub
        ...
        Protected Shadows Sub Page_Load_AttachmentDate(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	...
        	If BI__RISK_ATTACH_DATE.Text = "" or BI__RISK_ATTACH_DATE.Text Is Nothing Then
        		If transactionValue = "NB" Then
        			BI__RISK_ATTACH_DATE.Text = RTrim(LTrim(oQuote.InceptionDate))
        		Else
        			BI__RISK_ATTACH_DATE.Text = RTrim(LTrim(oQuote.CoverStartDate))
        		End If
        	End If
        	
        	If oQuote.Risks(Session(CNCurrentRiskKey)).RiskInceptionDate <> oQuote.InceptionDate And (BI__RISK_ATTACH_DATE.Text <> "" Or BI__RISK_ATTACH_DATE.Text IsNot Nothing) Then
        		If transactionValue <> "NB" Then
        ...
        Protected Shadows Sub Page_Load_EffectiveDate(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	...
        	If transactionValue = "NB" Then
        		BI__EFFECTIVEDATE.Text = RTrim(LTrim(oQuote.InceptionDate))
        	End If
        	
        	If transactionValue = "MTA" Then
        		BI__EFFECTIVEDATE.Text = RTrim(LTrim(oQuote.CoverStartDate))
        	End If
        	
        	If transactionValue = "REN" Then
        		BI__EFFECTIVEDATE.Text = RTrim(LTrim(oQuote.InceptionTPI))
        	End If
        ...
        Protected Shadows Sub FirePage_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	...
        	If oQuote IsNot Nothing Then
        		...
        		UpdateXML("BI_RISK_PREMIUM", BI__TOTAL_PREMIUM.Text, "PREMIUM_SUMMARY")
        		UpdateXML("BI_EXT_PREM", BI_EXTENSIONS__TOTAL_PREMIUM.Text, "PREMIUM_SUMMARY")
                UpdateXML("BI_ENDORSE_PREMIUM", BI__TOTAL_ENDORSE_PREM.Text, "PREMIUM_SUMMARY")
                UpdateXML("BI_TOTAL_PREMIUM", BI__TOTAL_FINAL_PREMIUM.Text, "PREMIUM_SUMMARY")
        
                'Section to update Sum Insured on the Overview Page
                UpdateXML("BI_TOTAL_SI", REINSEXP_BI__TOTAL_SI.Text, "EXPOSURE_SUMMARY")
                UpdateXML("BI_TARGET_SI", REINSEXP_BI__TOTAL_TARGET_SI.Text, "EXPOSURE_SUMMARY")
                UpdateXML("BI_RI_EXP", REINSEXP_BI__TOTAL_RI_EXP.Text, "EXPOSURE_SUMMARY")
        
        
        		If Not String.IsNullOrEmpty(REINSEXP_BI__TOTAL_RI_EXP.Text) Then
        			Dim BI_RI As Double = CDbl(REINSEXP_BI__TOTAL_RI_EXP.Text)
        			Dim BI_RI_VAT As Double = CDbl(BI_RI / ((15 + 100) / 100))
        			UpdateXML("BI_RI_EXPVAT", BI_RI_VAT, "EXPOSURE_SUMMARY")
        		End If
