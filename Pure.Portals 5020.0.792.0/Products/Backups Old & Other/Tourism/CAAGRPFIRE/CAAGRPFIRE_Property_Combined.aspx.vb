...
Namespace Nexus
    Partial Class PB2_CAAGRPFIRE_Property_Combined : Inherits BaseRisk
		...
		Protected Sub onValidate_FIRE__RISK_ATTACH_DATE()
        
End Sub
Protected Sub onValidate_FIRE__EFFECTIVEDATE()
        
End Sub
Protected Sub onValidate_REINSEXP_FIRE__TOTAL_RI_EXP()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_FIRE__RISK_ATTACH_DATE()
    onValidate_FIRE__EFFECTIVEDATE()
    onValidate_REINSEXP_FIRE__TOTAL_RI_EXP()
End Sub
        ...
        Protected Shadows Sub Page_Load_AttachmentDate(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	...
        	If FIRE__RISK_ATTACH_DATE.Text = "" or FIRE__RISK_ATTACH_DATE.Text Is Nothing Then
        		If transactionValue = "NB" Then
        			FIRE__RISK_ATTACH_DATE.Text = RTrim(LTrim(oQuote.InceptionDate))
        		Else
        			FIRE__RISK_ATTACH_DATE.Text = RTrim(LTrim(oQuote.CoverStartDate))
        		End If
        	End If
        	
        	If oQuote.Risks(Session(CNCurrentRiskKey)).RiskInceptionDate <> oQuote.InceptionDate And (FIRE__RISK_ATTACH_DATE.Text <> "" Or FIRE__RISK_ATTACH_DATE.Text IsNot Nothing) Then
        		If transactionValue <> "NB" Then
        ...
        Protected Shadows Sub Page_Load_EffectiveDate(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	...
        	If transactionValue = "NB" Then
        		FIRE__EFFECTIVEDATE.Text = RTrim(LTrim(oQuote.InceptionDate))
        	End If
        	
        	If transactionValue = "MTA" Then
        		FIRE__EFFECTIVEDATE.Text = RTrim(LTrim(oQuote.CoverStartDate))
        	End If
        	
        	If transactionValue = "REN" Then
        		FIRE__EFFECTIVEDATE.Text = RTrim(LTrim(oQuote.InceptionTPI))
        	End If
        ...
        Protected Shadows Sub FirePage_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	...
        	If oQuote IsNot Nothing Then
        		...
        		UpdateXML("FIRE_RISK_PREMIUM", FIRE__TOTAL_PREMIUM.Text, "PREMIUM_SUMMARY")
        		UpdateXML("FIRE_EXT_PREMIUM", FIRE_EXTENSIONS__TOTAL_PREMIUM.Text, "PREMIUM_SUMMARY")
                UpdateXML("FIRE_ENDORSE_PREMIUM", FIRE__TOTAL_ENDORSE_PREM.Text, "PREMIUM_SUMMARY")
                UpdateXML("FIRE_TOTAL_PREMIUM", FIRE__TOTAL_FINAL_PREMIUM.Text, "PREMIUM_SUMMARY")
        
                'Section to update Sum Insured on the Overview Page
                UpdateXML("FIRE_TOTAL_SI", REINSEXP_FIRE__TOTAL_SI.Text, "EXPOSURE_SUMMARY")
                UpdateXML("FIRE_TARGET_SI", REINSEXP_FIRE__TOTAL_TARGET_SI.Text, "EXPOSURE_SUMMARY")
                UpdateXML("FIRE_RI_EXP", REINSEXP_FIRE__TOTAL_RI_EXP.Text, "EXPOSURE_SUMMARY")
        		
        		If Not String.IsNullOrEmpty(REINSEXP_FIRE__TOTAL_RI_EXP.Text) Then
        			Dim FI_RI As Double = CDbl(REINSEXP_FIRE__TOTAL_RI_EXP.Text)
        			Dim FI_RI_VAT As Double = CDbl(FI_RI / ((15 + 100) / 100))
        			UpdateXML("FIRE_RI_EXPVAT", FI_RI_VAT, "EXPOSURE_SUMMARY")
        		End If
