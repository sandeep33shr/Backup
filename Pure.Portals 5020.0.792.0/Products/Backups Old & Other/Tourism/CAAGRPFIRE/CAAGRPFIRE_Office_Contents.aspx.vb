...
Namespace Nexus
    Partial Class PB2_CAAGRPFIRE_Office_Contents : Inherits BaseRisk
		...
		Protected Sub onValidate_OFFICE__RISK_ATTACH_DATE()
        
End Sub
Protected Sub onValidate_OFFICE__EFFECTIVEDATE()
        
End Sub
Protected Sub onValidate_REINSEXP_OC__TOTAL_RI_EXP()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_OFFICE__RISK_ATTACH_DATE()
    onValidate_OFFICE__EFFECTIVEDATE()
    onValidate_REINSEXP_OC__TOTAL_RI_EXP()
End Sub
        ...
        Protected Shadows Sub Page_Load_AttachmentDate(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	...
        	If OFFICE__RISK_ATTACH_DATE.Text = "" or OFFICE__RISK_ATTACH_DATE.Text Is Nothing Then
        		If transactionValue = "NB" Then
        			OFFICE__RISK_ATTACH_DATE.Text = RTrim(LTrim(oQuote.InceptionDate))
        		Else
        			OFFICE__RISK_ATTACH_DATE.Text = RTrim(LTrim(oQuote.CoverStartDate))
        		End If
        	End If
        	
        	If oQuote.Risks(Session(CNCurrentRiskKey)).RiskInceptionDate <> oQuote.InceptionDate And (OFFICE__RISK_ATTACH_DATE.Text <> "" And OFFICE__RISK_ATTACH_DATE.Text IsNot Nothing) Then
        		If transactionValue <> "NB" Then
        			OFFICE__RISK_ATTACH_DATE.Text = RTrim(LTrim(oQuote.CoverStartDate))
        		Else
        			OFFICE__RISK_ATTACH_DATE.Text = RTrim(LTrim(oQuote.InceptionDate))
        ...
        Protected Shadows Sub Page_Load_EffectiveDate(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	...
        	If transactionValue = "NB" Then
        		OFFICE__EFFECTIVEDATE.Text = RTrim(LTrim(oQuote.InceptionDate))
        	End If
        	
        	If transactionValue = "MTA" Then
        		OFFICE__EFFECTIVEDATE.Text = RTrim(LTrim(oQuote.CoverStartDate))
        	End If
        	
        	If transactionValue = "REN" Then
        		OFFICE__EFFECTIVEDATE.Text = RTrim(LTrim(oQuote.InceptionTPI))
        	End If
        ...
        Protected Shadows Sub FirePage_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	...
        	If oQuote IsNot Nothing Then
        		...
        		UpdateXML("OC_RISK_PREMIUM", OFFICE__TOTAL_PREMIUM.Text, "PREMIUM_SUMMARY")
        		UpdateXML("OC_EXT_PREMIUM", OFFICE_EXTENSIONS__TOTAL_PREMIUM.Text, "PREMIUM_SUMMARY")
                UpdateXML("OC_ENDORSE_PREMIUM", OFFICE__TOTAL_ENDORSE_PREM.Text, "PREMIUM_SUMMARY")
                UpdateXML("OC_TOTAL_PREMIUM", OFFICE__TOTAL_FINAL_PREMIUM.Text, "PREMIUM_SUMMARY")
        
                'Section to update Sum Insured on the Overview Page
                UpdateXML("OC_TOTAL_SI", REINSEXP_OC__TOTAL_SI.Text, "EXPOSURE_SUMMARY")
                UpdateXML("OC_TARGET_SI", REINSEXP_OC__TOTAL_TARGET_SI.Text, "EXPOSURE_SUMMARY")
                UpdateXML("OC_RI_EXP", REINSEXP_OC__TOTAL_RI_EXP.Text, "EXPOSURE_SUMMARY")
        		
        		If Not String.IsNullOrEmpty(REINSEXP_OC__TOTAL_RI_EXP.Text) Then
        			Dim OC_RI As Double = CDbl(REINSEXP_OC__TOTAL_RI_EXP.Text)
        			Dim OC_RI_VAT As Double = CDbl(OC_RI / ((15 + 100) / 100))
        			UpdateXML("OC_RI_EXPVAT", OC_RI_VAT, "EXPOSURE_SUMMARY")
        		End If
