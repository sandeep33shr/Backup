...
Namespace Nexus
    Partial Class PB2_CONWORKINS_Nasria : Inherits BaseRisk
		...
		Protected Sub onValidate_NASRIA__POLICYNUM()
         
End Sub
Protected Sub CallRuleScripts()
    onValidate_NASRIA__POLICYNUM()
End Sub
         ...
         Protected Shadows Sub GetProdCode(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	...
        	If Session(CNQuote) IsNot Nothing Then
                NASRIA__POLICYNUM.Text = Trim(oQuote.InsuranceFileRef).ToString
        		NASRIA__POLICYTYPE.Text = Trim(oQuote.FrequencyCode).ToString
                'NASRIA__PRODNAME.Text = Trim(oQuote.ProductName).ToString
                NASRIA__AGENT.Text = Trim(oQuote.AgentDesc).ToString
                NASRIA__COVERSTART.Text = Trim(oQuote.CoverStartDate).ToString
                NASRIA__COVEREND.Text = Trim(oQuote.CoverEndDate).ToString
                NASRIA__RENEWAL.Text = Trim(oQuote.RenewalDate).ToString
        	End If
        End Sub
