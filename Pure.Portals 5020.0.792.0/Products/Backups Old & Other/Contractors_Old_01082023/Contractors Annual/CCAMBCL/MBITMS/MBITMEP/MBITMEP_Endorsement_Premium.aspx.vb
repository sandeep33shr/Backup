...
Namespace Nexus
    Partial Class PB2_MBITMEP_Endorsement_Premium : Inherits BaseRisk
		...
		Protected Sub onValidate_MBITEM_CLAUSEPREM__COUNTER_ID()
        
End Sub
Protected Sub onValidate_MBITEM_CLAUSEPREM__ENDORSE_ID()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_MBITEM_CLAUSEPREM__COUNTER_ID()
    onValidate_MBITEM_CLAUSEPREM__ENDORSE_ID()
End Sub

		    
        'MB_ITEM - Parent object
        'MBITEM_CLAUSEPREM - Child object
        		 ...
        		 Dim oQuote As NexusProvider.Quote = Session(CNQuote)
                    ...
                    If (transactionValue <> "VIEW") Then
                        ...
                        For index = 0 To oQuote.Risks.Count - 1
                            ...
                            Dim sChildId As String = GetAutoNumber("//" & "MB_ITEM" & "/" & "MBITEM_CLAUSEPREM", sXMLDataset)
                            If sChildId <> "0" Then
                                MBITEM_CLAUSEPREM__COUNTER_ID.Text = "I" & sChildId
                            End If
                        Next
        ...
        'REFFERRAL_CLAUSES - Standard Wording Parent Property
        'MBITEM_CLAUSEPREM - Child Screen Object
        ...
        Protected Shadows Sub Page_Load_SelectedEndorsements(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	...
        	Dim Doc As New XmlDocument
        	Doc.Load(xmlTR)
        	Dim xmlLocations As XmlNodeList = Doc.SelectNodes("//" & Session.Item(CNDataModelCode) & "_POLICY_BINDER/MBREAK/MB_ITEM/SW.REFFERRAL_CLAUSES")
        	Dim EndorsementScheme As Object = oMaster.FindControl("MBITEM_CLAUSEPREM__ENDORSE_DESC")
        ...
        Protected Shadows Sub btnNextFinish_SelectedEndorsements(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnNext.Click, btnFinish.Click, btnNextTop.Click, btnFinishTop.Click
        	...
        	Dim Doc As New XmlDocument
        	Doc.Load(xmlTR)
        	Dim xmlLocations As XmlNodeList = Doc.SelectNodes("//" & Session.Item(CNDataModelCode) & "_POLICY_BINDER/MBREAK/MB_ITEM/MBITEM_CLAUSEPREM")
        
        	'Dim CurrentSelected As String = CLAUSEPREM__ENDORSE_CAP.Text
        	
        	Dim CurrentSelected As String = MBITEM_CLAUSEPREM__ENDORSE_CAP.Text 'Changed so it can just use property passed as Parameters to make it more dynamic @Badimu
