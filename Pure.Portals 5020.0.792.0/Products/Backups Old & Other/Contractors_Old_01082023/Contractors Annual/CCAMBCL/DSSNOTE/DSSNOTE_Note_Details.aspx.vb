...
Namespace Nexus
    Partial Class PB2_DSSNOTE_Note_Details : Inherits BaseRisk
		...
		Protected Sub onValidate_DS_SNOTES__NOTE_DETAILS()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        	If DS_SNOTES__Date_Created.Text.ToString = "" Then
        		DS_SNOTES__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        		DS_SNOTES__Created_By.Text = Session(CNLoginName).ToString
        	End If
        End If
End Sub
Protected Sub onValidate_DS_SNOTES__NoteCheck()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_DS_SNOTES__NOTE_DETAILS()
    onValidate_DS_SNOTES__NoteCheck()
End Sub
