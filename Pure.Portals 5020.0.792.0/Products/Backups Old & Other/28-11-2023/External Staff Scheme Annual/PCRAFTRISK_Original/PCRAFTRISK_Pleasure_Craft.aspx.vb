...
Namespace Nexus
    Partial Class PB2_PCRAFTRISK_Pleasure_Craft : Inherits BaseRisk
		...
		Protected Sub onValidate_PCRAFTRISK__TYPEBOAT()
        
End Sub
Protected Sub onValidate_PCRAFTRISK__VATNUMBER()
        
End Sub
Protected Sub onValidate_PCRAFTRISK__REASON()
        
End Sub
Protected Sub onValidate_PCRAFTRISK__PREMIUM()
        
        
End Sub
Protected Sub onValidate_Button_btnPCraftCalculate()
        
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_PCRAFTRISK__TYPEBOAT()
    onValidate_PCRAFTRISK__VATNUMBER()
    onValidate_PCRAFTRISK__REASON()
    onValidate_PCRAFTRISK__PREMIUM()
    onValidate_Button_btnPCraftCalculate()
End Sub

		    
        
        Protected Shadows Sub Page_Load_For_Defaults_PCRAFTRISK_TYPEBOAT(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Page.IsPostBack Then
            ...
            If Not parameter Is Nothing Then
                ...
                If result.Length > 0 Then
                    ...
                    If command.ToUpper() = "POPULATEDEFAULTS_PCRAFTRISK_TYPEBOAT".ToUpper() Then
                        If result.Length >= 1 Then
                            ...
                            If onChangeType <> "" Then
                                Retrieve_List_Defaults_For_PCRAFTRISK_TYPEBOAT(onChangeType, vCode)
                            End If
                        End If
                    End If
        			
        			If command.ToUpper() = "POPULATECHECKBOXDEFAULTS_PCRAFTRISK_TYPEBOAT".ToUpper() Then
                        If result.Length >= 1 Then
                            Dim onChangeType As String = If(Not result(1) Is Nothing, result(1).ToString(), "")
                            If onChangeType <> "" Then
                                Retrieve_Checkbox_Defaults_For_PCRAFTRISK_TYPEBOAT(onChangeType)
                            End If
                        End If
                    End If
                End If
        ...
        Protected Sub Retrieve_List_Defaults_For_PCRAFTRISK_TYPEBOAT(ByVal onChangeType As String, ByVal sCode As String)
        
        Dim buildScript As new System.Text.StringBuilder()
        
        Dim ListCode = "UDL_KBOATTYP"
        ...
        If XmlList IsNot Nothing Then
                ...
                If NodeList IsNot Nothing And NodeList.Count > 0 Then
                    For Each oNode As XmlNode In NodeList
                                    If onChangeType = "TYPEBOAT" Then
                                        If oNode.SelectSingleNode("code").InnerXml.ToString().Trim() = sCode Then
                                            Dim ObjectsPropertiesArray As String() = {"PCRAFTRISK.MAXSPEED,Max_Speed", "PCRAFTRISK.MAXHORSEPOWER,Max_HP", "PCRAFTRISK.MAX_NO_MOTOR,Max_No_Motors", "PCRAFTRISK.LENGHTHULL,Length_Hull", "PCRAFTRISK.IS_HULLVALUE_REQUIRED,HValue_Required", "PCRAFTRISK.IS_MOTORVALUE_REQUIRED,MValue_Required", "PCRAFTRISK.IS_SAILVALUE_REQUIRED,SValue_Required", "PCRAFTRISK.XSPERC,XSPERC", "PCRAFTRISK.XSAMT,XSAMT", "PCRAFTRISK.MIN_SI,Min_SI", "PCRAFTRISK.MAX_SI,Max_SI", "{12}", "{13}", "{14}", "{15}", "{16}"}
                                            Dim x As Integer
                                    ...
                                    End If
                                Next
                    CompleteASyncPostback_For_PCRAFTRISK_TYPEBOAT(buildScript.ToString())
                End If
            End If
        ...
        Protected Sub Retrieve_Checkbox_Defaults_For_PCRAFTRISK_TYPEBOAT(ByVal onChangeType As String)
        
        Dim buildScript As new System.Text.StringBuilder()
        
        Dim ListCode = "UDL_KBOATTYP"
        Dim sCode = "PCRAFTRISK.MAXSPEED,Max_Speed"
                    ...
                    For Each oNode As XmlNode In NodeList
                                    If onChangeType = "TYPEBOAT" Then
                                        If oNode.SelectSingleNode("code").InnerXml.ToString().Trim() = sCode.Trim() Then
                                            Dim ObjectsPropertiesArray As String() = {"PCRAFTRISK.MAXHORSEPOWER,Max_HP", "PCRAFTRISK.MAX_NO_MOTOR,Max_No_Motors", "PCRAFTRISK.LENGHTHULL,Length_Hull", "PCRAFTRISK.IS_HULLVALUE_REQUIRED,HValue_Required", "PCRAFTRISK.IS_MOTORVALUE_REQUIRED,MValue_Required", "PCRAFTRISK.IS_SAILVALUE_REQUIRED,SValue_Required", "PCRAFTRISK.XSPERC,XSPERC", "PCRAFTRISK.XSAMT,XSAMT", "PCRAFTRISK.MIN_SI,Min_SI", "PCRAFTRISK.MAX_SI,Max_SI", "{12}", "{13}", "{14}", "{15}", "{16}"}
                                            Dim x As Integer
        ...
        Protected Sub CompleteASyncPostback_For_PCRAFTRISK_TYPEBOAT(ByVal buildScript As String)
        ...
        If Session("vatNumber") IsNot Nothing Then
        		PCRAFTRISK__VATNUMBER.Text = Session("vatNumber")
         End If
        ...
        Protected Shadows Sub Page_Load_DiscountSave(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	...
        	If Session("DiscountData") is Nothing Then
        		If (PCRAFTRISK__INDICATOR_PERC.Text = "") Or (PCRAFTRISK__INDICATOR_PERC.Text = Nothing) Or (PCRAFTRISK__PREMIUM.Text = "") Or (PCRAFTRISK__PREMIUM.Text = Nothing) Then
        			Session("DiscountData") = -1
        		Else
        			Session("DiscountData") = PCRAFTRISK__INDICATOR_PERC.Text
        		End If
        	End If
        	DiscountValidator()
        End Sub
        ...
        Private Sub DiscountValidator()
        	...
        	DiscountStatus = PCRAFTRISK__INDICATORCDE.Text
        	'Get the sanction amount
        	SanctionPercentage = CDbl(GetValue("GENERAL","DISCSANC"))
        	'Get the current amount
        	CurrentPercentage = PCRAFTRISK__INDICATOR_PERC.Text
        ...
        Private Sub PremiumValidator()
        	...
        	If (PCRAFTRISK__PREMIUM.Text = "") And (PCRAFTRISK__PREMIUM.Text = Nothing) Then
        ...
        Private Sub btnCalculatePremium_Click(sender As Object, e As EventArgs) Handles btnPCraftCalculate.Click
        	If Not (CType(Session.Item(CNMode), Mode) = Mode.View Or CType(Session.Item(CNMode), Mode) = Mode.Review) Then
        		DiscountValidator()
         		CalculatePremiums()
            End If
