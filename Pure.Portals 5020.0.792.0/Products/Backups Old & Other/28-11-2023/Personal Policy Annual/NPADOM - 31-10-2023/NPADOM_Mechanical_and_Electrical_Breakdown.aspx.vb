...
End Sub
Protected Sub onValidate_MECHELECTRIC__REASON()
...
Protected Sub CallRuleScripts()
    ...
    onValidate_MECHELECTRIC__REASON()
...
End Sub
        ...
        Protected Shadows Sub Page_Load_DiscountSave(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	...
        	If Session("MECHELC_DISC_SANCTION") is Nothing Then
        		If (MECHELECTRIC__INDICATOR_PERC.Text = "") Or (MECHELECTRIC__INDICATOR_PERC.Text = Nothing) Or (MECHELECTRIC__PREMIUM.Text = "") Or (MECHELECTRIC__PREMIUM.Text = Nothing) Then
        			Session("MECHELC_DISC_SANCTION") = -1
        		Else
        			Session("MECHELC_DISC_SANCTION") = MECHELECTRIC__INDICATOR_PERC.Text
        		End If
        	End If
        	DiscountValidator()
        End Sub
        ...
        Protected Sub btnNextDiscount_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnNext.Click
        	If Not (CType(Session.Item(CNMode), Mode) = Mode.View Or CType(Session.Item(CNMode), Mode) = Mode.Review) Then
        		...
        		If Session("DiscountChange") = 1 Then
        			Session("MECHELC_DISC_SANCTION") = Nothing
        		End If
        	End If
        End Sub
        ...
        Private Sub DiscountValidator()
        	...
        	DiscountStatus = MECHELECTRIC__INDICATORCDE.Text
        	'Get the sanction amount
        	SanctionPercentage = CDbl(GetValue("MECHELECTRIC","MECHELC_DISC_SANCTION"))
        	'Get the current amount
        	CurrentPercentage = MECHELECTRIC__INDICATOR_PERC.Text
        	'Get the previously entered amount from the session
        	PreviousPercentage = CDbl(Session("MECHELC_DISC_SANCTION"))
