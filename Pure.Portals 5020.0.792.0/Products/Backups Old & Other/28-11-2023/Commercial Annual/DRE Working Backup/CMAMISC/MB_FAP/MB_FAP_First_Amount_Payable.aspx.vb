...
Namespace Nexus
    Partial Class PB2_MB_FAP_First_Amount_Payable : Inherits BaseRisk
		...
		Protected Sub onValidate_MB_FAP__COUNTER_ID()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_MB_FAP__COUNTER_ID()
End Sub
        		 ...
        		 Dim oQuote As NexusProvider.Quote = Session(CNQuote)
                    ...
                    If (transactionValue <> "VIEW") Then
                        ...
                        For index = 0 To oQuote.Risks.Count - 1
                            ...
                            Dim sChildId As String = GetAutoNumber("//" & "MBREAK" & "/" & "MB_FAP", sXMLDataset)
                            If sChildId <> "0" Then
                                MB_FAP__COUNTER_ID.Text = "I" & sChildId
                            End If
                        Next
