...
Namespace Nexus
    Partial Class PB2_MOTORP_Motor_Details_Cont : Inherits BaseRisk
		...
		Protected Sub onValidate_MOTOR__ADDRESS()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_MOTOR__ADDRESS()
End Sub

		    
        
        Protected Shadows Sub LoadClientAddress(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
            
            Dim oQuote As NexusProvider.Quote = Session(CNQuote)
        
            If String.IsNullOrEmpty(MOTOR__ADDRESS.Text) Then
                ...
                With oParty
                    For iCount As Integer = 0 To oParty.Addresses.Count - 1
                        If oParty.Addresses(iCount).AddressType = NexusProvider.AddressType.HomeAddress Then
                            FetchAddress(iCount, oParty.Addresses)
                            Exit For
                        ElseIf oParty.Addresses(iCount).AddressType = NexusProvider.AddressType.CorrespondenceAddress Then
                            FetchAddress(iCount, oParty.Addresses)
                        End If
                    Next
                End With
        ...
        Sub FetchAddress(icount As Integer, oAddress As NexusProvider.AddressCollection)
            Dim sAddress1 As String = oAddress(icount).Address1.ToString
            If (Not (String.IsNullOrEmpty(sAddress1))) Then
                sAddress1 = sAddress1
            End If
        
            Dim sAddress2 As String = oAddress(icount).Address2.ToString
            If (Not (String.IsNullOrEmpty(sAddress2))) Then
                sAddress2 = ", " & sAddress2
            End If
        
            Dim sAddress3 As String = oAddress(icount).Address3.ToString
            If (Not (String.IsNullOrEmpty(sAddress3))) Then
                sAddress3 = ", " & sAddress3
            End If
        	
        	Dim sAddress4 As String = oAddress(icount).Address4.ToString
            If (Not (String.IsNullOrEmpty(sAddress4))) Then
                sAddress4 = ", " & sAddress4
            End If
        	
            Dim sCountry As String = oAddress(icount).CountryDescription
            If (Not (String.IsNullOrEmpty(sCountry))) Then
                sCountry = ", " & sCountry
            End If
        
            Dim sPostCode As String = oAddress(icount).PostCode.ToString
            If (Not (String.IsNullOrEmpty(sPostCode))) Then
                sPostCode = ", " & sPostCode
            End If
        	
            MOTOR__ADDRESS.Text = sAddress1 & sAddress2 & sAddress3 & sAddress4 & sCountry & sPostCode
        End Sub
