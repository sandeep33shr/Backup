...
Namespace Nexus
    Partial Class PB2_TAMSNOTE_Note_Details : Inherits BaseRisk
		...
		Protected Sub onValidate_SNOTE__NOTE_DETAILS()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        	If SNOTE__Date_Created.Text.ToString = "" Then
        		SNOTE__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        		SNOTE__Created_By.Text = Session(CNLoginName).ToString
        	End If
        End If
End Sub
Protected Sub CallRuleScripts()
    onValidate_SNOTE__NOTE_DETAILS()
End Sub
