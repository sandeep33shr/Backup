...
Namespace Nexus
    Partial Class PB2_TAMRSK_TAM_Income_Protector_Scheme : Inherits BaseRisk
		...
		Protected Sub onValidate_TAMRSK__RISK_ATTACH_DATE()
        
End Sub
Protected Sub onValidate_TAMRSK__EFFECTIVEDATE()
...
Protected Sub CallRuleScripts()
    onValidate_TAMRSK__RISK_ATTACH_DATE()
    onValidate_TAMRSK__EFFECTIVEDATE()
...
End Sub
        ...
        Protected Shadows Sub Page_Load_AttachmentDate(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	...
        	If TAMRSK__RISK_ATTACH_DATE.Text = "" or TAMRSK__RISK_ATTACH_DATE.Text Is Nothing Then
        		If transactionValue = "NB" Then
        			TAMRSK__RISK_ATTACH_DATE.Text = RTrim(LTrim(oQuote.InceptionDate))
        		Else
        			TAMRSK__RISK_ATTACH_DATE.Text = RTrim(LTrim(oQuote.CoverStartDate))
        		End If
        	End If
        	
        	If oQuote.Risks(Session(CNCurrentRiskKey)).RiskInceptionDate <> oQuote.InceptionDate And (TAMRSK__RISK_ATTACH_DATE.Text <> "" Or TAMRSK__RISK_ATTACH_DATE.Text IsNot Nothing) Then
        		If transactionValue <> "NB" Then
        ...
        Protected Shadows Sub Page_Load_EffectiveDate(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	...
        	If transactionValue = "NB" Then
        		TAMRSK__EFFECTIVEDATE.Text = RTrim(LTrim(oQuote.InceptionDate))
        	End If
        	
        	If transactionValue = "MTA" Then
        		TAMRSK__EFFECTIVEDATE.Text = RTrim(LTrim(oQuote.CoverStartDate))
        	End If
        	
        	If transactionValue = "REN" Then
        		TAMRSK__EFFECTIVEDATE.Text = RTrim(LTrim(oQuote.InceptionTPI))
        	End If
