...
Namespace Nexus
    Partial Class PB2_TAMCNOTE_Note_Details : Inherits BaseRisk
		...
		Protected Sub onValidate_CNOTE__NOTE_DETAILS()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        	If CNOTE__Date_Created.Text.ToString = "" Then
        		CNOTE__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        		CNOTE__Created_By.Text = Session(CNLoginName).ToString
        	End If
        End If
End Sub
Protected Sub onValidate_CNOTE__NoteCheck()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_CNOTE__NOTE_DETAILS()
    onValidate_CNOTE__NoteCheck()
End Sub
