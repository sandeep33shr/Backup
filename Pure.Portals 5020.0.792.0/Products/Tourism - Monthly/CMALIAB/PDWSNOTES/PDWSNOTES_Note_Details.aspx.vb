...
Namespace Nexus
    Partial Class PB2_PDWSNOTES_Note_Details : Inherits BaseRisk
		...
		Protected Sub onValidate_PDWSNOTE_DETAILS__NOTE_DETAILS()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        	If PDWSNOTE_DETAILS__Date_Created.Text.ToString = "" Then
        		PDWSNOTE_DETAILS__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        		PDWSNOTE_DETAILS__Created_By.Text = Session(CNLoginName).ToString
        	End If
        End If
End Sub
Protected Sub CallRuleScripts()
    onValidate_PDWSNOTE_DETAILS__NOTE_DETAILS()
End Sub
