...
Namespace Nexus
    Partial Class PB2_GRPFIRE_Accounts_Receivable : Inherits BaseRisk
		...
		Protected Sub onValidate_AR__RISK_ATTACH_DATE()
        
End Sub
Protected Sub onValidate_AR__EFFECTIVEDATE()
        
End Sub
Protected Sub onValidate_REINSEXP_AR__TOTAL_RI_EXP()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_AR__RISK_ATTACH_DATE()
    onValidate_AR__EFFECTIVEDATE()
    onValidate_REINSEXP_AR__TOTAL_RI_EXP()
End Sub
        ...
        Protected Shadows Sub Page_Load_AttachmentDate(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	...
        	If AR__RISK_ATTACH_DATE.Text = "" or AR__RISK_ATTACH_DATE.Text Is Nothing Then
        		If transactionValue = "NB" Then
        			AR__RISK_ATTACH_DATE.Text = RTrim(LTrim(oQuote.InceptionDate))
        		Else
        			AR__RISK_ATTACH_DATE.Text = RTrim(LTrim(oQuote.CoverStartDate))
        		End If
        	End If
        	
        	If oQuote.Risks(Session(CNCurrentRiskKey)).RiskInceptionDate <> oQuote.InceptionDate And (AR__RISK_ATTACH_DATE.Text <> "" Or AR__RISK_ATTACH_DATE.Text IsNot Nothing) Then
        		If transactionValue <> "NB" Then
        ...
        Protected Shadows Sub Page_Load_EffectiveDate(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	...
        	If transactionValue = "NB" Then
        		AR__EFFECTIVEDATE.Text = RTrim(LTrim(oQuote.InceptionDate))
        	End If
        	
        	If transactionValue = "MTA" Then
        		AR__EFFECTIVEDATE.Text = RTrim(LTrim(oQuote.CoverStartDate))
        	End If
        	
        	If transactionValue = "REN" Then
        		AR__EFFECTIVEDATE.Text = RTrim(LTrim(oQuote.InceptionTPI))
        	End If
        	
        End Sub
    
        Protected Shadows Sub ARPage_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	Dim oQuote As NexusProvider.Quote = Session(CNQuote)
        	
        	If oQuote IsNot Nothing Then
                'Section to update Premium on the Overview Page
        		UpdateXML("AR_RISK_PREMIUM", AR__ODB_PREMIUM.Text, "PREMIUM_SUMMARY")
        		UpdateXML("AC_EXT_PREMIUM", AR_EXTENSIONS__TOTAL_PREMIUM.Text, "PREMIUM_SUMMARY")
                UpdateXML("AR_ENDORSE_PREMIUM", AR__TOTAL_ENDORSE_PREM.Text, "PREMIUM_SUMMARY")
                UpdateXML("AR_TOTAL_PREMIUM", AR__TOTAL_FINAL_PREMIUM.Text, "PREMIUM_SUMMARY")
        
                'Section to update Sum Insured on the Overview Page
                UpdateXML("AR_TOTAL_SI", REINSEXP_AR__TOTAL_SI.Text, "EXPOSURE_SUMMARY")
                UpdateXML("AR_TARGET_SI", REINSEXP_AR__TOTAL_TARGET_SI.Text, "EXPOSURE_SUMMARY")
                UpdateXML("AR_RI_EXP", REINSEXP_AR__TOTAL_RI_EXP.Text, "EXPOSURE_SUMMARY")
        		
        		If Not String.IsNullOrEmpty(REINSEXP_AR__TOTAL_RI_EXP.Text) Then
        			Dim AR_RI As Double = CDbl(REINSEXP_AR__TOTAL_RI_EXP.Text)
        			Dim AR_RI_VAT As Double = CDbl(AR_RI / ((15 + 100) / 100))
        			UpdateXML("AR_RI_EXPVAT", AR_RI_VAT, "EXPOSURE_SUMMARY")
        		End If
