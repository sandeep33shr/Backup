...
Namespace Nexus
    Partial Class PB2_FIDGNOTESS_Note_Details : Inherits BaseRisk
		...
		Protected Sub onValidate_FIDGNOTES__NOTE_DETAILS()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        	If FIDGNOTES__Date_Created.Text.ToString = "" Then
        		FIDGNOTES__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        		FIDGNOTES__Created_By.Text = Session(CNLoginName).ToString
        	End If
        End If
End Sub
Protected Sub CallRuleScripts()
    onValidate_FIDGNOTES__NOTE_DETAILS()
End Sub
