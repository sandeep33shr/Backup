...
Namespace Nexus
    Partial Class PB2_ARGSNOTES_Note_Details : Inherits BaseRisk
		...
		Protected Sub onValidate_ARS_DETAILS__NOTE_DETAILS()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        	If ARS_DETAILS__Date_Created.Text.ToString = "" Then
        		ARS_DETAILS__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        		ARS_DETAILS__Created_By.Text = Session(CNLoginName).ToString
        	End If
        End If
End Sub
Protected Sub CallRuleScripts()
    onValidate_ARS_DETAILS__NOTE_DETAILS()
End Sub
