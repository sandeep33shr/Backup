...
Namespace Nexus
    Partial Class PB2_BISNOTE_Note_Details : Inherits BaseRisk
		...
		Protected Sub onValidate_SECCS_DETAILS__NOTE_DETAILS()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        	If SECCS_DETAILS__Date_Created.Text.ToString = "" Then
        		SECCS_DETAILS__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        		SECCS_DETAILS__Created_By.Text = Session(CNLoginName).ToString
        	End If
        End If
End Sub
Protected Sub CallRuleScripts()
    onValidate_SECCS_DETAILS__NOTE_DETAILS()
End Sub
