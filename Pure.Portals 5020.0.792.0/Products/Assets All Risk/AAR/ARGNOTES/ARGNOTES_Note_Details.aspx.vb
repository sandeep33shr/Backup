...
Namespace Nexus
    Partial Class PB2_ARGNOTES_Note_Details : Inherits BaseRisk
		...
		Protected Sub onValidate_AR_DETAILS__NOTE_DETAILS()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        	If AR_DETAILS__Date_Created.Text.ToString = "" Then
        		AR_DETAILS__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        		AR_DETAILS__Created_By.Text = Session(CNLoginName).ToString
        	End If
        End If
End Sub
Protected Sub onValidate_AR_DETAILS__NoteCheck()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_AR_DETAILS__NOTE_DETAILS()
    onValidate_AR_DETAILS__NoteCheck()
End Sub
