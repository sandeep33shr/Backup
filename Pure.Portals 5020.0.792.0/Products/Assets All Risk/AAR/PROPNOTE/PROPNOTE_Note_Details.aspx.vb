...
Namespace Nexus
    Partial Class PB2_PROPNOTE_Note_Details : Inherits BaseRisk
		...
		Protected Sub onValidate_SECAS_DETAILS__NOTE_DETAILS()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        	If SECAS_DETAILS__Date_Created.Text.ToString = "" Then
        		SECAS_DETAILS__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        		SECAS_DETAILS__Created_By.Text = Session(CNLoginName).ToString
        	End If
        End If
End Sub
Protected Sub CallRuleScripts()
    onValidate_SECAS_DETAILS__NOTE_DETAILS()
End Sub
