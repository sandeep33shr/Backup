...
Namespace Nexus
    Partial Class PB2_MATNOTES_Note_Details : Inherits BaseRisk
		...
		Protected Sub onValidate_MAT_DETAILS__NOTE_DETAILS()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        	If MAT_DETAILS__Date_Created.Text.ToString = "" Then
        		MAT_DETAILS__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        		MAT_DETAILS__Created_By.Text = Session(CNLoginName).ToString
        	End If
        End If
End Sub
Protected Sub onValidate_MAT_DETAILS__NoteCheck()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_MAT_DETAILS__NOTE_DETAILS()
    onValidate_MAT_DETAILS__NoteCheck()
End Sub
