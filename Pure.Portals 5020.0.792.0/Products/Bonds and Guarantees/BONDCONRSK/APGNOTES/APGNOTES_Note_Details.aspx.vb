...
Namespace Nexus
    Partial Class PB2_APGNOTES_Note_Details : Inherits BaseRisk
		...
		Protected Sub onValidate_APG_DETAILS__NOTE_DETAILS()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        	If APG_DETAILS__Date_Created.Text.ToString = "" Then
        		APG_DETAILS__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        		APG_DETAILS__Created_By.Text = Session(CNLoginName).ToString
        	End If
        End If
End Sub
Protected Sub onValidate_APG_DETAILS__NoteCheck()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_APG_DETAILS__NOTE_DETAILS()
    onValidate_APG_DETAILS__NoteCheck()
End Sub
