...
Namespace Nexus
    Partial Class PB2_RETENDO_Endorsement_Premium : Inherits BaseRisk
		...
		Protected Sub onValidate_RETENTION_SEC_RET__COUNTER_ID()
        
End Sub
Protected Sub onValidate_RETENTION_SEC_RET__ENDORSE_ID()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_RETENTION_SEC_RET__COUNTER_ID()
    onValidate_RETENTION_SEC_RET__ENDORSE_ID()
End Sub

		    
        'RETENTION - Parent object
        'RETENTION_SEC_RET - Child object
        		 ...
        		 Dim oQuote As NexusProvider.Quote = Session(CNQuote)
                    ...
                    If (transactionValue <> "VIEW") Then
                        ...
                        For index = 0 To oQuote.Risks.Count - 1
                            ...
                            Dim sChildId As String = GetAutoNumber("//" & "RETENTION" & "/" & "RETENTION_SEC_RET", sXMLDataset)
                            If sChildId <> "0" Then
                                RETENTION_SEC_RET__COUNTER_ID.Text = "I" & sChildId
                            End If
                        Next
        ...
        'Parameters:
        'RETENTION - Standard Wording Parent Object
        'RETENTION_CLAUSES - Standard Wording Parent Property
        'RETENTION_SEC_RET - Child Screen Object
        ...
        Protected Shadows Sub Page_Load_SelectedEndorsements(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	...
        	Dim Doc As New XmlDocument
        	Doc.Load(xmlTR)
        	Dim xmlLocations As XmlNodeList = Doc.SelectNodes("//" & Session.Item(CNDataModelCode) & "_POLICY_BINDER/RETENTION/SW.RETENTION_CLAUSES")
        	Dim EndorsementScheme As Object = oMaster.FindControl("RETENTION_SEC_RET__ENDORSE_DESC")
        ...
        Protected Shadows Sub btnNextFinish_SelectedEndorsements(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnNext.Click, btnFinish.Click, btnNextTop.Click, btnFinishTop.Click
        	...
        	Dim Doc As New XmlDocument
        	Doc.Load(xmlTR)
        	Dim xmlLocations As XmlNodeList = Doc.SelectNodes("//" & Session.Item(CNDataModelCode) & "_POLICY_BINDER/RETENTION/RETENTION_SEC_RET")
        
        	Dim CurrentSelected As String = RETENTION_SEC_RET__ENDORSE_CAP.Text
