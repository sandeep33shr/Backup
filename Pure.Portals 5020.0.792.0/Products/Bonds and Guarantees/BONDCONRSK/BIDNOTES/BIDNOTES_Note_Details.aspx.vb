...
Namespace Nexus
    Partial Class PB2_BIDNOTES_Note_Details : Inherits BaseRisk
		...
		Protected Sub onValidate_BID_DETAILS__NOTE_DETAILS()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        	If BID_DETAILS__Date_Created.Text.ToString = "" Then
        		BID_DETAILS__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        		BID_DETAILS__Created_By.Text = Session(CNLoginName).ToString
        	End If
        End If
End Sub
Protected Sub onValidate_BID_DETAILS__NoteCheck()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_BID_DETAILS__NOTE_DETAILS()
    onValidate_BID_DETAILS__NoteCheck()
End Sub
