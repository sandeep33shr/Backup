...
Namespace Nexus
    Partial Class PB2_PEFNOTES_Note_Details : Inherits BaseRisk
		...
		Protected Sub onValidate_PEF_DETAILS__NOTE_DETAILS()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        	If PEF_DETAILS__Date_Created.Text.ToString = "" Then
        		PEF_DETAILS__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        		PEF_DETAILS__Created_By.Text = Session(CNLoginName).ToString
        	End If
        End If
End Sub
Protected Sub onValidate_PEF_DETAILS__NoteCheck()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_PEF_DETAILS__NOTE_DETAILS()
    onValidate_PEF_DETAILS__NoteCheck()
End Sub
