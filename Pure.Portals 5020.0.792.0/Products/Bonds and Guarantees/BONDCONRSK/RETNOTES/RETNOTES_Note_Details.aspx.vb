...
Namespace Nexus
    Partial Class PB2_RETNOTES_Note_Details : Inherits BaseRisk
		...
		Protected Sub onValidate_RET_DETAILS__NOTE_DETAILS()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        	If RET_DETAILS__Date_Created.Text.ToString = "" Then
        		RET_DETAILS__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        		RET_DETAILS__Created_By.Text = Session(CNLoginName).ToString
        	End If
        End If
End Sub
Protected Sub onValidate_RET_DETAILS__NoteCheck()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_RET_DETAILS__NOTE_DETAILS()
    onValidate_RET_DETAILS__NoteCheck()
End Sub
