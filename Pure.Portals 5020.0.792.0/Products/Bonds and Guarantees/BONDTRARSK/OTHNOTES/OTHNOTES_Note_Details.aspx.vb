...
Namespace Nexus
    Partial Class PB2_OTHNOTES_Note_Details : Inherits BaseRisk
		...
		Protected Sub onValidate_OTH_DETAILS__NOTE_DETAILS()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        	If OTH_DETAILS__Date_Created.Text.ToString = "" Then
        		OTH_DETAILS__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        		OTH_DETAILS__Created_By.Text = Session(CNLoginName).ToString
        	End If
        End If
End Sub
Protected Sub onValidate_OTH_DETAILS__NoteCheck()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_OTH_DETAILS__NOTE_DETAILS()
    onValidate_OTH_DETAILS__NoteCheck()
End Sub
