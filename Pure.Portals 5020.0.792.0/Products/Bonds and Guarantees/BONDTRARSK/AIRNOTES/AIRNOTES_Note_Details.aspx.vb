...
Namespace Nexus
    Partial Class PB2_AIRNOTES_Note_Details : Inherits BaseRisk
		...
		Protected Sub onValidate_AIR_DETAILS__NOTE_DETAILS()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        	If AIR_DETAILS__Date_Created.Text.ToString = "" Then
        		AIR_DETAILS__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        		AIR_DETAILS__Created_By.Text = Session(CNLoginName).ToString
        	End If
        End If
End Sub
Protected Sub onValidate_AIR_DETAILS__NoteCheck()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_AIR_DETAILS__NOTE_DETAILS()
    onValidate_AIR_DETAILS__NoteCheck()
End Sub
