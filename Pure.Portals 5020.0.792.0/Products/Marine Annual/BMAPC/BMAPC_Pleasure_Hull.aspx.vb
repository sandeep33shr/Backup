...
Namespace Nexus
    Partial Class PB2_BMAPC_Pleasure_Hull : Inherits BaseRisk
...
End Sub
Protected Sub onValidate_PC_GENERAL__PRIMARY_INDUSTRY()
...
End Sub
Protected Sub onValidate_PC_VESSELS__INCEP_DATE()
...
Protected Sub CallRuleScripts()
    ...
    onValidate_PC_GENERAL__PRIMARY_INDUSTRY()
    onValidate_PC_VESSELS__INCEP_DATE()
...
End Sub
            ...
            If oWebService.GetParty(oQuote.PartyKey).GetType.Name = "CorporateParty" Then
        		...
        		If PrimaryKey <> "" Then
        			If ListIndOne.FindItemByKey(PrimaryKey).ToString() IsNot Nothing Then
        				If PC_GENERAL__PRIMARY_INDUSTRY.Value Is Nothing Then
        					PC_GENERAL__PRIMARY_INDUSTRY.Value = PrimaryKey
        				End If
        			End If
        
        			If ListIndTwo.FindItemByKey(SecondaryKey).ToString() IsNot Nothing Then
        				If PC_GENERAL__SECOND_INDUSTRY.Value Is Nothing Then
        					PC_GENERAL__SECOND_INDUSTRY.Value = SecondaryKey
        				End If
        			End If
        
        			If ListIndThree.FindItemByKey(TertiaryKey).ToString() IsNot Nothing Then
        				If PC_GENERAL__TERTIARY_INDUSTRY.Value Is Nothing Then
        					PC_GENERAL__TERTIARY_INDUSTRY.Value = TertiaryKey
        				End If
        			End If
        
        			If ListInd.FindItemByKey(IndustryKey).ToString() IsNot Nothing Then
        				If PC_GENERAL__INDUSTRY.Value Is Nothing Then
        					PC_GENERAL__INDUSTRY.Value = IndustryKey
        				End If
        			End If
        		End If
        	End If
        ...
        Protected Shadows Sub LoadCoverStart(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	...
        	If isTrueMonthly Then
        		PC_VESSELS__INCEP_DATE.Text = RTrim(LTrim(oQuote.InceptionDate))
        	Else
        		PC_VESSELS__INCEP_DATE.Text = RTrim(LTrim(oQuote.InceptionTPI))
        	End If
        	
        	'PC_VESSELS__ENDDATE.Text = RTrim(LTrim(oQuote.CoverEndDate))
