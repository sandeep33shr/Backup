...
Namespace Nexus
    Partial Class PB2_CMAMISC_Fidelity : Inherits BaseRisk
		...
		Protected Sub onValidate_FIDGUAR__RISK_ATTACH_DATE()
        
        
End Sub
Protected Sub onValidate_FIDGUAR__EFFECTIVEDATE()
        
End Sub
Protected Sub onValidate_FIDGUAR__IS_NEWRISK()
        
End Sub
Protected Sub onValidate_FIDGUAR__NCATBASISF()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_FIDGUAR__RISK_ATTACH_DATE()
    onValidate_FIDGUAR__EFFECTIVEDATE()
    onValidate_FIDGUAR__IS_NEWRISK()
    onValidate_FIDGUAR__NCATBASISF()
End Sub
        ...
        Protected Shadows Sub Page_Load_AttachmentDate(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	...
        	If FIDGUAR__RISK_ATTACH_DATE.Text = "" or FIDGUAR__RISK_ATTACH_DATE.Text Is Nothing Then
        		If transactionValue = "NB" Then
        			FIDGUAR__RISK_ATTACH_DATE.Text = RTrim(LTrim(oQuote.InceptionDate))
        		Else
        			FIDGUAR__RISK_ATTACH_DATE.Text = RTrim(LTrim(oQuote.CoverStartDate))
        		End If
        ...
        Protected Shadows Sub Page_Load_EffectiveDate(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	...
        	If transactionValue = "NB" Then
        		FIDGUAR__EFFECTIVEDATE.Text = RTrim(LTrim(oQuote.InceptionDate))
        	End If
        	
        	If transactionValue = "MTA" Then
        		FIDGUAR__EFFECTIVEDATE.Text = RTrim(LTrim(oQuote.CoverStartDate))
        	End If
        	
        	If transactionValue = "REN" Then
        		FIDGUAR__EFFECTIVEDATE.Text = RTrim(LTrim(oQuote.InceptionTPI))
        	End If
        ...
        Protected Shadows Sub Page_Load_IsNewRisk(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	...
        	If transactionValue <> "NB" Then
        		If oQuote.Risks(Session(CNCurrentRiskKey)).RiskInceptionDate = oQuote.InceptionDate Then
        			FIDGUAR__IS_NEWRISK.Text = 0
        		End If
        	End If
        	
        End Sub
    
        Protected Shadows Sub ClearChildScreen(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnNext.Click, btnFinish.Click, btnNextTop.Click
        	If FIDGUAR__BASISCVR.Value = 1 Then
        		ClearReferrals()
        	End If
        End Sub
        
        Private Sub ClearReferrals()
        	...
        	Dim riskRefCount As Integer = 0
        
        	Try
        		riskRefCount = objDataSet.Risk.Item("FIDGUAR").Count("GUAR")
        		If riskRefCount > 0 Then
        			For riskRefCount = riskRefCount To 1 Step -1
        				objDataSet.Risk.Item("FIDGUAR").Item("GUAR", riskRefCount).DeleteObject()
        			Next
        		End If
