...
Namespace Nexus
    Partial Class PB2_NPADOM_Contents : Inherits BaseRisk
...
End Sub
        ...
        Protected Shadows Sub Page_Load_SICheck(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	...
        	If Session("InvalidSI") IsNot Nothing Then
        		CONTENTS__SUMINSURED.Text = Session("InvalidSI")
        	End If
        	Session("CONTENTSSI") = CONTENTS__SUMINSURED.Text											
        	SIValidator()
        End Sub
        ...
        Protected Sub btnBackTopSICheck_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnBackTop.Click
        	If Not (CType(Session.Item(CNMode), Mode) = Mode.View Or CType(Session.Item(CNMode), Mode) = Mode.Review) Then
        		If Session("ValidSI") = 1 Then
        			CONTENTS__SUMINSURED.Text = Session("SIData")
        		End If
        		Session("SIData") = Nothing
        	End If
        End Sub
        ...
        Protected Sub tabClickSICheck_TabClicked(ByVal v_sPath As String) Handles ctrlTabIndex.TabClicked
        	If Not (CType(Session.Item(CNMode), Mode) = Mode.View Or CType(Session.Item(CNMode), Mode) = Mode.Review) Then
        		...
        		If Session("ValidSI") = 0 And CONTENTS__PREMIUM.Text = "" And CONTENTS__SUMINSURED.Text <> Session("SIData") Then
        			CONTENTS__SUMINSURED.Text = Session("SIData")
        		End If
        		Session("SIData") = Nothing
        	End If
        ...
        Private Sub SIValidator()
        	...
        	Select Case CurrentCover
        		...
        		Case "CONTENTS"
        			...
        			ContentSI = Session("CONTENTSSI")
        			AllRiskSI = GetValue("ALLRISK", "TOTSUMINSURED")
        			MachBrkSI = GetValue("MECHELECTRIC", "SUMINSURED")
        		Case "ALLRISK"
        			BuildSI = GetValue("BUILDINGS", "SUMINSURED")
        			ContentSI = GetValue("CONTENTS", "SUMINSURED")
        			AllRiskSI = Session("CONTENTSSI")
        			MachBrkSI = GetValue("MECHELECTRIC", "SUMINSURED")
        		Case "MECHELECTRIC"
        			BuildSI = GetValue("BUILDINGS", "SUMINSURED")
        			ContentSI = GetValue("CONTENTS", "SUMINSURED")
        			AllRiskSI = GetValue("ALLRISK", "TOTSUMINSURED")
        			MachBrkSI = Session("CONTENTSSI")
        	End Select
        ...
        Protected Sub btnBackTopAlarm_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnBackTop.Click
        	If Not (CType(Session.Item(CNMode), Mode) = Mode.View Or CType(Session.Item(CNMode), Mode) = Mode.Review) Then
        		Session("AlarmData") = Nothing
        	End If
        End Sub
        
        Protected Sub btnBackAlarm_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnBack.Click
        	If Not (CType(Session.Item(CNMode), Mode) = Mode.View Or CType(Session.Item(CNMode), Mode) = Mode.Review) Then
        		Session("AlarmData") = Nothing
        	End If
        End Sub
        
        Protected Sub tabClickAlarm_TabClicked(ByVal v_sPath As String) Handles ctrlTabIndex.TabClicked
        	Session("AlarmData") = Nothing
        End Sub
        ...
        Protected Sub CalculatePremiums()
        	...
        	If Session("ValidSI") = 1 Then
        		Session("InvalidSI") = CONTENTS__SUMINSURED.Text
        		CONTENTS__SUMINSURED.Text = Session("SIData")
        	End If
