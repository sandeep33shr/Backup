...
Namespace Nexus
    Partial Class PB2_CBASRSK_Support_Risks : Inherits BaseRisk
		...
		Protected Sub onValidate_RTARSK__RISK_ATTACH_DATE()
        
End Sub
Protected Sub onValidate_RTARSK__EFFECTIVEDATE()
...
End Sub
Protected Sub onValidate_INS_DET__INS_NAME()
...
Protected Sub CallRuleScripts()
    onValidate_RTARSK__RISK_ATTACH_DATE()
    onValidate_RTARSK__EFFECTIVEDATE()
    onValidate_ADDRESS__TOWN()
    onValidate_ADDRESS__COUNTRY()
    onValidate_INS_DET__INS_NAME()
...
End Sub
        ...
        Protected Shadows Sub Page_Load_AttachmentDate(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	...
        	If RTARSK__RISK_ATTACH_DATE.Text = "" or RTARSK__RISK_ATTACH_DATE.Text Is Nothing Then
        		If transactionValue = "NB" Then
        			RTARSK__RISK_ATTACH_DATE.Text = RTrim(LTrim(oQuote.InceptionDate))
        		Else
        			RTARSK__RISK_ATTACH_DATE.Text = RTrim(LTrim(oQuote.CoverStartDate))
        		End If
        	End If
        	
        	If oQuote.Risks(Session(CNCurrentRiskKey)).RiskInceptionDate <> oQuote.InceptionDate And (RTARSK__RISK_ATTACH_DATE.Text <> "" Or RTARSK__RISK_ATTACH_DATE.Text IsNot Nothing) Then
        		If transactionValue <> "NB" Then
        ...
        Protected Shadows Sub Page_Load_EffectiveDate(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	...
        	If transactionValue = "NB" Then
        		RTARSK__EFFECTIVEDATE.Text = RTrim(LTrim(oQuote.InceptionDate))
        	End If
        	
        	If transactionValue = "MTA" Then
        		RTARSK__EFFECTIVEDATE.Text = RTrim(LTrim(oQuote.CoverStartDate))
        	End If
        	
        	If transactionValue = "REN" Then
        		RTARSK__EFFECTIVEDATE.Text = RTrim(LTrim(oQuote.InceptionTPI))
        	End If
        ...
        End Sub
         ...
         Protected Shadows Sub Page_LoadInsuredName(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        		...
        		If Session(CNParty) IsNot Nothing Then
        			If TypeOf Session(CNParty) Is NexusProvider.CorporateParty Then
        				...
        				sFullName = Trim(DirectCast(oParty, NexusProvider.CorporateParty).CompanyName).ToString				
        			...
        			ElseIf TypeOf Session(CNParty) Is NexusProvider.PersonalParty Then
        				...
        				sFullName = Trim(DirectCast(oParty, NexusProvider.PersonalParty).Forename).ToString & " " & Trim(DirectCast(oParty, NexusProvider.PersonalParty).Lastname).ToString 'fullname of the client
        				
        			End If
        		End If
        
        		INS_DET__INS_NAME.Text = sFullName
