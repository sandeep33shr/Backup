...
Namespace Nexus
    Partial Class PB2_CCAPLSNOTE_Notes_Detials : Inherits BaseRisk
		...
		Protected Sub onValidate_GPANOTES__Note_Details()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        	If GPANOTES__Date_Created.Text.ToString = "" Then
        		GPANOTES__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        		GPANOTES__Created_By.Text = Session(CNLoginName).ToString
        	End If
        End If
End Sub
Protected Sub onValidate_GPANOTES__NoteCheck()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_GPANOTES__Note_Details()
    onValidate_GPANOTES__NoteCheck()
End Sub
