...
Namespace Nexus
    Partial Class PB2_DW_SNOTE_Note_Details : Inherits BaseRisk
		...
		Protected Sub onValidate_DWSNOTE_DETAILS__NOTE_DETAILS()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        	If DWSNOTE_DETAILS__Date_Created.Text.ToString = "" Then
        		DWSNOTE_DETAILS__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        		DWSNOTE_DETAILS__Created_By.Text = Session(CNLoginName).ToString
        	End If
        End If
End Sub
Protected Sub CallRuleScripts()
    onValidate_DWSNOTE_DETAILS__NOTE_DETAILS()
End Sub
