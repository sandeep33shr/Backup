...
Namespace Nexus
    Partial Class PB2_PUBNOTES_Note_Details : Inherits BaseRisk
		...
		Protected Sub onValidate_GENTEN_CNOTE_DETAILS__NOTE_DETAILS()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        	If GENTEN_CNOTE_DETAILS__Date_Created.Text.ToString = "" Then
        		GENTEN_CNOTE_DETAILS__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        		GENTEN_CNOTE_DETAILS__Created_By.Text = Session(CNLoginName).ToString
        	End If
        End If
End Sub
Protected Sub onValidate_GENTEN_CNOTE_DETAILS__NoteCheck()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_GENTEN_CNOTE_DETAILS__NOTE_DETAILS()
    onValidate_GENTEN_CNOTE_DETAILS__NoteCheck()
End Sub
        ...
        Protected Sub NotesAdded(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnNext.Click, btnNextTop.Click, btnFinish.Click, btnFinishTop.Click
        	...
        	If NoteAddedCheck = 0 Then
        		GENTEN_CNOTE_DETAILS__NoteCheck.Text = 1
        	End If
        End Sub
