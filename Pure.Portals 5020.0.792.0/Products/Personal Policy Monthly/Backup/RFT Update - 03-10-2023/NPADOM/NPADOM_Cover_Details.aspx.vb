...
Protected Sub onValidate_GENERAL__TRANSTYPE()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
                ...
                If (Session("IsDuplicate") = 1) And (transactionValue = "NB") Then
                    GENERAL__ACCEPT_DUPLIC_CLIENT.Text = 1
                End If
...
End Sub
		Protected Shadows Sub Page_Load_AddressCheck(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.PreLoad
            If Session("AddressCheck") Is Nothing Or Session("AddressCheck") = 0 Then
                Session("AddressChanged") = 0
                Session("RemoveFinish") = 0
            End If

            If ADDRESS__ADDRESSLIST.Text = "Business Address" Then
                Session("Address") = ADDRESS__B_ADDRESSLIST.Text
            End If
            If ADDRESS__ADDRESSLIST.Text = "Home Address" Then
                Session("Address") = ADDRESS__HOMEADDRESSLIST.Text
            End If
            If ADDRESS__ADDRESSLIST.Text = "Site Address" Then
                Session("Address") = ADDRESS__SITEADDRESSLIST.Text
            End If
        End Sub


        Protected Sub AddressCheck(CurrentAddress As String)
            Session("AddressCheck") = 1
            If CurrentAddress <> Session("Address") Then
                Session("AddressChanged") = 1
            Else
                Session("AddressChanged") = 0
            End If

            Dim oQuote As NexusProvider.Quote = Session(CNQuote)
            Dim RiskKey = Session(CNCurrentRiskKey)
            Dim Status As String = oQuote.Risks(RiskKey).RiskLinkStatusFlag

            If (Session("AddressChanged") = 1) Or (Session("AddressChanged") = 0 And Status <> "C") Then
                Session("RemoveFinish") = 1
            Else
                Session("RemoveFinish") = 0
            End If
        End Sub
		...
		Protected Sub RunDefaultRule()
			...
			Dim sScreenCode As String = "NPADOM"
			...
			If sTransactionType = "NB" Then
				oQuote.Risks(Session(CNCurrentRiskKey)).XMLDataset = oWebService.RunDefaultRulesEdit(sScreenCode, oQuote.Risks(Session(CNCurrentRiskKey)).XMLDataset, Nothing, Nothing, "NB", v_bSkipSaveToDB)
			End If
		End Sub
        ...
        Protected Shadows Sub LoadCoverStart(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
            ...
            If transactionValue = "NB" Then
                If GENERAL__QUOTEDATE.Text = "" Or GENERAL__QUOTEDATE.Text Is Nothing Then
        ...
        Private Sub DuplicateClient()
            Session("IsDuplicate") = 0

            If (GENERAL__ACCEPT_DUPLIC_CLIENT.Text = True) Then
                Session("IsDuplicate") = 1
            End If
