...
Namespace Nexus
    Partial Class PB2_GPANOTE_Note_Details : Inherits BaseRisk
		...
		Protected Sub onValidate_GPASNOTES__NOTE_DETAILS()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        	If GPASNOTES__Date_Created.Text.ToString = "" Then
        		GPASNOTES__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        		GPASNOTES__Created_By.Text = Session(CNLoginName).ToString
        	End If
        End If
End Sub
Protected Sub onValidate_GPASNOTES__NoteCheck()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_GPASNOTES__NOTE_DETAILS()
    onValidate_GPASNOTES__NoteCheck()
End Sub
