...
Namespace Nexus
    Partial Class PB2_SPPC_Pleasure_Craft : Inherits BaseRisk
		...
		Protected Sub onValidate_PLADDRESS__TOWN()
        
End Sub
Protected Sub onValidate_PLADDRESS__COUNTRY()
        
End Sub
Protected Sub onValidate_IN_GENERAL__PRIMARY_INDUSTRY()
...
End Sub
Protected Sub onValidate_SPIPPC_VESSELS__INCEP_DATE()
...
Protected Sub CallRuleScripts()
    onValidate_PLADDRESS__TOWN()
    onValidate_PLADDRESS__COUNTRY()
    onValidate_IN_GENERAL__PRIMARY_INDUSTRY()
    onValidate_SPIPPC_VESSELS__INCEP_DATE()
...
End Sub
        ...
        Protected Sub ValidateAddress()
        	...
        	Dim Line1 as String = PLADDRESS__LINE1.Text
        	Dim Suburb as String = PLADDRESS__SUBURB.Text
            ...
            If oWebService.GetParty(oQuote.PartyKey).GetType.Name = "CorporateParty" Then
        		...
        		If PrimaryKey <> "" Then
        			If ListIndOne.FindItemByKey(PrimaryKey).ToString() IsNot Nothing Then
        				If IN_GENERAL__PRIMARY_INDUSTRY.Value Is Nothing Then
        					IN_GENERAL__PRIMARY_INDUSTRY.Value = PrimaryKey
        				End If
        			End If
        
        			If ListIndTwo.FindItemByKey(SecondaryKey).ToString() IsNot Nothing Then
        				If IN_GENERAL__SECOND_INDUSTRY.Value Is Nothing Then
        					IN_GENERAL__SECOND_INDUSTRY.Value = SecondaryKey
        				End If
        			End If
        
        			If ListIndThree.FindItemByKey(TertiaryKey).ToString() IsNot Nothing Then
        				If IN_GENERAL__TERTIARY_INDUSTRY.Value Is Nothing Then
        					IN_GENERAL__TERTIARY_INDUSTRY.Value = TertiaryKey
        				End If
        			End If
        
        			If ListInd.FindItemByKey(IndustryKey).ToString() IsNot Nothing Then
        				If IN_GENERAL__INDUSTRY.Value Is Nothing Then
        					IN_GENERAL__INDUSTRY.Value = IndustryKey
        				End If
        			End If
        		End If
        	End If
        ...
        Protected Shadows Sub LoadCoverStart(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	...
        	If isTrueMonthly Then
        		SPIPPC_VESSELS__INCEP_DATE.Text = RTrim(LTrim(oQuote.InceptionDate))
        	Else
        		SPIPPC_VESSELS__INCEP_DATE.Text = RTrim(LTrim(oQuote.InceptionTPI))
        	End If
        	
        	'SPIPPC_VESSELS__ENDDATE.Text = RTrim(LTrim(oQuote.CoverEndDate))
