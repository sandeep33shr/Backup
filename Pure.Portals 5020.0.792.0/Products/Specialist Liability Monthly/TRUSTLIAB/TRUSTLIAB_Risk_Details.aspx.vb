...
Namespace Nexus
    Partial Class PB2_TRUSTLIAB_Risk_Details : Inherits BaseRisk
		...
		Protected Sub onValidate_RISK_DETAILS__RETROACTIVE_DATE()
         
End Sub
Protected Sub CallRuleScripts()
    onValidate_RISK_DETAILS__RETROACTIVE_DATE()
End Sub

		    
         Protected Shadows Sub InceptionDate(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	Dim oQuote As NexusProvider.Quote = Session(CNQuote)
        	 
        	If Session(CNQuote) IsNot Nothing Then
        		Dim InceptDate = oQuote.InceptionDate
        		Dim RetroDate = RISK_DETAILS__RETROACTIVE_DATE.Text
        		If (Not IsDate(InceptDate)) Then
        			InceptDate = Date.Today
        		End If
        		
        		If RetroDate Is Nothing Or RetroDate = "" Then
        			RISK_DETAILS__RETROACTIVE_DATE.Text = InceptDate
        		End If
        	End If
        End Sub
