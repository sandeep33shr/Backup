...
Namespace Nexus
    Partial Class PB2_CMNASRIA_NASRIA_And_Stamp_Duty : Inherits BaseRisk
		...
		Protected Sub onValidate_NASRIA__RISK_ATTACH_DATE()
        
End Sub
Protected Sub onValidate_NASRIA__EFFECTIVEDATE()
        
End Sub
Protected Sub onValidate_NASRIA__IS_NASRIA()
        If (CType(Session.Item(Nexus.Constants.CNMode), Nexus.Constants.Mode) <> Nexus.Constants.Mode.View) And (CType(Session.Item(Nexus.Constants.CNMode), Nexus.Constants.Mode) <> Nexus.Constants.Mode.Review) Then
        	If eLifecycleEvent = LifecycleEvent.Page_Load Then
        		RunRatingRuleAndRefreshValues()
        	End If
        End If
...
Protected Sub CallRuleScripts()
    onValidate_NASRIA__RISK_ATTACH_DATE()
    onValidate_NASRIA__EFFECTIVEDATE()
    onValidate_NASRIA__IS_NASRIA()
End Sub
        ...
        Protected Shadows Sub Page_Load_AttachmentDate(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	...
        	If NASRIA__RISK_ATTACH_DATE.Text = "" or NASRIA__RISK_ATTACH_DATE.Text Is Nothing Then
        		If transactionValue = "NB" Then
        			NASRIA__RISK_ATTACH_DATE.Text = RTrim(LTrim(oQuote.InceptionDate))
        		Else
        			NASRIA__RISK_ATTACH_DATE.Text = RTrim(LTrim(oQuote.CoverStartDate))
        		End If
        	End If
        	
        	If oQuote.Risks(Session(CNCurrentRiskKey)).RiskInceptionDate <> oQuote.InceptionDate And (NASRIA__RISK_ATTACH_DATE.Text <> "" Or NASRIA__RISK_ATTACH_DATE.Text IsNot Nothing) Then
        		If transactionValue <> "NB" Then
        ...
        Protected Shadows Sub Page_Load_EffectiveDate(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        	...
        	If transactionValue = "NB" Then
        		NASRIA__EFFECTIVEDATE.Text = RTrim(LTrim(oQuote.InceptionDate))
        	End If
        	
        	If transactionValue = "MTA" Then
        		NASRIA__EFFECTIVEDATE.Text = RTrim(LTrim(oQuote.CoverStartDate))
        	End If
        	
        	If transactionValue = "REN" Then
        		NASRIA__EFFECTIVEDATE.Text = RTrim(LTrim(oQuote.InceptionTPI))
        	End If
        	
        End Sub
    
        Sub RunRatingRuleAndRefreshValues()
            Try
                Dim oWebService As NexusProvider.ProviderBase = New NexusProvider.ProviderManager().Provider
        		Dim oQuote As NexusProvider.Quote = Session(CNQuote)
        		Dim transactionValue As String = String.Empty
        		...
        		If Session(CNMTAType) = MTAType.PERMANENT Or Session(CNMTAType) = MTAType.TEMPORARY Then
        			...
        			ElseIf Session(CNMTAType) = MTAType.REINSTATEMENT Then
        				transactionValue = "MTR"
                    Else
                        transactionValue = "NB"
                End If
        		
        		If (CType(Session.Item(Nexus.Constants.CNMode), Nexus.Constants.Mode) <> Nexus.Constants.Mode.View) And (CType(Session.Item(Nexus.Constants.CNMode), Nexus.Constants.Mode) <> Nexus.Constants.Mode.Review) Then
        			oWebService.UpdateRisk(oQuote, CInt(Session(CNCurrentRiskKey)), Nothing, Nothing, transactionValue)
        		End If
                
                ' Stamp Duty
                ' STAMPDUTY__STAMP_DUTY_SI.Text = GetValue("STAMPDUTY", "STAMP_DUTY_SI")
                STAMPDUTY__SD_RATE.Text = GetValue("STAMPDUTY", "SD_RATE")
                STAMPDUTY__SD_AMOUNT.Text = GetValue("STAMPDUTY", "SD_AMOUNT")
                STAMPDUTY__SD_ANNUAL_PREMIUM.Text = GetValue("STAMPDUTY", "SD_ANNUAL_PREMIUM")
                ' STAMPDUTY__NAMFISA_LEVY.Text = GetValue("STAMPDUTY", "NAMFISA_LEVY")
                
        				 
        		If Session("Refresh") = "1" Then
        			Dim sUrl As String = "/Products/Commercial%20Annual/CMNASRIA/CMNASRIA_NASRIA_And_Stamp_Duty.aspx"
                ...
                ' Register startup script to reload the page
                'ClientScript.RegisterStartupScript(Me.GetType(), "RefreshScript", "window.location.reload();", True)
