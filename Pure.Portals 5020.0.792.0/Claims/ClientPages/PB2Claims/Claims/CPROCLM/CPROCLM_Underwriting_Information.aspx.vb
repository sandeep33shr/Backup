...
Namespace Nexus
    Partial Class PB2_CPROCLM_Underwriting_Information : Inherits BaseClaim
		...
		Protected Sub onValidate_CPROCLAIM__IS_FAC()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_CPROCLAIM__IS_FAC()
End Sub
        ...
        Protected Shadows Sub Page_Load_ClientInfo(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
            ...
            If Session(CNParty) IsNot Nothing Then
                If TypeOf Session(CNParty) Is NexusProvider.CorporateParty Then
                    ...
                    CPROCLAIM__INSURED.Text = DirectCast(oParty, NexusProvider.CorporateParty).CompanyName
                End If
                ...
                If TypeOf Session(CNParty) Is NexusProvider.PersonalParty Then
                    ...
                    CPROCLAIM__INSURED.Text = (firstName + " " + LastName).ToString
        			CPROCLAIM__CONTACTNUM.Text = Trim(MobileNo).ToString
                End If
            ...
            If Session(CNClaimQuote) IsNot Nothing Then
                CPROCLAIM__POLNUMBER.Text = RTrim(LTrim(oClaimQuote.InsuranceFileRef))
                CPROCLAIM__ALTERPOLICY.Text = RTrim(LTrim(oClaimQuote.AlternativeRef))
                CPROCLAIM__CVR_STARTDATE.Text = RTrim(LTrim(oClaimQuote.CoverStartDate))
                CPROCLAIM__CVR_ENDDATE.Text = RTrim(LTrim(oClaimQuote.CoverEndDate))
                CPROCLAIM__INCEPT_DATE.Text = RTrim(LTrim(oClaimQuote.InceptionDate))
        		...
        		For nIndex = 0 To Cnt
        			...
        			End If
        		Next
        		CPROCLAIM__IS_FAC.Text = FAC.ToString
            End If
