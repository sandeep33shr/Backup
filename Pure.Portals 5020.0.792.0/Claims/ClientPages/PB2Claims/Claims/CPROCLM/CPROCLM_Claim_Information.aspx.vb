...
Namespace Nexus
    Partial Class PB2_CPROCLM_Claim_Information : Inherits BaseClaim
		...
		Protected Sub onValidate_CPROCLAIM__SALVAGESTAT()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        
        	If CType(Session.Item(CNMode), Mode) = Mode.PayClaim Then
        		Response.Redirect("~/Claims/ClientPages/PB2Claims/Claims/CPROCLM/CPROCLM_Schedule_Of_Loss.aspx")
        	End If
        	
        End If
End Sub
Protected Sub onValidate_CPROCLAIM__CPROTRAIL()
        
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_CPROCLAIM__SALVAGESTAT()
    onValidate_CPROCLAIM__CPROTRAIL()
End Sub

		     
        ' Note: This rule removes the "Add" button row of a child-screen grid
        ' CPROCLAIM = The parent object name of child screen
        ' CPROAUDIT = The child screen object name
        
        Protected Sub CPROCLAIM__CPROAUDIT_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles CPROCLAIM__CPROAUDIT.RowDataBound
        ...
        Protected Sub UpdateAuditTrail()
        	...
        	Dim salvage As String = CPROCLAIM__SALVAGESTAT.Text
        	Dim legal As String = CPROCLAIM__LEGALSTAT.Text
        	Dim decision As String = CPROCLAIM__CLMDECISION.Text
        	...
        	If (salvage_updatereq Or legal_updatereq Or decision_updatereq) Then
        		...
        		Session(CNDataSet) = oWebService.RunDefaultRulesEdit(CPROCLAIM__CPROAUDIT.ScreenCode, Session(CNDataSet))
        	End If
        ...
        Protected Function AuditEntryRequired(ByVal propertyName As String, ByVal propertyValue As String, ByRef previousValue As String) As Boolean
        	...
        	For Each el As Linq.XElement In datamodelXML.DescendantNodes()
        		If (el.Name.ToString().ToUpper() = "CPROAUDIT") Then
        ...
        Protected Sub LogAuditEntry(ByVal propertyName As String, ByVal propertyValue As String, ByVal previousValue As String)
        	...
        	Doc.LoadXml(strDS)
        	Dim xmlParent As XmlNode = Doc.SelectSingleNode("//" & Session.Item(CNDataModelCode) & "_POLICY_BINDER/CPROCLAIM/CPROAUDIT")
        	Dim v_sOI As String = Nothing
        	If xmlParent IsNot Nothing Then
        		v_sOI = xmlParent.Attributes("OI").Value
        	End If
        	Dim v_sScreenCode As String = CPROCLAIM__CPROAUDIT.ScreenCode
        	Dim sParentElement, sChildElement As String
        	Dim sTmp() As String = Regex.Split(CPROCLAIM__CPROAUDIT.ID, "__")
        	...
        	CPROCLAIM__CPROAUDIT.DataSource = oXMLSource
        	CPROCLAIM__CPROAUDIT.DataBind()
