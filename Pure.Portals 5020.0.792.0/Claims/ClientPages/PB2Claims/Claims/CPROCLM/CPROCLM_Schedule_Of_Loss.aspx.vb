...
Namespace Nexus
    Partial Class PB2_CPROCLM_Schedule_Of_Loss : Inherits BaseClaim
		...
		Protected Sub onValidate_CPROCLAIM__CPOWNSCRN()
...
End Sub
Protected Sub onValidate_CPROCLAIM__TransactionType()
        ...
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
           Dim sTransactionType As String = String.Empty
                     ...
                     CPROCLAIM__TransactionType.Text = sTransactionType		 
                    ...
                    Try
                        If (Not String.IsNullOrEmpty(Session(CNDataSet))) Then
                            ...
                            Doc.Load(xmlTR)
                            xmlTR.Close()
                            Dim oNodelist As XmlNodeList = Doc.SelectNodes("//CPROCLAIM/RECOVERIES")
...
End Sub
Protected Sub onValidate_CPROCLAIM__CPISOLSCRN()
                    If eLifecycleEvent = LifecycleEvent.Page_Load Then
                        If Not Page.IsPostBack Then
                            GetClaimDetails(CType(Session.Item(CNClaim), NexusProvider.ClaimOpen).ClaimKey, Nothing)
                            SetScriptPayment()
...
Protected Sub CallRuleScripts()
    onValidate_CPROCLAIM__CPOWNSCRN()
    onValidate_CPROCLAIM__TransactionType()
    onValidate_CPROCLAIM__CPISOLSCRN()
End Sub
        ...
        Protected Sub btnNextTop_Click(sender As Object, e As EventArgs) Handles btnNextTop.Click, btnNext.Click
             ...
             End If
                    ...
                    '    UpdateClaimXML("CPROCLAIM", "PROCESS_RECEIPT_FLAG", "1", True)
                    '    UpdateClaimXML("CPROCLAIM", "RECEIPT_PAYEE_TYPE", RESERVES__Receipt_Payee_Type.Value, True)
                    '    UpdateClaimXML("CPROCLAIM", "RECEIPT_PARTY_NAME", RESERVES__Receipt_Party_Name.PartyKey, True)
                    '    RunDefaultRule()
                    '    GetClaimDetails(CType(Session.Item(CNClaim), NexusProvider.ClaimOpen).ClaimKey, Nothing)
        
                    '    'Get get flags updated in back office rules
                    '    RESERVES__Paid_Payment_Processed_Flag.Text = GetValue("CPROCLAIM", "RECEIPT_PROCESSED")
                    '    RESERVES__Process_Payment_Flag.Text = GetValue("CPROCLAIM", "PROCESS_RECEIPT_FLAG")
                    '    SetScriptReceipt()
                    'End If
        End Sub
        ...
        Private Sub ProcessPayment()
                     If (Not String.IsNullOrEmpty(Session(CNDataSet)) AndAlso CType(Session.Item(CNMode), Mode) = Mode.PayClaim) Then
                        ...
                        xmlDoc.Load(xmlTR)
                        xmlTR.Close()
                        Dim oNodelist As XmlNodeList = xmlDoc.SelectNodes("//CPROCLAIM/OWNDCLAIM")
                        For Each oNode As XmlNode In oNodelist
                            If oNode IsNot Nothing Then
                                ...
                                If (dThisPayment > 0) Then ' AndAlso (sStatusCode = "NP" OrElse sStatusCode = "AUTH")) Then
                                    ...
                                    End If
                                    If oNode.Attributes("PARTYKEY") IsNot Nothing Then
        								sPartyKey = oNode.Attributes("PARTYKEY").Value
                                    End If
                                    ...
                                    If oNode.Attributes("PAYMENT_TYPE") IsNot Nothing Then
                                        sMediaTypeCode = GetListItemCodefromID("PMLookUp", "UDL_CLA_MEDIATYPE", oNode.Attributes("PAYMENT_TYPE").Value)
                                    End If
                                    ...
                                    End If
                                    If oNode.Attributes("PAYMENT_TYPE") IsNot Nothing Then
                                        sMediaType = GetListItemDescfromCode("PMLookUp", "UDL_CLA_MEDIATYPE", oNode.Attributes("PAYMENT_TYPE").Value)
                                    End If
                                    If oNode.Attributes("ACCOUNT_TYPE") IsNot Nothing Then
                                    sAccountType = GetListItemCodefromID("PMLookUp", "UDL_CLA_ACCTYPE", oNode.Attributes("ACCOUNT_TYPE").Value)
                                    End If
                                    ...
                                    htPaymentDetails.Add("PARTYKEY", sPartyKey)
                                    ...
                                    ' htPaymentDetails.Add("POSTCODE", sPostCode)
                                     ...
                                     Dim oPaymentNodelist As XmlNodeList = xmlDoc.SelectNodes("//CPROCLAIM/OWNDCLAIM")
                                    For Each oPaymentNode As XmlNode In oPaymentNodelist
        								...
        								Dim sTaxAmount As String = String.Empty
        								Dim sTaxGroupCode As String = String.Empty
                                        ...
                                        If oPaymentNode IsNot Nothing Then
                                            If oPaymentNode.Attributes("PERIL") IsNot Nothing Then
                                                sPerilTypeCode = GetListItemCodefromID("PMLookUp", "UDL_CPRO_PERIL_TYPE", oPaymentNode.Attributes("PERIL").Value)
                                            End If
                                            If oPaymentNode.Attributes("DESCRIPTION") IsNot Nothing Then
                                                sReserveTypeCode = GetListItemCodefromID("PMLookUp", "UDL_CLA_INDEMNITY", oPaymentNode.Attributes("DESCRIPTION").Value)
                                            End If
                                            If oPaymentNode.Attributes("PAID_AMOUNT") IsNot Nothing Then
                                                sPaymentAmount = oPaymentNode.Attributes("PAID_AMOUNT").Value
                                            End If
        									
        									If oPaymentNode.Attributes("TAX_GROUP") IsNot Nothing Then
                                                TaxGroupCode = GetListItemCodefromID("PMLookUp", "UDL_CLA_TAXGROUP",oPaymentNode.Attributes("TAX_GROUP").Value)
                                            End If
        									
        									If oPaymentNode.Attributes("TAX_GROUP") IsNot Nothing Then
        										Select Case TaxGroupCode
        											Case "VAT"
        												sTaxGroupCode = "2"
        											Case "VAT0"
        												sTaxGroupCode = "5"
        									...
        									If oPaymentNode.Attributes("TAX_AMOUNT") IsNot Nothing Then
                                                sTaxAmount = oPaymentNode.Attributes("TAX_AMOUNT").Value
                                            End If
        
                                            If (AddPayment(sPerilTypeCode, sReserveTypeCode, sTaxGroupCode, sTaxAmount, sPaymentAmount, htPaymentDetails)) Then
                                    ...
                                    If (bIsPaymentProcessed And cPaymentCollection IsNot Nothing) Then
                                        ...
                                        oNodelist = xmlDoc.SelectNodes("//CPROCLAIM/OWNDCLAIM")
                                        ...
                                        For Each oPaymentNode As XmlNode In oNodelist
                                            ...
                                            If oPaymentNode.Attributes("PAYMENT_AMOUNT") IsNot Nothing Then
        										sThisPaymentAmount = oPaymentNode.Attributes("PAYMENT_AMOUNT").Value
                                                
                                            End If
        									
        								
                                            If oPaymentNode.Attributes("ITEM_NUMBER") IsNot Nothing Then
                                                sLossId = oPaymentNode.Attributes("ITEM_NUMBER").Value
                                                'sTotalPaidAmount = oPaymentNode.Attributes("RESERVE").Value
                                                sTotalPaidAmount = oPaymentNode.Attributes("TOTAL_PAID").Value
                                            End If
        
                                            If oPaymentNode.Attributes("OUT_PAYMENT") IsNot Nothing Then
                                                sOutstandingAmount = oPaymentNode.Attributes("OUT_PAYMENT").Value
                                            End If
        
        									sNetTotalPaymentAmount = CDbl(sTotalPaidAmount) + CDbl(sThisPaymentAmount)
                                            sNetTotalOutstandingAmount = oPaymentNode.Attributes("RESERVE").Value - CDbl(sNetTotalPaymentAmount) '(sThisPaymentAmount) CDbl(sOutstandingAmount) '
                                            'UpdateClaimXML("CPROCLAIM/OWNDCLAIM", "PAYMENT_AMOUNT", "0.00", True, sReseveOI)
                                            UpdateClaimXML("CPROCLAIM/OWNDCLAIM", "OUT_PAYMENT", sNetTotalOutstandingAmount, True, sReseveOI)
                                            UpdateClaimXML("CPROCLAIM/OWNDCLAIM", "TOTAL_PAID", sNetTotalPaymentAmount, True, sReseveOI)
                                        Next
        ...
        Function AddPayment(sPerilTypeCode As String, sReserveTypeCode As String, sTaxGroupCode As String, sTaxAmount As String, sPaymentAmount As String, htPaymentDetails As Hashtable) As Boolean
            ...
            Try
                ...
                With oClaimOpen.ClaimPeril(nPeril)
                    ...
                    .ClaimReserve(nPaymentIndex).ThisPaymentTax = CDec(sTaxAmount) 'Added to see what the VAT Amount is
                    .ClaimReserve(nPaymentIndex).ThisPaymentINCLTax = CDec(sPaymentAmount)
                    .ClaimReserve(nPaymentIndex).CostToClaim = CDec(CDec(sPaymentAmount) - CDec(sTaxAmount)) 'Added to see what VAT does
        			...
        			oClaimOpen.ClaimPeril(nPeril).Payment.TaxGroupCode = CDec(sTaxGroupCode) 'Added to test if it will post on GL
                    ...
                    oClaimOpen.ClaimPeril(nPeril).Payment.TaxAmount = CDec(sTaxAmount)
            ...
            Try
                If Session(CNMode) = Mode.PayClaim Then
                    ...
                    With oClaimOpen.ClaimPeril(nPeril).Payment.Payee
                        ...
                        '.PartyBankKey = OWNDCLAIM__Account_Type.Value
                ...
                End If
                        ...
                        Select Case htPaymentDetails("PAYEE_TYPECODE")
                            ...
                            Case "PARTY"
                                ...
                                oClaimOpen.ClaimPeril(nPeril).Payment.PartyKey = htPaymentDetails("PARTYKEY")
                            ' Case "INSURED"
                                ' oClaimOpen.ClaimPeril(nPeril).Payment.PaymentPartyType = NexusProvider.ClaimPaymentPartyTypeType.CLIENT
                                ' oClaimOpen.ClaimPeril(nPeril).Payment.PartyPaidCode = oClaimOpen.ClaimPeril(nPeril).Payment.PartyPaidCode
                            Case "CLMPAYABLE"
        ...
        Sub UpdatePaymentData(iPeril As Integer)
            ...
            Dim oPayment As NexusProvider.ClaimPayment = CType(Session(CNClaim), NexusProvider.ClaimOpen).ClaimPeril(iPeril).Payment  'To be investigated
            Dim oWebservice As NexusProvider.ProviderBase = New NexusProvider.ProviderManager().Provider
        ...
        'For updating reserves For claim reinsurance 
        ...
        Protected Sub RunDefaultRule()
            ...
            Dim sScreenCode As String = "CPOWNSCRN"
        ...
        Protected Sub btnNextTop_ClickExpenses(sender As Object, e As EventArgs) Handles btnNextTop.Click, btnNext.Click
             If (CType(Session.Item(CNMode), Mode) = Mode.PayClaim) Then 'And GetValue("RESERVES", "PROCESSED_THIS_PAYMENT") <> "1" 
                        ProcessExpensesPayment()
                        Session(CNEnablePayClaim) = True
        	 ...
        	 'Added for updating reserves for claim reinsurance
        	 If Session(CNMode) = Mode.NewClaim Or Session(CNMode) = Mode.EditClaim Then
                        UpdateExpensesReserveData()
             End If
        
                    'Dim dThisReceipt As Double
                    'Double.TryParse(RESERVES__Total_This_Receipt.Text, dThisReceipt)
                    'If ((CType(Session.Item(CNMode), Mode) = Mode.SalvageClaim) OrElse (CType(Session.Item(CNMode), Mode) = Mode.TPRecovery)) AndAlso (dThisReceipt > 0) Then
                    '    UpdateExpensesClaimXML("CPROCLAIM", "PROCESS_RECEIPT_FLAG", "1", True)
                    '    UpdateExpensesClaimXML("CPROCLAIM", "RECEIPT_PAYEE_TYPE", RESERVES__Receipt_Payee_Type.Value, True)
                    '    UpdateExpensesClaimXML("CPROCLAIM", "RECEIPT_PARTY_NAME", RESERVES__Receipt_Party_Name.PartyKey, True)
                    '    RunExpensesDefaultRule()
                    '    GetClaimDetails(CType(Session.Item(CNClaim), NexusProvider.ClaimOpen).ClaimKey, Nothing)
        ...
        Dim cExpensesPaymentCollection As New System.Collections.Generic.Dictionary(Of Integer, Hashtable)
        Private Sub ProcessExpensesPayment()
                     If (Not String.IsNullOrEmpty(Session(CNDataSet)) AndAlso CType(Session.Item(CNMode), Mode) = Mode.PayClaim) Then
                        Dim srDataset As New System.IO.StringReader(Session(CNDataSet))
                        Dim xmlTR As New XmlTextReader(srDataset)
                        Dim xmlDoc As New XmlDocument
                        ...
                        xmlDoc.Load(xmlTR)
                        xmlTR.Close()
                        Dim oNodelist As XmlNodeList = xmlDoc.SelectNodes("//CPROCLAIM/SCHEDULE_LOSS")
                        For Each oNode As XmlNode In oNodelist
                            If oNode IsNot Nothing Then
                                ...
                                If (dThisPayment > 0) Then ' AndAlso (sStatusCode = "NP" OrElse sStatusCode = "AUTH")) Then
                                    ...
                                    ' htPaymentDetails.Add("POSTCODE", sPostCode)
                                     ...
                                     Dim oPaymentNodelist As XmlNodeList = xmlDoc.SelectNodes("//CPROCLAIM/SCHEDULE_LOSS")
                                    For Each oPaymentNode As XmlNode In oPaymentNodelist
                                        Dim sPerilTypeCode As String = String.Empty
                                        Dim sReserveTypeCode As String = String.Empty
                                        Dim sPaymentAmount As String = String.Empty
                                        ...
                                        If oPaymentNode IsNot Nothing Then
                                            ...
                                            End If
                                            If oPaymentNode.Attributes("DESCRIPTION") IsNot Nothing Then
                                                sReserveTypeCode = GetListItemDescfromCode("PMLookUp", "UDL_CLA_RESERVE", oPaymentNode.Attributes("DESCRIPTION").Value)
                                            End If
                                            If oPaymentNode.Attributes("PAID_AMOUNT") IsNot Nothing Then
                                                sPaymentAmount = oPaymentNode.Attributes("PAID_AMOUNT").Value
                                            End If
                                            ...
                                            If (AddExpensesPayment(sPerilTypeCode, sReserveTypeCode, sTaxGroupCode, sTaxAmount, sPaymentAmount, htPaymentDetails)) Then
                                                bIsPaymentProcessed = True
                                            End If
                                        End If
                                    Next
        
                                    If (bIsPaymentProcessed And cExpensesPaymentCollection IsNot Nothing) Then
                                        For Each oPaymentItem In cExpensesPaymentCollection
                                            SaveExpensesPaymentData(oPaymentItem.Key, oPaymentItem.Value)
                                        Next
        
                                        'UpdateExpensesClaimXML("RESERVES/PAYMENT", "STATUSCODE", "PAID", True, sPaymentOI)
                                        'UpdateExpensesClaimXML("RESERVES/PAYMENT", "STATUS", "4", True, sPaymentOI)
                                        'UpdateExpensesClaimXML("RESERVES/PAYMENT", "PAID_PAYMENT_PROCESSED_FLAG", "1", True, sPaymentOI)
                                        oNodelist = xmlDoc.SelectNodes("//CPROCLAIM/SCHEDULE_LOSS")
                                        Dim sReseveOI As String = String.Empty
                                        Dim sTotalPaidAmount As String = String.Empty
                                        Dim sThisPaymentAmount As String = String.Empty
                                        Dim sOutstandingAmount As String = String.Empty
                                        ...
                                        For Each oPaymentNode As XmlNode In oNodelist
                                            ...
                                            sNetTotalPaymentAmount = CDbl(sTotalPaidAmount) + CDbl(sThisPaymentAmount)
                                            sNetTotalOutstandingAmount = oPaymentNode.Attributes("RESERVE").Value - CDbl(sNetTotalPaymentAmount) '(sThisPaymentAmount) CDbl(sOutstandingAmount) '
                                            'UpdateClaimXML("CPROCLAIM/SCHEDULE_LOSS", "PAYMENT_AMOUNT", "0.00", True, sReseveOI)
                                            UpdateClaimXML("CPROCLAIM/SCHEDULE_LOSS", "OUT_PAYMENT", sNetTotalOutstandingAmount, True, sReseveOI)
                                            UpdateClaimXML("CPROCLAIM/SCHEDULE_LOSS", "TOTAL_PAID", sNetTotalPaymentAmount, True, sReseveOI)
                                        Next
                                        'UpdateExpensesClaimXML("RESERVES/PAYMENT[@ID='" & RESERVES__PROCESS_PAYMENT_ID.Text & "']", "PAID_PAYMENT_PROCESSED_FLAG", "1", True)
                                        'RESERVES__PROCESS_PAYMENT_ID.Text = ""
                                        'UpdateExpensesClaimXML("RESERVES", "PROCESS_PAYMENT_FLAG", "", True)
                                        'UpdateExpensesClaimXML("RESERVES", "PROCESS_PAYMENT", "1", True)
                                        'UpdateExpensesClaimXML("RESERVES", "PROCESSED_THIS_PAYMENT", "1", True)
                                        RunExpensesDefaultRule()
                                        bIsPaymentProcessed = False
                                    End If
                                End If
        ...
        Dim nExpensesPayment As Integer = 0
        
        Function AddExpensesPayment(sPerilTypeCode As String, sReserveTypeCode As String, sTaxGroupCode As String, sTaxAmount As String, sPaymentAmount As String, htPaymentDetails As Hashtable) As Boolean
            Dim nPeril As Integer
            Dim nPaymentIndex As Integer
            Dim sFormatString As String = String.Empty
            Dim bAddPayment As Boolean = False
            ...
            Try
                ...
                If nExpensesPayment = 0 Then
                    For nPerilCount As Integer = 0 To oClaimOpen.ClaimPeril.Count - 1
                        ...
                        End If
                    Next
                    If Not PopulateExpensesReserveItem(nPeril) Then
                        Return bAddPayment
                    End If
                End If
                nExpensesPayment = nExpensesPayment + 1
                ...
                With oClaimOpen.ClaimPeril(nPeril)
                    ...
                    If (dBalanceAmount > 0) Then
                        If .ClaimReserve(nPaymentIndex).CurrentReserve <> String.Format(sFormatString, CDec(dBalanceAmount)) Then
                            UpdateExpensesReserve(oClaimOpen, nPeril, nPaymentIndex, dBalanceAmount)
                        End If
                    End If
                End With
                Session(CNClaim) = oClaimOpen
                If Not cExpensesPaymentCollection.ContainsKey(nPeril) Then
                       cExpensesPaymentCollection.Add(nPeril, htPaymentDetails)
                End If
        ...
        Sub UpdateExpensesReserve(ByRef oClaim As NexusProvider.ClaimOpen, nPeril As Integer, nPaymentIndex As Integer, sBalanceAmount As String)
            Dim oModeClaim As Mode = CType(Session.Item(CNMode), Mode)
            Dim sAmount As String = sBalanceAmount
            Dim dTaxAmount As Decimal = 0D
            Dim sOption As String
        ...
        Function PopulateExpensesReserveItem(iPeril As Integer) As Boolean
            Dim oClaimOpen As NexusProvider.ClaimOpen = Nothing
            Dim oClaimPayment As New NexusProvider.ClaimPayment
            Dim oQuote As NexusProvider.Quote = CType(Session(CNClaimQuote), NexusProvider.Quote)
            Dim bIsPaymentPopulated As Boolean = False
        ...
        Function SaveExpensesPaymentData(nPerilIndex As Integer, htPaymentDetails As Hashtable) As Boolean
            Dim nPeril As Integer = nPerilIndex
            Dim bIsPaymentSaved As Boolean = False
            Dim oWebservice As NexusProvider.ProviderBase = New NexusProvider.ProviderManager().Provider
            Dim oNexusConfig As Config.NexusFrameWork = CType(ConfigurationManager.GetSection("NexusFrameWork"), Config.NexusFrameWork)
            ...
            Try
                ...
                If Session(CNMode) = Mode.PayClaim Then
                    ...
                    Try
                        If Session(CNPayClaimError) Is Nothing Then
                            UpdateExpensesPaymentData(nPeril)
                        End If
                    Catch ex As NexusProvider.NexusException
                        If ex.Errors(0).Code = "1000151" Then
                            oPayClaimResponse = New NexusProvider.PayClaimResponse
        ...
        Sub UpdateExpensesPaymentData(iPeril As Integer)
            Dim oClaim As NexusProvider.ClaimOpen = CType(Session.Item(CNClaim), NexusProvider.ClaimOpen)
            Dim oModeClaim As Mode = CType(Session.Item(CNMode), Mode)
            Dim oNexusConfig As Config.NexusFrameWork = CType(ConfigurationManager.GetSection("NexusFrameWork"), Config.NexusFrameWork)
            Dim oPortal As Nexus.Library.Config.Portal = oNexusConfig.Portals.Portal(CMS.Library.Portal.GetPortalID())
        ...
        Public Sub UpdateExpensesClaimXML(ByVal sObjectName As String, ByVal sFieldName As String, ByVal sFieldValue As String, Optional ByVal bIsDelete As Boolean = False, Optional ByVal sOINumber As String = "")
            Dim oDataSet As New SiriusFS.SAM.Client.DataSetControl.Application
            Dim sDatasetXml As String = String.Empty
            Dim oNode As XmlNode
            'get the dataset definition
        ...
        Protected Sub RunExpensesDefaultRule()
            Const kTRANSACTIONTYPE_OpenClaim As String = "C_CO"
            Const kTRANSACTIONTYPE_MaintainClaim As String = "C_CR"
            Const kTRANSACTIONTYPE_PayClaim As String = "C_CP"
            'run default rules edit to populate the category and Special fields on the screen on the basis of version selected as per "Date Of Incident"
            ...
            Dim oWebService As NexusProvider.ProviderBase = New NexusProvider.ProviderManager().Provider
            Dim v_bSkipSaveToDB As Boolean = False
            Dim sScreenCode As String = "CPISOLSCRN"
            Dim sTransactionType As String = ""
