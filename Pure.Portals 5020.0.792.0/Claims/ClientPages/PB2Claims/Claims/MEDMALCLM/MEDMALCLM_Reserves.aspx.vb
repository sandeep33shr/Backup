...
Namespace Nexus
    Partial Class PB2_MEDMALCLM_Reserves : Inherits BaseClaim
		...
		Protected Sub onValidate_MEDMALCLAIM__MEDOWNDSCR()
...
End Sub
Protected Sub onValidate_MEDMALCLAIM__TransactionType()
        ...
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
           Dim sTransactionType As String = String.Empty
                     ...
                     MEDMALCLAIM__TransactionType.Text = sTransactionType		 
                    ...
                    Try
                        If (Not String.IsNullOrEmpty(Session(CNDataSet))) Then
                            ...
                            Doc.Load(xmlTR)
                            xmlTR.Close()
                            Dim oNodelist As XmlNodeList = Doc.SelectNodes("//MEDMALCLAIM/RECOVERIES")
...
End Sub
Protected Sub onValidate_MEDMALCLAIM__MEDSOLSCRN()
                    If eLifecycleEvent = LifecycleEvent.Page_Load Then
                        If Not Page.IsPostBack Then
                            GetClaimDetails(CType(Session.Item(CNClaim), NexusProvider.ClaimOpen).ClaimKey, Nothing)
                            SetScriptPayment()
...
Protected Sub CallRuleScripts()
    onValidate_MEDMALCLAIM__MEDOWNDSCR()
    onValidate_MEDMALCLAIM__TransactionType()
    onValidate_MEDMALCLAIM__MEDSOLSCRN()
End Sub
        ...
        Private Sub ProcessPayment()
                     If (Not String.IsNullOrEmpty(Session(CNDataSet)) AndAlso CType(Session.Item(CNMode), Mode) = Mode.PayClaim) Then
                        ...
                        xmlDoc.Load(xmlTR)
                        xmlTR.Close()
                        Dim oNodelist As XmlNodeList = xmlDoc.SelectNodes("//MEDMALCLAIM/OWNDCLAIM")
                        For Each oNode As XmlNode In oNodelist
                            If oNode IsNot Nothing Then
                                ...
                                If (dThisPayment > 0) Then ' AndAlso (sStatusCode = "NP" OrElse sStatusCode = "AUTH")) Then
                                    ...
                                    ' htPaymentDetails.Add("POSTCODE", sPostCode)
                                     ...
                                     Dim oPaymentNodelist As XmlNodeList = xmlDoc.SelectNodes("//MEDMALCLAIM/OWNDCLAIM")
                                    For Each oPaymentNode As XmlNode In oPaymentNodelist
                                        ...
                                        If oPaymentNode IsNot Nothing Then
                                            If oPaymentNode.Attributes("PERIL") IsNot Nothing Then
                                                sPerilTypeCode = GetListItemCodefromID("PMLookUp", "UDL_MEDMAL_PERIL_TYPE", oPaymentNode.Attributes("PERIL").Value)
                                            End If
                                    ...
                                    If (bIsPaymentProcessed And cPaymentCollection IsNot Nothing) Then
                                        ...
                                        oNodelist = xmlDoc.SelectNodes("//MEDMALCLAIM/OWNDCLAIM")
                                        ...
                                        For Each oPaymentNode As XmlNode In oNodelist
                                            ...
                                            UpdateClaimXML("MEDMALCLAIM/OWNDCLAIM", "OUT_PAYMENT", sNetTotalOutstandingAmount, True, sReseveOI)
                                            UpdateClaimXML("MEDMALCLAIM/OWNDCLAIM", "TOTAL_PAID", sNetTotalPaymentAmount, True, sReseveOI)
                                        Next
        ...
        Protected Sub RunDefaultRule()
            ...
            Dim sScreenCode As String = "MEDOWNDSCR"
        ...
        Private Sub ProcessExpensesPayment()
                     If (Not String.IsNullOrEmpty(Session(CNDataSet)) AndAlso CType(Session.Item(CNMode), Mode) = Mode.PayClaim) Then
                        ...
                        xmlDoc.Load(xmlTR)
                        xmlTR.Close()
                        Dim oNodelist As XmlNodeList = xmlDoc.SelectNodes("//MEDMALCLAIM/SCHEDULE_LOSS")
                        For Each oNode As XmlNode In oNodelist
                            If oNode IsNot Nothing Then
                                ...
                                If (dThisPayment > 0) Then ' AndAlso (sStatusCode = "NP" OrElse sStatusCode = "AUTH")) Then
                                    ...
                                    ' htPaymentDetails.Add("POSTCODE", sPostCode)
                                     ...
                                     Dim oPaymentNodelist As XmlNodeList = xmlDoc.SelectNodes("//MEDMALCLAIM/SCHEDULE_LOSS")
                                    For Each oPaymentNode As XmlNode In oPaymentNodelist
                                        Dim sPerilTypeCode As String = String.Empty
                                        Dim sReserveTypeCode As String = String.Empty
                                        Dim sPaymentAmount As String = String.Empty
                                    ...
                                    If (bIsPaymentProcessed And cExpensesPaymentCollection IsNot Nothing) Then
                                        ...
                                        oNodelist = xmlDoc.SelectNodes("//MEDMALCLAIM/SCHEDULE_LOSS")
                                        Dim sReseveOI As String = String.Empty
                                        Dim sTotalPaidAmount As String = String.Empty
                                        Dim sThisPaymentAmount As String = String.Empty
                                        Dim sOutstandingAmount As String = String.Empty
                                        ...
                                        For Each oPaymentNode As XmlNode In oNodelist
                                            ...
                                            UpdateClaimXML("MEDMALCLAIM/SCHEDULE_LOSS", "OUT_PAYMENT", sNetTotalOutstandingAmount, True, sReseveOI)
                                            UpdateClaimXML("MEDMALCLAIM/SCHEDULE_LOSS", "TOTAL_PAID", sNetTotalPaymentAmount, True, sReseveOI)
                                        Next
        ...
        Protected Sub RunExpensesDefaultRule()
            ...
            Dim oWebService As NexusProvider.ProviderBase = New NexusProvider.ProviderManager().Provider
            Dim v_bSkipSaveToDB As Boolean = False
            Dim sScreenCode As String = "MEDSOLSCRN"
            Dim sTransactionType As String = ""
