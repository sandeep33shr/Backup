...
Namespace Nexus
    Partial Class PB2_MEDMALCLM_Underwriting_Information : Inherits BaseClaim
		...
		Protected Sub onValidate_MEDMALCLAIM__IS_FAC()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_MEDMALCLAIM__IS_FAC()
End Sub
        ...
        Protected Shadows Sub Page_Load_ClientInfo(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
            ...
            If Session(CNParty) IsNot Nothing Then
                If TypeOf Session(CNParty) Is NexusProvider.CorporateParty Then
                    ...
                    MEDMALCLAIM__INSURED.Text = DirectCast(oParty, NexusProvider.CorporateParty).CompanyName
                End If
                ...
                If TypeOf Session(CNParty) Is NexusProvider.PersonalParty Then
                    ...
                    MEDMALCLAIM__INSURED.Text = (firstName + " " + LastName).ToString
        			MEDMALCLAIM__CONTACTNUM.Text = Trim(MobileNo).ToString
                End If
            ...
            If Session(CNClaimQuote) IsNot Nothing Then
                MEDMALCLAIM__POLNUMBER.Text = RTrim(LTrim(oClaimQuote.InsuranceFileRef))
                MEDMALCLAIM__ALTERPOLICY.Text = RTrim(LTrim(oClaimQuote.AlternativeRef))
                MEDMALCLAIM__CVR_STARTDATE.Text = RTrim(LTrim(oClaimQuote.CoverStartDate))
                MEDMALCLAIM__CVR_ENDDATE.Text = RTrim(LTrim(oClaimQuote.CoverEndDate))
                MEDMALCLAIM__INCEPT_DATE.Text = RTrim(LTrim(oClaimQuote.InceptionDate))
        		...
        		For nIndex = 0 To Cnt
        			...
        			End If
        		Next
        		MEDMALCLAIM__IS_FAC.Text = FAC.ToString
