...
Namespace Nexus
    Partial Class PB2_MEDMALCLM_Claim_Information : Inherits BaseClaim
		...
		Protected Sub onValidate_MEDMALCLAIM__SALVAGESTAT()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        
        	If CType(Session.Item(CNMode), Mode) = Mode.PayClaim Then
        		Response.Redirect("~/Claims/ClientPages/PB2Claims/Claims/MEDMALCLM/MEDMALCLM_Reserves.aspx")
        	End If
        	
        End If
End Sub
Protected Sub onValidate_MEDMALCLAIM__MEDTRAIL()
        
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_MEDMALCLAIM__SALVAGESTAT()
    onValidate_MEDMALCLAIM__MEDTRAIL()
End Sub
        ...
        Protected Sub UpdateAuditTrail()
        	...
        	Dim salvage As String = MEDMALCLAIM__SALVAGESTAT.Text
        	Dim legal As String = MEDMALCLAIM__LEGALSTAT.Text
        	Dim decision As String = MEDMALCLAIM__CLMDECISION.Text
        	...
        	If (salvage_updatereq Or legal_updatereq Or decision_updatereq) Then
        		...
        		Session(CNDataSet) = oWebService.RunDefaultRulesEdit(MEDMALCLAIM__MEDMALAUDIT.ScreenCode, Session(CNDataSet))
        	End If
        ...
        Protected Function AuditEntryRequired(ByVal propertyName As String, ByVal propertyValue As String, ByRef previousValue As String) As Boolean
        	...
        	For Each el As Linq.XElement In datamodelXML.DescendantNodes()
        		If (el.Name.ToString().ToUpper() = "MEDMALAUDIT") Then
        ...
        Protected Sub LogAuditEntry(ByVal propertyName As String, ByVal propertyValue As String, ByVal previousValue As String)
        	...
        	Doc.LoadXml(strDS)
        	Dim xmlParent As XmlNode = Doc.SelectSingleNode("//" & Session.Item(CNDataModelCode) & "_POLICY_BINDER/MEDMALCLAIM/MEDMALAUDIT")
        	Dim v_sOI As String = Nothing
        	If xmlParent IsNot Nothing Then
        		v_sOI = xmlParent.Attributes("OI").Value
        	End If
        	Dim v_sScreenCode As String = MEDMALCLAIM__MEDMALAUDIT.ScreenCode
        	Dim sParentElement, sChildElement As String
        	Dim sTmp() As String = Regex.Split(MEDMALCLAIM__MEDMALAUDIT.ID, "__")
        	...
        	MEDMALCLAIM__MEDMALAUDIT.DataSource = oXMLSource
        	MEDMALCLAIM__MEDMALAUDIT.DataBind()
        ...
        ' MEDMALCLAIM = The parent object name of child screen
        ' MEDMALAUDIT = The child screen object name
        
        Protected Sub MEDMALCLAIM__MEDMALAUDIT_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles MEDMALCLAIM__MEDMALAUDIT.RowDataBound
