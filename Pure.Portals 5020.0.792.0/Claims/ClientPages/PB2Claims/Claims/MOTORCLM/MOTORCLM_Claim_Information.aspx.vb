...
Namespace Nexus
    Partial Class PB2_MOTORCLM_Claim_Information : Inherits BaseClaim
		...
		Protected Sub onValidate_MOTORCLAIM__MOTTRAIL()
        
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_MOTORCLAIM__MOTTRAIL()
End Sub
        ...
        Protected Sub UpdateAuditTrail()
        	...
        	Dim salvage As String = MOTORCLAIM__SALVAGESTAT.Text
        	Dim legal As String = MOTORCLAIM__LEGALSTAT.Text
        	Dim decision As String = MOTORCLAIM__CLMDECISION.Text
        	...
        	If (salvage_updatereq Or legal_updatereq Or decision_updatereq) Then
        		...
        		Session(CNDataSet) = oWebService.RunDefaultRulesEdit(MOTORCLAIM__AUDIT.ScreenCode, Session(CNDataSet))
        	End If
        ...
        Protected Sub LogAuditEntry(ByVal propertyName As String, ByVal propertyValue As String, ByVal previousValue As String)
        	...
        	Doc.LoadXml(strDS)
        	Dim xmlParent As XmlNode = Doc.SelectSingleNode("//" & Session.Item(CNDataModelCode) & "_POLICY_BINDER/MOTORCLAIM/AUDIT")
        	Dim v_sOI As String = Nothing
        	If xmlParent IsNot Nothing Then
        		v_sOI = xmlParent.Attributes("OI").Value
        	End If
        	Dim v_sScreenCode As String = MOTORCLAIM__AUDIT.ScreenCode
        	Dim sParentElement, sChildElement As String
        	Dim sTmp() As String = Regex.Split(MOTORCLAIM__AUDIT.ID, "__")
        	...
        	MOTORCLAIM__AUDIT.DataSource = oXMLSource
        	MOTORCLAIM__AUDIT.DataBind()
        ...
        ' MOTORCLAIM = The parent object name of child screen
        ' AUDIT = The child screen object name
        
        Protected Sub MOTORCLAIM__AUDIT_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles MOTORCLAIM__AUDIT.RowDataBound
