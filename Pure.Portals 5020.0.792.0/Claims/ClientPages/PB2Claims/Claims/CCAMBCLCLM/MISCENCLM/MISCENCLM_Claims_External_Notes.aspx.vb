...
Namespace Nexus
    Partial Class PB2_MISCENCLM_Claims_External_Notes : Inherits BaseClaim
		...
		Protected Sub onValidate_CLMMISC_EXT_DETAILS__Note_Details()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        	If CLMMISC_EXT_DETAILS__Date_Created.Text.ToString = "" Then
        		CLMMISC_EXT_DETAILS__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        		CLMMISC_EXT_DETAILS__Created_By.Text = Session(CNLoginName).ToString
        	End If
        End If
End Sub
Protected Sub onValidate_CLMMISC_EXT_DETAILS__NoteCheck()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_CLMMISC_EXT_DETAILS__Note_Details()
    onValidate_CLMMISC_EXT_DETAILS__NoteCheck()
End Sub
        ...
        Protected Sub NotesAdded() 
        	Dim NoteAddedCheck = CLMMISC_EXT_DETAILS__NoteCheck.Text
        	
        	If NoteAddedCheck = 0 Then
        		CLMMISC_EXT_DETAILS__NoteCheck.Text = 1
        	End If
        End Sub
