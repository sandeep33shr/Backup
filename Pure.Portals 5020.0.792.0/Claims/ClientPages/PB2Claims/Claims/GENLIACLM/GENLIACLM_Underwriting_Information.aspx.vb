...
Namespace Nexus
    Partial Class PB2_GENLIACLM_Underwriting_Information : Inherits BaseClaim
		...
		Protected Sub onValidate_GENLIABCLAIM__IS_FAC()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_GENLIABCLAIM__IS_FAC()
End Sub
        ...
        Protected Shadows Sub Page_Load_ClientInfo(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
            ...
            If Session(CNParty) IsNot Nothing Then
                If TypeOf Session(CNParty) Is NexusProvider.CorporateParty Then
                    ...
                    GENLIABCLAIM__INSURED.Text = DirectCast(oParty, NexusProvider.CorporateParty).CompanyName
                End If
                ...
                If TypeOf Session(CNParty) Is NexusProvider.PersonalParty Then
                    ...
                    GENLIABCLAIM__INSURED.Text = (firstName + " " + LastName).ToString
                    GENLIABCLAIM__CONTACTNUM.Text = Trim(MobileNo).ToString
                End If
            ...
            If Session(CNClaimQuote) IsNot Nothing Then
                GENLIABCLAIM__POLNUMBER.Text = RTrim(LTrim(oClaimQuote.InsuranceFileRef))
                GENLIABCLAIM__ALTERPOLICY.Text = RTrim(LTrim(oClaimQuote.AlternativeRef))
                GENLIABCLAIM__CVR_STARTDATE.Text = RTrim(LTrim(oClaimQuote.CoverStartDate))
                GENLIABCLAIM__CVR_ENDDATE.Text = RTrim(LTrim(oClaimQuote.CoverEndDate))
                GENLIABCLAIM__INCEPT_DATE.Text = RTrim(LTrim(oClaimQuote.InceptionDate))
        		...
        		For nIndex = 0 To Cnt
        			...
        			End If
        		Next
        		GENLIABCLAIM__IS_FAC.Text = FAC.ToString
