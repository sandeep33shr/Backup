...
Namespace Nexus
    Partial Class PB2_TUPMOTOCLM_Claim_Information : Inherits BaseClaim
		...
		Protected Sub onValidate_TMOTOCLAIM__TMOTOTRAIL()
        
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_TMOTOCLAIM__TMOTOTRAIL()
End Sub
        ...
        Protected Sub UpdateAuditTrail()
        	...
        	Dim salvage As String = TMOTOCLAIM__SALVAGESTAT.Text
        	Dim legal As String = TMOTOCLAIM__LEGALSTAT.Text
        	Dim decision As String = TMOTOCLAIM__CLMDECISION.Text
        	...
        	If (salvage_updatereq Or legal_updatereq Or decision_updatereq) Then
        		...
        		Session(CNDataSet) = oWebService.RunDefaultRulesEdit(TMOTOCLAIM__TMOTOAUDIT.ScreenCode, Session(CNDataSet))
        	End If
        ...
        Protected Function AuditEntryRequired(ByVal propertyName As String, ByVal propertyValue As String, ByRef previousValue As String) As Boolean
        	...
        	For Each el As Linq.XElement In datamodelXML.DescendantNodes()
        		If (el.Name.ToString().ToUpper() = "TMOTOAUDIT") Then
        ...
        Protected Sub LogAuditEntry(ByVal propertyName As String, ByVal propertyValue As String, ByVal previousValue As String)
        	...
        	Doc.LoadXml(strDS)
        	Dim xmlParent As XmlNode = Doc.SelectSingleNode("//" & Session.Item(CNDataModelCode) & "_POLICY_BINDER/TMOTOCLAIM/TMOTOAUDIT")
        	Dim v_sOI As String = Nothing
        	If xmlParent IsNot Nothing Then
        		v_sOI = xmlParent.Attributes("OI").Value
        	End If
        	Dim v_sScreenCode As String = TMOTOCLAIM__TMOTOAUDIT.ScreenCode
        	Dim sParentElement, sChildElement As String
        	Dim sTmp() As String = Regex.Split(TMOTOCLAIM__TMOTOAUDIT.ID, "__")
        	...
        	TMOTOCLAIM__TMOTOAUDIT.DataSource = oXMLSource
        	TMOTOCLAIM__TMOTOAUDIT.DataBind()
        ...
        ' TMOTOCLAIM = The parent object name of child screen
        ' TMOTOAUDIT = The child screen object name
        
        Protected Sub TMOTOCLAIM__TMOTOAUDIT_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles TMOTOCLAIM__TMOTOAUDIT.RowDataBound
