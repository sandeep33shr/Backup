...
Namespace Nexus
    Partial Class PB2_PIPESTRCLM_Claim_Information : Inherits BaseClaim
		...
		Protected Sub onValidate_PIPESTRCLAIM__PIPETRAIL()
        
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_PIPESTRCLAIM__PIPETRAIL()
End Sub
        ...
        Protected Sub UpdateAuditTrail()
        	...
        	Dim salvage As String = PIPESTRCLAIM__SALVAGESTAT.Text
        	Dim legal As String = PIPESTRCLAIM__LEGALSTAT.Text
        	Dim decision As String = PIPESTRCLAIM__CLMDECISION.Text
        	...
        	If (salvage_updatereq Or legal_updatereq Or decision_updatereq) Then
        		...
        		Session(CNDataSet) = oWebService.RunDefaultRulesEdit(PIPESTRCLAIM__PIPESTRAUDIT.ScreenCode, Session(CNDataSet))
        	End If
        ...
        Protected Function AuditEntryRequired(ByVal propertyName As String, ByVal propertyValue As String, ByRef previousValue As String) As Boolean
        	...
        	For Each el As Linq.XElement In datamodelXML.DescendantNodes()
        		If (el.Name.ToString().ToUpper() = "PIPESTRAUDIT") Then
        ...
        Protected Sub LogAuditEntry(ByVal propertyName As String, ByVal propertyValue As String, ByVal previousValue As String)
        	...
        	Doc.LoadXml(strDS)
        	Dim xmlParent As XmlNode = Doc.SelectSingleNode("//" & Session.Item(CNDataModelCode) & "_POLICY_BINDER/PIPESTRCLAIM/PIPESTRAUDIT")
        	Dim v_sOI As String = Nothing
        	If xmlParent IsNot Nothing Then
        		v_sOI = xmlParent.Attributes("OI").Value
        	End If
        	Dim v_sScreenCode As String = PIPESTRCLAIM__PIPESTRAUDIT.ScreenCode
        	Dim sParentElement, sChildElement As String
        	Dim sTmp() As String = Regex.Split(PIPESTRCLAIM__PIPESTRAUDIT.ID, "__")
        	...
        	PIPESTRCLAIM__PIPESTRAUDIT.DataSource = oXMLSource
        	PIPESTRCLAIM__PIPESTRAUDIT.DataBind()
        ...
        ' PIPESTRAUDIT = The child screen object name
        
        Protected Sub PIPESTRCLAIM__PIPESTRAUDIT_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles PIPESTRCLAIM__PIPESTRAUDIT.RowDataBound
