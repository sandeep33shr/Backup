...
Namespace Nexus
    Partial Class PB2_CLMPERSACC_General : Inherits BaseClaim
		...
		Protected Sub onValidate_PSACLAIM__PSATRAIL()
        
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_PSACLAIM__PSATRAIL()
End Sub
        ...
        Protected Sub UpdateAuditTrail()
        	...
        	Dim salvage As String = PSACLAIM__SALVAGESTAT.Text
        	Dim legal As String = PSACLAIM__LEGALSTAT.Text
        	Dim decision As String = PSACLAIM__CLMDECISION.Text
        	...
        	If (salvage_updatereq Or legal_updatereq Or decision_updatereq) Then
        		...
        		Session(CNDataSet) = oWebService.RunDefaultRulesEdit(PSACLAIM__PSAAUDT.ScreenCode, Session(CNDataSet))
        	End If
        ...
        Protected Function AuditEntryRequired(ByVal propertyName As String, ByVal propertyValue As String, ByRef previousValue As String) As Boolean
        	...
        	For Each el As Linq.XElement In datamodelXML.DescendantNodes()
        		If (el.Name.ToString().ToUpper() = "PSAAUDT") Then
        ...
        Protected Sub LogAuditEntry(ByVal propertyName As String, ByVal propertyValue As String, ByVal previousValue As String)
        	...
        	Doc.LoadXml(strDS)
        	Dim xmlParent As XmlNode = Doc.SelectSingleNode("//" & Session.Item(CNDataModelCode) & "_POLICY_BINDER/PSACLAIM/PSAAUDT")
        	Dim v_sOI As String = Nothing
        	If xmlParent IsNot Nothing Then
        		v_sOI = xmlParent.Attributes("OI").Value
        	End If
        	Dim v_sScreenCode As String = PSACLAIM__PSAAUDT.ScreenCode
        	Dim sParentElement, sChildElement As String
        	Dim sTmp() As String = Regex.Split(PSACLAIM__PSAAUDT.ID, "__")
        	...
        	PSACLAIM__PSAAUDT.DataSource = oXMLSource
        	PSACLAIM__PSAAUDT.DataBind()
        ...
        ' PSACLAIM = The parent object name of child screen
        ' PSAAUDT = The child screen object name
        
        Protected Sub PSACLAIM__PSAAUDT_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles PSACLAIM__PSAAUDT.RowDataBound
