...
Namespace Nexus
    Partial Class PB2_ELECEQCLM_Underwriting_Information : Inherits BaseClaim
		...
		Protected Sub onValidate_ELECEQUIPCLAIM__IS_FAC()
        
End Sub
Protected Sub onValidate_ELECEQUIPCLAIM__ELECCLDCLM()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_ELECEQUIPCLAIM__IS_FAC()
    onValidate_ELECEQUIPCLAIM__ELECCLDCLM()
End Sub
        ...
        Protected Shadows Sub Page_Load_ClientInfo(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
            ...
            If Session(CNParty) IsNot Nothing Then
                If TypeOf Session(CNParty) Is NexusProvider.CorporateParty Then
                    ...
                    ELECEQUIPCLAIM__INSURED.Text = DirectCast(oParty, NexusProvider.CorporateParty).CompanyName
                End If
                ...
                If TypeOf Session(CNParty) Is NexusProvider.PersonalParty Then
                    ...
                    ELECEQUIPCLAIM__INSURED.Text = (firstName + " " + LastName).ToString
        			...
        			For mIndex = 0 To Count
        				...
        				End If
        			Next
        			ELECEQUIPCLAIM__CONTACTNUM.Text = Trim(MobileNo).ToString
                End If
            ...
            If Session(CNClaimQuote) IsNot Nothing Then
                ELECEQUIPCLAIM__POLNUMBER.Text = RTrim(LTrim(oClaimQuote.InsuranceFileRef))
                ELECEQUIPCLAIM__ALTERPOLICY.Text = RTrim(LTrim(oClaimQuote.AlternativeRef))
                ELECEQUIPCLAIM__CVR_STARTDATE.Text = RTrim(LTrim(oClaimQuote.CoverStartDate))
                ELECEQUIPCLAIM__CVR_ENDDATE.Text = RTrim(LTrim(oClaimQuote.CoverEndDate))
                ELECEQUIPCLAIM__INCEPT_DATE.Text = RTrim(LTrim(oClaimQuote.InceptionDate))
        		...
        		For nIndex = 0 To Cnt
        			...
        			End If
        		Next
        		ELECEQUIPCLAIM__IS_FAC.Text = FAC.ToString
            End If
        
                  'ELECEQUIPCLAIM__CONTACTNUM.Text = oQuote.ClientMobileNo
                    'ELECEQUIPCLAIM__POLNUMBER.Text = oQuote.InsuranceFileRef
                    'ELECEQUIPCLAIM__ALTERPOLICY.Text = oQuote.OldPolicyNumber
                    'ELECEQUIPCLAIM__CVR_STARTDATE.Text = String.Format("{0:dd/MM/yyyy}",oQuote.CoverStartDate)
                    'ELECEQUIPCLAIM__CVR_ENDDATE.Text = String.Format("{0:dd/MM/yyyy}",oQuote.CoverEndDate)
                    'ELECEQUIPCLAIM__INCEPT_DATE.Text = String.Format("{0:dd/MM/yyyy}",oQuote.InceptionDate)
        End Sub
     
        ' Note: This rule removes the "Add" button row of a child-screen grid
        ' ELECEQUIPCLAIM = The parent object name of child screen
        ' ELECEQUIPCLM = The child screen object name
        
        Protected Sub ELECEQUIPCLAIM__ELECEQUIPCLM_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles ELECEQUIPCLAIM__ELECEQUIPCLM.RowDataBound
