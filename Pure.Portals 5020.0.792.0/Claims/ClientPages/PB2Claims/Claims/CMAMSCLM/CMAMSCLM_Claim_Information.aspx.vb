...
Namespace Nexus
    Partial Class PB2_CMAMSCLM_Claim_Information : Inherits BaseClaim
...
End Sub
Protected Sub onValidate_CMAMSCLAIM__CMSTRAIL()
        
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_GENERAL__TRANSACTION_TYPE()
    onValidate_CMAMSCLAIM__CMSTRAIL()
End Sub
        ...
        Protected Sub UpdateAuditTrail()
        	...
        	If (salvage_updatereq Or legal_updatereq Or decision_updatereq) Then
        		...
        		Session(CNDataSet) = oWebService.RunDefaultRulesEdit(CMAMSCLAIM__AUDIT.ScreenCode, Session(CNDataSet))
        	End If
        ...
        Protected Sub LogAuditEntry(ByVal propertyName As String, ByVal propertyValue As String, ByVal previousValue As String)
        	...
        	Doc.LoadXml(strDS)
        	Dim xmlParent As XmlNode = Doc.SelectSingleNode("//" & Session.Item(CNDataModelCode) & "_POLICY_BINDER/CMAMSCLAIM/AUDIT")
        	Dim v_sOI As String = Nothing
        	If xmlParent IsNot Nothing Then
        		v_sOI = xmlParent.Attributes("OI").Value
        	End If
        	Dim v_sScreenCode As String = CMAMSCLAIM__AUDIT.ScreenCode
        	Dim sParentElement, sChildElement As String
        	Dim sTmp() As String = Regex.Split(CMAMSCLAIM__AUDIT.ID, "__")
        	...
        	CMAMSCLAIM__AUDIT.DataSource = oXMLSource
        	CMAMSCLAIM__AUDIT.DataBind()
        ...
        ' CMAMSCLAIM = The parent object name of child screen
        ' AUDIT = The child screen object name
        
        Protected Sub CMAMSCLAIM__AUDIT_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles CMAMSCLAIM__AUDIT.RowDataBound
