...
Namespace Nexus
    Partial Class PB2_MACBRKCLM_Claim_Information : Inherits BaseClaim
		...
		Protected Sub onValidate_MACBRKCLAIM__CMBTRAIL()
        
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_MACBRKCLAIM__CMBTRAIL()
End Sub
        ...
        Protected Sub UpdateAuditTrail()
        	...
        	Dim salvage As String = MACBRKCLAIM__SALVAGESTAT.Text
        	Dim legal As String = MACBRKCLAIM__LEGALSTAT.Text
        	Dim decision As String = MACBRKCLAIM__CLMDECISION.Text
        	...
        	If (salvage_updatereq Or legal_updatereq Or decision_updatereq) Then
        		...
        		Session(CNDataSet) = oWebService.RunDefaultRulesEdit(MACBRKCLAIM__MACBRKAUDIT.ScreenCode, Session(CNDataSet))
        	End If
        ...
        Protected Function AuditEntryRequired(ByVal propertyName As String, ByVal propertyValue As String, ByRef previousValue As String) As Boolean
        	...
        	For Each el As Linq.XElement In datamodelXML.DescendantNodes()
        		If (el.Name.ToString().ToUpper() = "MACBRKAUDIT") Then
        ...
        Protected Sub LogAuditEntry(ByVal propertyName As String, ByVal propertyValue As String, ByVal previousValue As String)
        	...
        	Doc.LoadXml(strDS)
        	Dim xmlParent As XmlNode = Doc.SelectSingleNode("//" & Session.Item(CNDataModelCode) & "_POLICY_BINDER/MACBRKCLAIM/MACBRKAUDIT")
        	Dim v_sOI As String = Nothing
        	If xmlParent IsNot Nothing Then
        		v_sOI = xmlParent.Attributes("OI").Value
        	End If
        	Dim v_sScreenCode As String = MACBRKCLAIM__MACBRKAUDIT.ScreenCode
        	Dim sParentElement, sChildElement As String
        	Dim sTmp() As String = Regex.Split(MACBRKCLAIM__MACBRKAUDIT.ID, "__")
        	...
        	MACBRKCLAIM__MACBRKAUDIT.DataSource = oXMLSource
        	MACBRKCLAIM__MACBRKAUDIT.DataBind()
        ...
        ' MACBRKAUDIT = The child screen object name
        
        Protected Sub MACBRKCLAIM__MACBRKAUDIT_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles MACBRKCLAIM__MACBRKAUDIT.RowDataBound
