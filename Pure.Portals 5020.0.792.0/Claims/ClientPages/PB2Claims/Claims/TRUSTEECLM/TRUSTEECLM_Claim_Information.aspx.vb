...
Namespace Nexus
    Partial Class PB2_TRUSTEECLM_Claim_Information : Inherits BaseClaim
		...
		Protected Sub onValidate_TRUSTEECLAIM__SALVAGESTAT()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        
        	If CType(Session.Item(CNMode), Mode) = Mode.PayClaim Then
        		Response.Redirect("~/Claims/ClientPages/PB2Claims/Claims/TRUSTEECLM/TRUSTEECLM_Schedule_Of_Loss.aspx")
        	End If
        	
        End If
End Sub
Protected Sub onValidate_TRUSTEECLAIM__TRUSTTRAIL()
        
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_TRUSTEECLAIM__SALVAGESTAT()
    onValidate_TRUSTEECLAIM__TRUSTTRAIL()
End Sub
        ...
        Protected Sub UpdateAuditTrail()
        	...
        	Dim salvage As String = TRUSTEECLAIM__SALVAGESTAT.Text
        	Dim legal As String = TRUSTEECLAIM__LEGALSTAT.Text
        	Dim decision As String = TRUSTEECLAIM__CLMDECISION.Text
        	...
        	If (salvage_updatereq Or legal_updatereq Or decision_updatereq) Then
        		...
        		Session(CNDataSet) = oWebService.RunDefaultRulesEdit(TRUSTEECLAIM__TRUSTEEAUDIT.ScreenCode, Session(CNDataSet))
        	End If
        ...
        Protected Function AuditEntryRequired(ByVal propertyName As String, ByVal propertyValue As String, ByRef previousValue As String) As Boolean
        	...
        	For Each el As Linq.XElement In datamodelXML.DescendantNodes()
        		If (el.Name.ToString().ToUpper() = "TRUSTEEAUDIT") Then
        ...
        Protected Sub LogAuditEntry(ByVal propertyName As String, ByVal propertyValue As String, ByVal previousValue As String)
        	...
        	Doc.LoadXml(strDS)
        	Dim xmlParent As XmlNode = Doc.SelectSingleNode("//" & Session.Item(CNDataModelCode) & "_POLICY_BINDER/TRUSTEECLAIM/TRUSTEEAUDIT")
        	Dim v_sOI As String = Nothing
        	If xmlParent IsNot Nothing Then
        		v_sOI = xmlParent.Attributes("OI").Value
        	End If
        	Dim v_sScreenCode As String = TRUSTEECLAIM__TRUSTEEAUDIT.ScreenCode
        	Dim sParentElement, sChildElement As String
        	Dim sTmp() As String = Regex.Split(TRUSTEECLAIM__TRUSTEEAUDIT.ID, "__")
        	...
        	TRUSTEECLAIM__TRUSTEEAUDIT.DataSource = oXMLSource
        	TRUSTEECLAIM__TRUSTEEAUDIT.DataBind()
        ...
        ' TRUSTEECLAIM = The parent object name of child screen
        ' TRUSTEEAUDIT = The child screen object name
        
        Protected Sub TRUSTEECLAIM__TRUSTEEAUDIT_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles TRUSTEECLAIM__TRUSTEEAUDIT.RowDataBound
