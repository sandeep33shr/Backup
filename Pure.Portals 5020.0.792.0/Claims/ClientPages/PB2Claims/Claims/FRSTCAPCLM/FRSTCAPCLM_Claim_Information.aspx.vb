...
Namespace Nexus
    Partial Class PB2_FRSTCAPCLM_Claim_Information : Inherits BaseClaim
		...
		Protected Sub onValidate_FCAPCLAIM__FCAPTRAIL()
        
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_FCAPCLAIM__FCAPTRAIL()
End Sub
        ...
        Protected Sub UpdateAuditTrail()
        	...
        	Dim salvage As String = FCAPCLAIM__SALVAGESTAT.Text
        	Dim legal As String = FCAPCLAIM__LEGALSTAT.Text
        	Dim decision As String = FCAPCLAIM__CLMDECISION.Text
        	...
        	If (salvage_updatereq Or legal_updatereq Or decision_updatereq) Then
        		...
        		Session(CNDataSet) = oWebService.RunDefaultRulesEdit(FCAPCLAIM__FCAPAUDIT.ScreenCode, Session(CNDataSet))
        	End If
        ...
        Protected Function AuditEntryRequired(ByVal propertyName As String, ByVal propertyValue As String, ByRef previousValue As String) As Boolean
        	...
        	For Each el As Linq.XElement In datamodelXML.DescendantNodes()
        		If (el.Name.ToString().ToUpper() = "FCAPAUDIT") Then
        ...
        Protected Sub LogAuditEntry(ByVal propertyName As String, ByVal propertyValue As String, ByVal previousValue As String)
        	...
        	Doc.LoadXml(strDS)
        	Dim xmlParent As XmlNode = Doc.SelectSingleNode("//" & Session.Item(CNDataModelCode) & "_POLICY_BINDER/FCAPCLAIM/FCAPAUDIT")
        	Dim v_sOI As String = Nothing
        	If xmlParent IsNot Nothing Then
        		v_sOI = xmlParent.Attributes("OI").Value
        	End If
        	Dim v_sScreenCode As String = FCAPCLAIM__FCAPAUDIT.ScreenCode
        	Dim sParentElement, sChildElement As String
        	Dim sTmp() As String = Regex.Split(FCAPCLAIM__FCAPAUDIT.ID, "__")
        	...
        	FCAPCLAIM__FCAPAUDIT.DataSource = oXMLSource
        	FCAPCLAIM__FCAPAUDIT.DataBind()
        ...
        ' FCAPCLAIM = The parent object name of child screen
        ' FCAPAUDIT = The child screen object name
        
        Protected Sub FCAPCLAIM__FCAPAUDIT_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles FCAPCLAIM__FCAPAUDIT.RowDataBound
