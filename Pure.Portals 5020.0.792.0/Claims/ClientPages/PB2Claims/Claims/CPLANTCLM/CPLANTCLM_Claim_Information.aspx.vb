...
Namespace Nexus
    Partial Class PB2_CPLANTCLM_Claim_Information : Inherits BaseClaim
		...
		Protected Sub onValidate_CPLANTCLAIM__CPLANTRAIL()
        
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_CPLANTCLAIM__CPLANTRAIL()
End Sub
        ...
        Protected Sub UpdateAuditTrail()
        	...
        	Dim salvage As String = CPLANTCLAIM__SALVAGESTAT.Text
        	Dim legal As String = CPLANTCLAIM__LEGALSTAT.Text
        	Dim decision As String = CPLANTCLAIM__CLMDECISION.Text
        	...
        	If (salvage_updatereq Or legal_updatereq Or decision_updatereq) Then
        		...
        		Session(CNDataSet) = oWebService.RunDefaultRulesEdit(CPLANTCLAIM__CPLANTAUDIT.ScreenCode, Session(CNDataSet))
        	End If
        ...
        Protected Function AuditEntryRequired(ByVal propertyName As String, ByVal propertyValue As String, ByRef previousValue As String) As Boolean
        	...
        	For Each el As Linq.XElement In datamodelXML.DescendantNodes()
        		If (el.Name.ToString().ToUpper() = "CPLANTAUDIT") Then
        ...
        Protected Sub LogAuditEntry(ByVal propertyName As String, ByVal propertyValue As String, ByVal previousValue As String)
        	...
        	Doc.LoadXml(strDS)
        	Dim xmlParent As XmlNode = Doc.SelectSingleNode("//" & Session.Item(CNDataModelCode) & "_POLICY_BINDER/CPLANTCLAIM/CPLANTAUDIT")
        	Dim v_sOI As String = Nothing
        	If xmlParent IsNot Nothing Then
        		v_sOI = xmlParent.Attributes("OI").Value
        	End If
        	Dim v_sScreenCode As String = CPLANTCLAIM__CPLANTAUDIT.ScreenCode
        	Dim sParentElement, sChildElement As String
        	Dim sTmp() As String = Regex.Split(CPLANTCLAIM__CPLANTAUDIT.ID, "__")
        	...
        	CPLANTCLAIM__CPLANTAUDIT.DataSource = oXMLSource
        	CPLANTCLAIM__CPLANTAUDIT.DataBind()
        ...
        ' CPLANTCLAIM = The parent object name of child screen
        ' CPLANTAUDIT = The child screen object name
        
        Protected Sub CPLANTCLAIM__CPLANTAUDIT_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles CPLANTCLAIM__CPLANTAUDIT.RowDataBound
