...
Namespace Nexus
    Partial Class PB2_CPLANTCLM_Underwriting_Information : Inherits BaseClaim
		...
		Protected Sub onValidate_CPLANTCLAIM__IS_FAC()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_CPLANTCLAIM__IS_FAC()
End Sub
        ...
        Protected Shadows Sub Page_Load_ClientInfo(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
            ...
            If Session(CNParty) IsNot Nothing Then
                If TypeOf Session(CNParty) Is NexusProvider.CorporateParty Then
                    ...
                    CPLANTCLAIM__INSURED.Text = DirectCast(oParty, NexusProvider.CorporateParty).CompanyName
                End If
                ...
                If TypeOf Session(CNParty) Is NexusProvider.PersonalParty Then
                    ...
                    CPLANTCLAIM__INSURED.Text = (firstName + " " + LastName).ToString
        			CPLANTCLAIM__CONTACTNUM.Text = Trim(MobileNo).ToString
                End If
            ...
            If Session(CNClaimQuote) IsNot Nothing Then
                CPLANTCLAIM__POLNUMBER.Text = RTrim(LTrim(oClaimQuote.InsuranceFileRef))
                CPLANTCLAIM__ALTERPOLICY.Text = RTrim(LTrim(oClaimQuote.AlternativeRef))
                CPLANTCLAIM__CVR_STARTDATE.Text = RTrim(LTrim(oClaimQuote.CoverStartDate))
                CPLANTCLAIM__CVR_ENDDATE.Text = RTrim(LTrim(oClaimQuote.CoverEndDate))
                CPLANTCLAIM__INCEPT_DATE.Text = RTrim(LTrim(oClaimQuote.InceptionDate))
        		...
        		For nIndex = 0 To Cnt
        			...
        			End If
        		Next
        		CPLANTCLAIM__IS_FAC.Text = FAC.ToString
            End If
