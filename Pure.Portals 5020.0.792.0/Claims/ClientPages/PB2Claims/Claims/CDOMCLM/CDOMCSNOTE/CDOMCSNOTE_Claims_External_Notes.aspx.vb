...
Namespace Nexus
    Partial Class PB2_CDOMCSNOTE_Claims_External_Notes : Inherits BaseClaim
		...
		Protected Sub onValidate_CMDM_DETAILS__Note_Details()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        	If CMDM_DETAILS__Date_Created.Text.ToString = "" Then
        		CMDM_DETAILS__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        		CMDM_DETAILS__Created_By.Text = Session(CNLoginName).ToString
        	End If
        End If
End Sub
Protected Sub CallRuleScripts()
    onValidate_CMDM_DETAILS__Note_Details()
End Sub
