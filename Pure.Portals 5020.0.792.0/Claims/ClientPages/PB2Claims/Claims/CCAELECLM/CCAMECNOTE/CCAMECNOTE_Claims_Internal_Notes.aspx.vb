...
Namespace Nexus
    Partial Class PB2_CCAMECNOTE_Claims_Internal_Notes : Inherits BaseClaim
		...
		Protected Sub onValidate_INT_DETAILS__Note_Details()
        If eLifecycleEvent = LifecycleEvent.Page_Load Then
        	If INT_DETAILS__Date_Created.Text.ToString = "" Then
        		INT_DETAILS__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        		INT_DETAILS__Created_By.Text = Session(CNLoginName).ToString
        	End If
        End If
End Sub
Protected Sub CallRuleScripts()
    onValidate_INT_DETAILS__Note_Details()
End Sub
