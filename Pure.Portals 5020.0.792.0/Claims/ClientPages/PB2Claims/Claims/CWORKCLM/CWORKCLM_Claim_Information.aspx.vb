...
Namespace Nexus
    Partial Class PB2_CWORKCLM_Claim_Information : Inherits BaseClaim
		...
		Protected Sub onValidate_CWORKCLAIM__CWORKRAIL()
        
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_CWORKCLAIM__CWORKRAIL()
End Sub
        ...
        Protected Sub UpdateAuditTrail()
        	...
        	Dim salvage As String = CWORKCLAIM__SALVAGESTAT.Text
        	Dim legal As String = CWORKCLAIM__LEGALSTAT.Text
        	Dim decision As String = CWORKCLAIM__CLMDECISION.Text
        	...
        	If (salvage_updatereq Or legal_updatereq Or decision_updatereq) Then
        		...
        		Session(CNDataSet) = oWebService.RunDefaultRulesEdit(CWORKCLAIM__CWORKAUDIT.ScreenCode, Session(CNDataSet))
        	End If
        ...
        Protected Function AuditEntryRequired(ByVal propertyName As String, ByVal propertyValue As String, ByRef previousValue As String) As Boolean
        	...
        	For Each el As Linq.XElement In datamodelXML.DescendantNodes()
        		If (el.Name.ToString().ToUpper() = "CWORKAUDIT") Then
        ...
        Protected Sub LogAuditEntry(ByVal propertyName As String, ByVal propertyValue As String, ByVal previousValue As String)
        	...
        	Doc.LoadXml(strDS)
        	Dim xmlParent As XmlNode = Doc.SelectSingleNode("//" & Session.Item(CNDataModelCode) & "_POLICY_BINDER/CWORKCLAIM/CWORKAUDIT")
        	Dim v_sOI As String = Nothing
        	If xmlParent IsNot Nothing Then
        		v_sOI = xmlParent.Attributes("OI").Value
        	End If
        	Dim v_sScreenCode As String = CWORKCLAIM__CWORKAUDIT.ScreenCode
        	Dim sParentElement, sChildElement As String
        	Dim sTmp() As String = Regex.Split(CWORKCLAIM__CWORKAUDIT.ID, "__")
        	...
        	CWORKCLAIM__CWORKAUDIT.DataSource = oXMLSource
        	CWORKCLAIM__CWORKAUDIT.DataBind()
        ...
        ' CWORKAUDIT = The child screen object name
        
        Protected Sub CWORKCLAIM__CWORKAUDIT_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles CWORKCLAIM__CWORKAUDIT.RowDataBound
