...
Namespace Nexus
    Partial Class PB2_DOMCLM_Claim_Information : Inherits BaseClaim
		...
		Protected Sub onValidate_DOMCLAIM__DOMTRAIL()
        
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_DOMCLAIM__DOMTRAIL()
End Sub
        ...
        Protected Sub UpdateAuditTrail()
        	...
        	Dim salvage As String = DOMCLAIM__SALVAGESTAT.Text
        	Dim legal As String = DOMCLAIM__LEGALSTAT.Text
        	Dim decision As String = DOMCLAIM__CLMDECISION.Text
        	...
        	If (salvage_updatereq Or legal_updatereq Or decision_updatereq) Then
        		...
        		Session(CNDataSet) = oWebService.RunDefaultRulesEdit(DOMCLAIM__DOMAUDIT.ScreenCode, Session(CNDataSet))
        	End If
        ...
        Protected Function AuditEntryRequired(ByVal propertyName As String, ByVal propertyValue As String, ByRef previousValue As String) As Boolean
        	...
        	For Each el As Linq.XElement In datamodelXML.DescendantNodes()
        		If (el.Name.ToString().ToUpper() = "DOMAUDIT") Then
        ...
        Protected Sub LogAuditEntry(ByVal propertyName As String, ByVal propertyValue As String, ByVal previousValue As String)
        	...
        	Doc.LoadXml(strDS)
        	Dim xmlParent As XmlNode = Doc.SelectSingleNode("//" & Session.Item(CNDataModelCode) & "_POLICY_BINDER/DOMCLAIM/DOMAUDIT")
        	Dim v_sOI As String = Nothing
        	If xmlParent IsNot Nothing Then
        		v_sOI = xmlParent.Attributes("OI").Value
        	End If
        	Dim v_sScreenCode As String = DOMCLAIM__DOMAUDIT.ScreenCode
        	Dim sParentElement, sChildElement As String
        	Dim sTmp() As String = Regex.Split(DOMCLAIM__DOMAUDIT.ID, "__")
        	...
        	DOMCLAIM__DOMAUDIT.DataSource = oXMLSource
        	DOMCLAIM__DOMAUDIT.DataBind()
        ...
        ' DOMCLAIM = The parent object name of child screen
        ' DOMAUDIT = The child screen object name
        
        Protected Sub DOMCLAIM__DOMAUDIT_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles DOMCLAIM__DOMAUDIT.RowDataBound
