...
Namespace Nexus
    Partial Class PB2_TSMCNOTE_Claims_Internal_Notes : Inherits BaseClaim
		...
		Protected Sub onValidate_TSMCNOTE_DETAILS__Note_Details()
                If eLifecycleEvent = LifecycleEvent.Page_Load Then
                        If CNOTE_DETAILS__Date_Created.Text.ToString = "" Then
                            CNOTE_DETAILS__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        					CNOTE_DETAILS__Created_By.Text = Session(CNLoginName).ToString
                        End If
                    End If
End Sub
Protected Sub onValidate_TSMCNOTE_DETAILS__NoteCheck()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_TSMCNOTE_DETAILS__Note_Details()
    onValidate_TSMCNOTE_DETAILS__NoteCheck()
End Sub
        ...
        Protected Sub NotesAdded()
        	Dim NoteAddedCheck As Integer =Cint(TSMCNOTE_DETAILS__NoteCheck.Text)
        	
        	If NoteAddedCheck = 0 Then
        		TSMCNOTE_DETAILS__NoteCheck.Text = 1
        	End If
        End Sub
