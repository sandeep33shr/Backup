...
Namespace Nexus
    Partial Class PB2_AHACLMS_Personal_Accident : Inherits BaseClaim
...
End Sub
Protected Sub onValidate_GENERAL__AHCTRAIL()
        
        
End Sub
Protected Sub onValidate_AHACLM__CLM_AHCOI()
        
End Sub
Protected Sub onValidate_AHACLM__CLM_AHNDOS()
        
End Sub
Protected Sub onValidate_AHACLM__CLMAHNOTES()
        
End Sub
Protected Sub onValidate_AHACLM__CLM_AHNOTE()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_GENERAL__TRANSACTION_TYPE()
    onValidate_GENERAL__AHCTRAIL()
    onValidate_AHACLM__CLM_AHCOI()
    onValidate_AHACLM__CLM_AHNDOS()
    onValidate_AHACLM__CLMAHNOTES()
    onValidate_AHACLM__CLM_AHNOTE()
End Sub
        ...
        Protected Sub UpdateAuditTrail()
        	...
        	Dim salvage As String = GENERAL__SALVAGESTAT.Text
        	Dim legal As String = GENERAL__LEGALSTAT.Text
        	Dim decision As String = GENERAL__CLMDECISION.Text
        	...
        	If (salvage_updatereq Or legal_updatereq Or decision_updatereq) Then
        		...
        		Session(CNDataSet) = oWebService.RunDefaultRulesEdit(GENERAL__AUDIT.ScreenCode, Session(CNDataSet))
        	End If
        ...
        Protected Function AuditEntryRequired(ByVal propertyName As String, ByVal propertyValue As String, ByRef previousValue As String) As Boolean
        	...
        	For Each el As Linq.XElement In datamodelXML.DescendantNodes()
        		If (el.Name.ToString().ToUpper() = "AUDIT") Then
        ...
        Protected Sub LogAuditEntry(ByVal propertyName As String, ByVal propertyValue As String, ByVal previousValue As String)
        	...
        	Doc.LoadXml(strDS)
        	Dim xmlParent As XmlNode = Doc.SelectSingleNode("//" & Session.Item(CNDataModelCode) & "_POLICY_BINDER/GENERAL/AUDIT")
        	Dim v_sOI As String = Nothing
        	If xmlParent IsNot Nothing Then
        		v_sOI = xmlParent.Attributes("OI").Value
        	End If
        	Dim v_sScreenCode As String = GENERAL__AUDIT.ScreenCode
        	Dim sParentElement, sChildElement As String
        	Dim sTmp() As String = Regex.Split(GENERAL__AUDIT.ID, "__")
        	...
        	GENERAL__AUDIT.DataSource = oXMLSource
        	GENERAL__AUDIT.DataBind()
        ...
        ' GENERAL = The parent object name of child screen
        ' AUDIT = The child screen object name
        
        Protected Sub GENERAL__AUDIT_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles GENERAL__AUDIT.RowDataBound
        ...
        ' Note: This rule removes the "Add" button row of a child-screen grid
        ' AHACLM = The parent object name of child screen
        ' AHCOI = The child screen object name
        
        Protected Sub AHACLM__AHCOI_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles AHACLM__AHCOI.RowDataBound
        	If e.Row.RowType = DataControlRowType.Footer Then
        		e.Row.Visible = False
        	End If
        ...
        ' Note: This rule removes the "Add" button row of a child-screen grid
        ' AHACLM = The parent object name of child screen
        ' AH_ENDOSE = The child screen object name
        
        Protected Sub AHACLM__AH_ENDOSE_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles AHACLM__AH_ENDOSE.RowDataBound
        	If e.Row.RowType = DataControlRowType.Footer Then
        		e.Row.Visible = False
        	End If
        ...
        ' Note: This rule removes the "Add" button row of a child-screen grid
        ' AHACLM = The parent object name of child screen
        ' AHNOTES = The child screen object name
        
        Protected Sub AHACLM__AHNOTES_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles AHACLM__AHNOTES.RowDataBound
        	If e.Row.RowType = DataControlRowType.Footer Then
        		e.Row.Visible = False
        	End If
        ...
        ' Note: This rule removes the "Add" button row of a child-screen grid
        ' AHACLM = The parent object name of child screen
        ' AHSNOTES = The child screen object name
        
        Protected Sub AHACLM__AHSNOTES_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles AHACLM__AHSNOTES.RowDataBound
        	If e.Row.RowType = DataControlRowType.Footer Then
        		e.Row.Visible = False
        	End If
