...
Namespace Nexus
    Partial Class PB2_TRAVSCHCLM_Claim_Information : Inherits BaseClaim
		...
		Protected Sub onValidate_TRAVCLAIM__TRAVTRAIL()
        
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_TRAVCLAIM__TRAVTRAIL()
End Sub
        ...
        Protected Sub UpdateAuditTrail()
        	...
        	Dim salvage As String = TRAVCLAIM__SALVAGESTAT.Text
        	Dim legal As String = TRAVCLAIM__LEGALSTAT.Text
        	Dim decision As String = TRAVCLAIM__CLMDECISION.Text
        	...
        	If (salvage_updatereq Or legal_updatereq Or decision_updatereq) Then
        		...
        		Session(CNDataSet) = oWebService.RunDefaultRulesEdit(TRAVCLAIM__TRAVAUDIT.ScreenCode, Session(CNDataSet))
        	End If
        ...
        Protected Function AuditEntryRequired(ByVal propertyName As String, ByVal propertyValue As String, ByRef previousValue As String) As Boolean
        	...
        	For Each el As Linq.XElement In datamodelXML.DescendantNodes()
        		If (el.Name.ToString().ToUpper() = "TRAVAUDIT") Then
        ...
        Protected Sub LogAuditEntry(ByVal propertyName As String, ByVal propertyValue As String, ByVal previousValue As String)
        	...
        	Doc.LoadXml(strDS)
        	Dim xmlParent As XmlNode = Doc.SelectSingleNode("//" & Session.Item(CNDataModelCode) & "_POLICY_BINDER/TRAVCLAIM/TRAVAUDIT")
        	Dim v_sOI As String = Nothing
        	If xmlParent IsNot Nothing Then
        		v_sOI = xmlParent.Attributes("OI").Value
        	End If
        	Dim v_sScreenCode As String = TRAVCLAIM__TRAVAUDIT.ScreenCode
        	Dim sParentElement, sChildElement As String
        	Dim sTmp() As String = Regex.Split(TRAVCLAIM__TRAVAUDIT.ID, "__")
        	...
        	TRAVCLAIM__TRAVAUDIT.DataSource = oXMLSource
        	TRAVCLAIM__TRAVAUDIT.DataBind()
        ...
        ' TRAVCLAIM = The parent object name of child screen
        ' TRAVAUDIT = The child screen object name
        
        Protected Sub TRAVCLAIM__TRAVAUDIT_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles TRAVCLAIM__TRAVAUDIT.RowDataBound
