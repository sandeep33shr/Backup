...
Namespace Nexus
    Partial Class PB2_ENVGITCLM_Claim_Information : Inherits BaseClaim
		...
		Protected Sub onValidate_ENVGITCLAIM__ENVGTRAIL()
        
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_ENVGITCLAIM__ENVGTRAIL()
End Sub
        ...
        Protected Sub UpdateAuditTrail()
        	...
        	Dim salvage As String = ENVGITCLAIM__SALVAGESTAT.Text
        	Dim legal As String = ENVGITCLAIM__LEGALSTAT.Text
        	Dim decision As String = ENVGITCLAIM__CLMDECISION.Text
        	...
        	If (salvage_updatereq Or legal_updatereq Or decision_updatereq) Then
        		...
        		Session(CNDataSet) = oWebService.RunDefaultRulesEdit(ENVGITCLAIM__ENVGITAUDIT.ScreenCode, Session(CNDataSet))
        	End If
        ...
        Protected Function AuditEntryRequired(ByVal propertyName As String, ByVal propertyValue As String, ByRef previousValue As String) As Boolean
        	...
        	For Each el As Linq.XElement In datamodelXML.DescendantNodes()
        		If (el.Name.ToString().ToUpper() = "ENVGITAUDIT") Then
        ...
        Protected Sub LogAuditEntry(ByVal propertyName As String, ByVal propertyValue As String, ByVal previousValue As String)
        	...
        	Doc.LoadXml(strDS)
        	Dim xmlParent As XmlNode = Doc.SelectSingleNode("//" & Session.Item(CNDataModelCode) & "_POLICY_BINDER/ENVGITCLAIM/ENVGITAUDIT")
        	Dim v_sOI As String = Nothing
        	If xmlParent IsNot Nothing Then
        		v_sOI = xmlParent.Attributes("OI").Value
        	End If
        	Dim v_sScreenCode As String = ENVGITCLAIM__ENVGITAUDIT.ScreenCode
        	Dim sParentElement, sChildElement As String
        	Dim sTmp() As String = Regex.Split(ENVGITCLAIM__ENVGITAUDIT.ID, "__")
        	...
        	ENVGITCLAIM__ENVGITAUDIT.DataSource = oXMLSource
        	ENVGITCLAIM__ENVGITAUDIT.DataBind()
        ...
        ' ENVGITAUDIT = The child screen object name
        
        Protected Sub ENVGITCLAIM__ENVGITAUDIT_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles ENVGITCLAIM__ENVGITAUDIT.RowDataBound
