...
Namespace Nexus
    Partial Class PB2_CFPCLM_Claim_Information : Inherits BaseClaim
		...
		Protected Sub onValidate_CFPCLAIM__CFPTRAIL()
        
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_CFPCLAIM__CFPTRAIL()
End Sub
        ...
        Protected Sub UpdateAuditTrail()
        	...
        	Dim salvage As String = CFPCLAIM__SALVAGESTAT.Text
        	Dim legal As String = CFPCLAIM__LEGALSTAT.Text
        	Dim decision As String = CFPCLAIM__CLMDECISION.Text
        	...
        	If (salvage_updatereq Or legal_updatereq Or decision_updatereq) Then
        		...
        		Session(CNDataSet) = oWebService.RunDefaultRulesEdit(CFPCLAIM__CFPAUDIT.ScreenCode, Session(CNDataSet))
        	End If
        ...
        Protected Function AuditEntryRequired(ByVal propertyName As String, ByVal propertyValue As String, ByRef previousValue As String) As Boolean
        	...
        	For Each el As Linq.XElement In datamodelXML.DescendantNodes()
        		If (el.Name.ToString().ToUpper() = "CFPAUDIT") Then
        ...
        Protected Sub LogAuditEntry(ByVal propertyName As String, ByVal propertyValue As String, ByVal previousValue As String)
        	...
        	Doc.LoadXml(strDS)
        	Dim xmlParent As XmlNode = Doc.SelectSingleNode("//" & Session.Item(CNDataModelCode) & "_POLICY_BINDER/CFPCLAIM/CFPAUDIT")
        	Dim v_sOI As String = Nothing
        	If xmlParent IsNot Nothing Then
        		v_sOI = xmlParent.Attributes("OI").Value
        	End If
        	Dim v_sScreenCode As String = CFPCLAIM__CFPAUDIT.ScreenCode
        	Dim sParentElement, sChildElement As String
        	Dim sTmp() As String = Regex.Split(CFPCLAIM__CFPAUDIT.ID, "__")
        	...
        	CFPCLAIM__CFPAUDIT.DataSource = oXMLSource
        	CFPCLAIM__CFPAUDIT.DataBind()
        ...
        ' CFPCLAIM = The parent object name of child screen
        ' CFPAUDIT = The child screen object name
        
        Protected Sub CFPCLAIM__CFPAUDIT_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles CFPCLAIM__CFPAUDIT.RowDataBound
