...
Namespace Nexus
    Partial Class PB2_PCOMPEQCLM_Claim_Information : Inherits BaseClaim
		...
		Protected Sub onValidate_COMPECLAIM__PCTRAIL()
        
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_COMPECLAIM__PCTRAIL()
End Sub
        ...
        Protected Sub UpdateAuditTrail()
        	...
        	Dim salvage As String = COMPECLAIM__SALVAGESTAT.Text
        	Dim legal As String = COMPECLAIM__LEGALSTAT.Text
        	Dim decision As String = COMPECLAIM__CLMDECISION.Text
        	...
        	If (salvage_updatereq Or legal_updatereq Or decision_updatereq) Then
        		...
        		Session(CNDataSet) = oWebService.RunDefaultRulesEdit(COMPECLAIM__PCAUDIT.ScreenCode, Session(CNDataSet))
        	End If
        ...
        Protected Sub LogAuditEntry(ByVal propertyName As String, ByVal propertyValue As String, ByVal previousValue As String)
        	...
        	Doc.LoadXml(strDS)
        	Dim xmlParent As XmlNode = Doc.SelectSingleNode("//" & Session.Item(CNDataModelCode) & "_POLICY_BINDER/COMPECLAIM/PCAUDIT")
        	Dim v_sOI As String = Nothing
        	If xmlParent IsNot Nothing Then
        		v_sOI = xmlParent.Attributes("OI").Value
        	End If
        	Dim v_sScreenCode As String = COMPECLAIM__PCAUDIT.ScreenCode
        	Dim sParentElement, sChildElement As String
        	Dim sTmp() As String = Regex.Split(COMPECLAIM__PCAUDIT.ID, "__")
        	...
        	COMPECLAIM__PCAUDIT.DataSource = oXMLSource
        	COMPECLAIM__PCAUDIT.DataBind()
        ...
        ' COMPECLAIM = The parent object name of child screen
        ' PCAUDIT = The child screen object name
        
        Protected Sub COMPECLAIM__PCAUDIT_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles COMPECLAIM__PCAUDIT.RowDataBound
