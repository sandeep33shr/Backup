...
Namespace Nexus
    Partial Class PB2_RTCCNOTE_Claims_Internal_Notes : Inherits BaseClaim
		...
		Protected Sub onValidate_ED_CNOTE_DETAILS__Note_Details()
                If eLifecycleEvent = LifecycleEvent.Page_Load Then
                        If ED_CNOTE_DETAILS__Date_Created.Text.ToString = "" Then
                            ED_CNOTE_DETAILS__Date_Created.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
        					ED_CNOTE_DETAILS__Created_By.Text = Session(CNLoginName).ToString
                        End If
                    End If
End Sub
Protected Sub onValidate_ED_CNOTE_DETAILS__NoteCheck()
        
End Sub
Protected Sub CallRuleScripts()
    onValidate_ED_CNOTE_DETAILS__Note_Details()
    onValidate_ED_CNOTE_DETAILS__NoteCheck()
End Sub

		    
        
        
        Protected Sub btnNextTop_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnNext.Click, btnNextTop.Click
        	NotesAdded()
        End Sub
        
        Protected Sub NotesAdded()
        	Dim NoteAddedCheck As Integer =Cint(ED_CNOTE_DETAILS__NoteCheck.Text)
        	
        	If NoteAddedCheck = 0 Then
        		ED_CNOTE_DETAILS__NoteCheck.Text = 1
        	End If
        End Sub
